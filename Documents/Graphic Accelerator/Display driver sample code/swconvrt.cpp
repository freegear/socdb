...
public:
    ...
    unsigned char    m_anBits[3];        // Number of bits per color
...
public:
    ...
    unsigned long
    BGRSrcToMaskedDst(
        unsigned long bgrSrc
        );

    unsigned long
    MaskedSrcToMaskedDst(
        unsigned long maskedSrc
       );
...
int
GroupColorConverter::InitLRU()
{
    return m_LRURecent.Init(NUMRECENTENTRIES);
}

void
GroupColorConverter::FreeLRU()
{
    m_LRURecent.Free();
}
    ...
    )
{
    ...
    PREFAST_ASSERT(listSize <= NUMRECENTENTRIES);
    ...
    m_usListSize = listSize;

    for( i=0; i < m_usListSize; i++ )
    {
        m_aiUniq[i]  = 0;
    ...
    )
{
    ...
    m_nPaletteSize = (unsigned short)XLATEOBJ_cGetPalette(pxo,XO_DESTPALETTE,0,(unsigned long *)NULL);
    ...
    )
{
    ...
    if( pxo->iDstType == PAL_INDEXED )
    {
        ...
        unsigned long * srcPtr = m_aLookup;
...
{
    short          lruIndex;
    unsigned long  dstValue    = 0;
    unsigned long  srcRGB;
    short          possibleDst;
    unsigned short lowGroup;
    unsigned short highGroup;
    unsigned long  error;
    unsigned long  smallestError;

    // We only want to look up RGB values, so mask off any alpha, or unused bits.
    if (m_Src.m_iType != PAL_RGB && m_Src.m_iType != PAL_BGR)
    {
        maskedSrc &= (m_Src.m_aMask[0] | m_Src.m_aMask[1] | m_Src.m_aMask[2]);
    }
    ...
    if( ( lruIndex = m_LRURecent.Find(maskedSrc) ) == -1 )
    {
        ...
        PREFAST_ASSERT(highGroup <= NUMINTENSITYGROUPS);
        PREFAST_ASSERT(lowGroup <= NUMINTENSITYGROUPS);
...
unsigned long
MaskColorConverter::RGBToFromBGR(
    unsigned long rgbSrc
    )
{
...
unsigned long
GroupColorConverter::RGBToFromBGR(
    unsigned long rgbSrc
    )
{
    return SwapRedBlue( rgbSrc );
...
unsigned long
MaskColorConverter::MaskedSrcToRGB(
    unsigned long srcValue
    )
{
    unsigned long ulRed   = (srcValue & m_Src.m_aMask[0]) << m_Src.m_anShift[0];
    unsigned long ulGreen = (srcValue & m_Src.m_aMask[1]) << m_Src.m_anShift[1];
    unsigned long ulBlue  = (srcValue & m_Src.m_aMask[2]) << m_Src.m_anShift[2];

    ulRed   |= ulRed   >> m_Src.m_anBits[0];
    ulGreen |= ulGreen >> m_Src.m_anBits[1];
    ulBlue  |= ulBlue  >> m_Src.m_anBits[2];
...
unsigned long
GroupColorConverter::MaskedSrcToRGB(
    unsigned long srcValue
    )
{
    unsigned long ulRed   = (srcValue & m_Src.m_aMask[0]) << m_Src.m_anShift[0];
...
unsigned long
MaskColorConverter::MaskedSrcToBGR(
    unsigned long srcValue
    )
{
    unsigned long ulRed   = (srcValue & m_Src.m_aMask[0]) << m_Src.m_anShift[0];
...
unsigned long
MaskColorConverter::RGBSrcToMaskedDst(
    unsigned long srcValue
    )
{
    return
...
unsigned long
LookupColorConverter::RGBSrcToMaskedDst(
    unsigned long srcValue
    )
{
    return
...
unsigned long
MaskColorConverter::BGRSrcToMaskedDst(
    unsigned long srcValue
    )
{
    return
...
unsigned long
MaskColorConverter::MaskedSrcToMaskedDst(
    unsigned long maskedSrc
    )
{
    unsigned long ulRed   = (maskedSrc & m_Src.m_aMask[0]) << m_Src.m_anShift[0];
    unsigned long ulGreen = (maskedSrc & m_Src.m_aMask[1]) << m_Src.m_anShift[1];
    unsigned long ulBlue  = (maskedSrc & m_Src.m_aMask[2]) << m_Src.m_anShift[2];
    ...
    return
        ((ulRed   >> m_Dst.m_anShift[0]) & m_Dst.m_aMask[0]) |
        ((ulGreen >> m_Dst.m_anShift[1]) & m_Dst.m_aMask[1]) |
        ((ulBlue  >> m_Dst.m_anShift[2]) & m_Dst.m_aMask[2]);
}

unsigned long
MonoColorConverter::AnyToMono(
    unsigned long srcValue
    )
{
    ...
    )
{
    ...
    case PAL_BITFIELDS:
        if (pPalette[0] == 0xf800
            && pPalette[1] == 0x7e0
            && pPalette[2] == 0x1f)
        {
            dstValue =  ( ( rgbColor << 24 ) >> 16 ) & *pPalette++;
            dstValue |= ( ( rgbColor << 16 ) >> 21 ) & *pPalette++;
            dstValue |= ( ( rgbColor <<  8 ) >> 27 ) & *pPalette++;
    ...
    )
{
    ...
    ULONG dstMasks[3] = {0xff, 0xff00, 0xff0000};
    ULONG * pDstMasks = dstMasks;
    ...
    case PAL_BITFIELDS:
        ...
        for(shiftRight = 24; (shiftRight >= 0) && (cEntries--); shiftRight -= 8)
        {
            ...
            ULONG color = (iRealizedColor & *pPalette++) << shiftLeft;

            color    |= color >> bits;
            dstValue |= (color >> shiftRight) & *pDstMasks++;
        }
