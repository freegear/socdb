    ...
    MIX        mix)
{
    ...
    parms.pDst       = pDst;
    parms.style      = 0;
    bRotate          = parms.pDst->IsRotate();

    if( plineattrs && (((mix>>8)&0x00ff)!=(mix&0x00ff))) // line 속성 체크
    {
        if( plineattrs->fl & LA_ALTERNATE )    // Every other pixel on
            parms.style = 0xaaaaaaaa;   // 점선 스타일.
...
//BUGBUG        if( firstrect )
//        {
//            DEBUGMSG(GPE_ZONE_WARNING,(TEXT("TBD\r\n")));
//            // Save linestate
//        }
//        else
//        {
...
//        }
        ...
        {
            rclCurrRot = *prclCurr;
            parms.pDst->RotateRectl(&rclCurrRot);
        }
    ...
    // FL_FLIP_H | FL_FLIP_V | FL_FLIP_D:
        ...
        do
        {
            ...
            for( pointNo=0; pointNo<=cptfx; pointNo++ )
            {
                ...
                // Calculate and store line state for next line segment
                // BUGBUG TBD !
                ...
                if (bRotate)
                {
                    parms.prclClip = &rclCurrRot;
                    parms.pDst->RotateLineParms(&parms);
                }
        ...
        // If not already done, save the linestate into the caller's structure
        // TBD !
    }
