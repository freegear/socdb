...
//	nFlag == 1 : 완성형 -> 조합형
//	nFlag == 0 : 조합형 -> 완성형
void	HangulCodeConvert(int nFlag, char* strSrc, char* strDest);
...
int printFont(char *pFont, int hangul)
{
    int i,j;
    int vCount;      
    unsigned char out, currLine;
...
int splitPrintJamo(unsigned wHan)
{	
	char abBuf[32];  
	int i;
	...
	ChoCode  = INDEXHF[0][(wHan & 0x7c00)>>10];
	JungCode = INDEXHF[1][(wHan & 0x03e0)>>5];
	JongCode = INDEXHF[2][ wHan & 0x001f];
	          	          
	if(JongCode !=0) Chk = 1;

	JongN=REFLAST10_8[JungCode];
	JungN=REFSECOND10_8[Chk][ChoCode];
	ChoN =REFFIRST8[Chk][JungCode]; 
	...
	ChoN  *= 640; 
	JungN *= 704; JungN += 640 * 8;	
	JongN *= 896; JongN += (640 * 8) + (704*4);
	                                              
	memcpy(abBuf,&HF[ChoN + ChoCode],32);
	      
	if(JungCode)
	{		
		for(i=0;i<32;i++) abBuf[i]|=HF[JungN + JungCode + i];
	}                

	if(JongCode)
	{
		for(i=0;i<32;i++) abBuf[i]|=HF[JongN + JongCode + i];  	
	}		
...
int printHChar(char *str)
{
	char buf[2];
	unsigned wHan;

	HangulCodeConvert(1, str, buf);

	wHan  = (buf[0]<<8) & 0x0000FFFF; 
	wHan |= (buf[1])    & 0x000000FF;	
...
int printString(char *str)
{
    ...
    str += 2;
		...
		wHan  = (str[0]<<8) & 0x0000FFFF; 
		wHan |= (str[1])    & 0x000000FF;
	} while(wHan != '\0');	           
...
void printEChar(char c)
{ 	
	char abBuf[16]; 
	              
	memcpy(abBuf,engFont + (c*16),16);
	printFont(abBuf, 0);	
}

void printEString(char *str)
{                   
	int i = strlen(str);
	int j;
