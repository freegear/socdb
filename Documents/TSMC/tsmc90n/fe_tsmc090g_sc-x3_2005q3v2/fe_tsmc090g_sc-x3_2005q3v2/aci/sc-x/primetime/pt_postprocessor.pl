...
#
    elsif (($cell_name =~ /^(RF[123]R1W|RFDO[12]W|DFF|EDFF|MDFF|JKFF|SDFF|SEDFF|SMDFF|TLAT|TTLAT|RSLAT)/) && 
...
#
	    ...
	    ((@var[2] eq "(posedge") || (@var[2] eq "(negedge")) &&
            ! ((@var[1]  =~ /clka|CLKA|clkb|CLKB/) && (@var[3] =~ /clka|clkb|CLKA|CLKB/))){
	@var[1]=~s/\\\[/\[/g;
	@var[1]=~s/\\\]/\]/g;
