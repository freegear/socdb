...
-- --------------------------------------------------------------------
--
-- Major Functions: I2S-AUDIO(LOOP-BACK TEST)
--
-- ---------------------------------------------------------------------
-- Revision History :
-- --------------------------------------------------------------------
...
--               |<------/------->|     ECONOMY      |
--      FPGA     |                |      AUDIO       |
--   (CONROLLER) | I2S-BUS SIGNAL |      CODEC       |
--               |<------/------->|                  |
-- --------------|                |------------------|
--                            
-----------------------------------------------------------------------
-- Refferance : WWW.XESS.COM  -Design Example(source)
-----------------------------------------------------------------------
...
--  signal left_chan : std_logic;          -- high when left channel is active; low when right channel is active
--  signal ADC       : std_logic_vector(RESOLUTION-1 downto 0);  -- digitized output from analog->digital converters
--  signal ADC_rdy   : std_logic;          -- high when output from ADC is ready
--  signal DAC       : std_logic_vector(RESOLUTION-1 downto 0);  -- input to codec digital->analog converters 
--  signal DAC_rdy   : std_logic;          -- high when input to DAC is needed
...
architecture arch of audio is
  ...
  PORT ( CLOCK : IN STD_LOGIC;
           ...
           L3_MODE : OUT STD_LOGIC);
  END COMPONENT;
  SIGNAL SERIAL_BUF : STD_LOGIC;
begin
  UL3 : L3_CONTROLLER PORT MAP ( CLK, RESET_N, L3_SCLK, L3_SDAT, L3_MODE );
...
--                      XSB_300E        =>      TRUE,  -- set the type of board being used
 ...
 serial_buf <= '0' when (sw='1') else sdto;
