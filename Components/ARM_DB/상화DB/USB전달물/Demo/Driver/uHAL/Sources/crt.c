 ...
 * ***************************************************************************
 *  C run time code, for example printf(), sprintf() etc.
 *
 *	$Id: crt.c,v 1.4 1999/12/02 14:19:14 mwelsh Exp $
 */
#include "uhal.h"
#include "except_h.h"
...
{
  while (count--) uHALr_putchar(c);
}

/* put string reverse */
void PutStringReverse(char *s, int index)
{
  while ((index--) > 0) uHALr_putchar(s[index]);
}
...
char *FormatItem(char *f, int a)
{
  ...
  while ((c = *f++)!=0) {
    ...
    case '-': leftjust = TRUE;
      break;
    case 'c': {
      if (leftjust) uHALr_putchar(a & 0x7f);
	...
	PutRepChar(fill, fieldwidth - uHALr_strlen((char *)a));
      if (!leftjust) PutString((char *) a);
      return(f);
    }
    case 'd': 
...
void uHALr_printf(char *f, ...)       /* variable arguments */
{
  U32 mode ;
  char *argP;

   if (lib_support_printf(f) == FALSE)
   {
