...
ROMonly		SETL	{FALSE}	; {TRUE} if no RAM at zero
	...
	; (CSLEE) This macro is block in the calling side
...
SELECTABLE_PRIMARY EQU 0
 ...
 IF SELECTABLE_PRIMARY <> 0
	...
	;
	;LDR	r5, =INTEGRATOR_DBG_BASE (CSLEE)
	;LDR	r5, [r5, #INTEGRATOR_DBG_SWITCH_OFFSET] ; Read switches (CSLEE)
	;AND	r5, r5, #0x6		; We only want switches 2 & 3 (middle two). (CSLEE)
	;MOV	r5, r5, LSR #1		; Shift right one to get selected header number (CSLEE)
	MOV r5, #0			; (CSLEE)
 ELSE
 ...
 ENDIF
	...
	;LDR	r6, [r4, #INTEGRATOR_HDR_STAT_OFFSET] ; Load HDR_STAT (CSLEE)
	MOV	r6, #0x00	; (CSLEE)
	AND	r6, r6, #0xFF		; Get header number
	CMP	r6, r5			; Is this the header select to be the primary	

	;LDR	r7, [r4, #INTEGRATOR_HDR_CTRL_OFFSET] (CSLEE)
	;ORREQ	r7, r7, #INTEGRATOR_HDR_CTRL_LED ; Primary - Ensure header LED is ON (CSLEE)
	;BICNE	r7, r7, #INTEGRATOR_HDR_CTRL_LED ; Secondary - Ensure header LED is OFF (CSLEE)
	;STR	r7, [r4, #INTEGRATOR_HDR_CTRL_OFFSET] (CSLEE)
	BEQ	%F5			; Continue
	...
	;
	;LDR	r7, =INTEGRATOR_RTC_BASE (CSLEE)
;1	LDR	r4, [r7] (CSLEE)
	;ANDS	r4, r4, #1 (CSLEE)
	;MOVNE	r4, #YELLOW_LED (CSLEE)
	; SET_LEDS r4, r6 (CSLEE)
	;B	%B1
...
3	LDR	r7, [r4, #INTEGRATOR_HDR_CTRL_OFFSET]
	...
	;
5	MOV	r4, #0
	; SET_LEDS r4, r5		; Ensure motherboard LED's are OFF (CSLEE)
...
;1	LDR	$w3, =INTEGRATOR_RTC_BASE	(CSLEE)
	;LDR	$w1, [$w3]		(CSLEE)
	;ANDS	$w1, $w1, #1	(CSLEE)
	;MOVNE	$w1, #RED_LED	(CSLEE)
...
2
	...
	; (CSLEE) This macro is blocked in calling side
	MACRO
	...
	;
	;LDR	$w1, =INTEGRATOR_HDR_BASE (CSLEE)
	;LDR	$w2, [$w1, #INTEGRATOR_HDR_OSC_OFFSET] (CSLEE)
	;AND	$w2, $w2, #INTEGRATOR_HDR_OSC_BUS_MODE_MASK (CSLEE)
	;CMP	$w2, #INTEGRATOR_HDR_OSC_BUS_MODE_CM9x0 (CSLEE)
	...
	;
	;LDRLT	$w2, [$w1, #INTEGRATOR_HDR_CTRL_OFFSET]	(CSLEE)
	;BICLT	$w2, $w2, #INTEGRATOR_HDR_CTRL_FASTBUS (CSLEE)
	;STRLT	$w2, [$w1, #INTEGRATOR_HDR_CTRL_OFFSET] (CSLEE)
	...
	; (CSLEE) This macro is not called
	MACRO
	...
	LDR	$w1, =INTEGRATOR_HDR_BASE
	;LDR	$w3, [$w1, #INTEGRATOR_HDR_PROC_OFFSET]	(CSLEE) Block
	MOV		$w3, #0x01	; (CSLEE) set $w3 to non-zero value
	CMP	$w3, #0
	BNE	%F01
...
1
	...
	LDR	$w1, =INTEGRATOR_HDR_BASE
	;LDR	$w3, =0xA05F	(CSLEE)
	;STR	$w3, [$w1, #INTEGRATOR_HDR_LOCK_OFFSET]	(CSLEE)
	;STR	$w2, [$w1, #INTEGRATOR_HDR_OSC_OFFSET]	(CSLEE)
	;MOV	$w2, #0	(CSLEE)
	;STR	$w2, [$w1, #INTEGRATOR_HDR_LOCK_OFFSET]	(CSLEE)
	...
	; VALIDATE_PROCESSOR	$w1, $w2, $w3 (CSLEE) Block Macro
	; (CSLEE) VALIDATE_PROCESSOR checks the validty of CPU and VENDOR
	; (CSLEE) If the types do not matched with expected value, turn on LED

	;SETUP_ASYNC_CLOCKS	$w1, $w2	(CSLEE) Block Macro
	...
	;
	;LDR	$w1, =INTEGRATOR_HDR_SDRAM	; Load address of HDR_SDRAM (CSLEE)
	;MOV	$w2, #0x10000		; Load count (CSLEE)
;1	LDR	$w3, [$w1]		; Load contents of HDR_SDRAM (CSLEE)
	;TST	$w3, #INTEGRATOR_HDR_SDRAM_SPD_OK ; Check to see if SPD data is loaded (CSLEE)
	;BNE	%F02	(CSLEE)
	;SUBS	$w2, $w2, #1	; Decrement the count (CSLEE)
	;BGT	%B01			; Try again	(CSLEE)
	;B	%F05

	;
	; Verify the SPD checksum
	;
;2	LDR	$w1, =INTEGRATOR_HDR_SPDBASE	; Load address of the base of SPD data (CSLEE)
	;MOV	$w3, #0	(CSLEE)

;3	LDRB	$w2, [$w1], #1		; Load byte	(CSLEE)
	;ADD	$w3, $w3, $w2		; Add to checksum (CSLEE)
	;LDR	$w2, =(INTEGRATOR_HDR_SPDBASE + 62) ; Have we got to byte 62 (CSLEE)
	;CMP	$w1, $w2 (CSLEE)
	;BLS	%B03 (CSLEE)

	;LDR	$w1, =INTEGRATOR_HDR_SPDBASE	; Load address of the base of SPD data (CSLEE)
	;LDRB	$w2, [$w1, #63]		; Get checksum from SPD (CSLEE)
	;AND	$w3, $w3, #0xFF		; Mask out calculated checksum (CSLEE)
	;CMP	$w2, $w3		; Are they the same (CSLEE)
...
;	BNE	%F05
	...
	;
	;LDRB	$w2, [$w1, #31]		; Get Module Bank Density	(CSLEE)
	;MOV	$w2, $w2, LSL #2	; Multiply by 4	(CSLEE)
	;LDRB	$w3, [$w1, #5]		; Get Number of Banks (CSLEE)
	;MULS	$w2, $w3, $w2		; Multiple together to get size in MBytes (CSLEE)
	;BEQ	%F05			; If zero then something has gone wrong (CSLEE)

	;
	; The maximum SDRAM DIMM supported is 256M
	;
	;CMP	$w2, #256	; (CSLEE) Compare Size With 256MBytes (CSLEE)
	;BGT	%F05		; (CSLEE) If memory size > 256M, goto 05 (CSLEE)
	...
	; zeros.
	;
	;MOV	$w1, #0			; Initialise the counter (CSLEE)
4	;TST	$w2, #1			; Is the bottom bit set of the size varible (CSLEE)
	;MOVEQ	$w2, $w2, LSR #1	; If not set then divide by 2 (CSLEE)
	;ADDEQ	$w1, $w1, #1		; If not set then increment the counter (CSLEE)
	;BEQ	%B04			; If not set then loop (CSLEE)

	; (CSLEE) check the validity of memory size information($w2)
	;CMP	$w2, #1			; $w2 should now contain 1 (CSLEE)
						; the size of memory should be exponential of 2
						
						; (CSLEE) $w1 now contain the MEMSIZE[4:2] field value
	;BNE	%F05			; If it doesn't then something has gone wrong (CSLEE)

	;LDR	$w2, =INTEGRATOR_HDR_BASE ; Load base address of header registers (CSLEE)
	;LDR	$w3, [$w2, #INTEGRATOR_HDR_SDRAM_OFFSET] ; Load contents of HDR_SDRAM (CSLEE)
	;AND	$w3, $w3, #3		; Clear the everything expect CASLAT (CSLEE)
	;SUBS	$w1, $w1, #4		; Subtract 4 from the number of trailing bits (CSLEE)
	;BMI	%F05   			; If negative then something has gone wrong (CSLEE)
	;ORR	$w3, $w3, $w1, LSL #2	; Merge it into contents of HDR_SDRAM (CSLEE)
	; (CSLEE) $w3 contains the value for being written into [#INTEGRATOR_HDR_SDRAM_OFFSET]

	;LDRB	$w1, [$w2, #(INTEGRATOR_HDR_SPDBASE_OFFSET + 3)] ; No. of Rows (CSLEE)
	;AND	$w1, $w1, #0xF		; Only want bottom 4 bits (CSLEE)
	;ORR	$w3, $w3, $w1, LSL #8	; Merge into HDR_SDRAM (CSLEE)

	;LDRB	$w1, [$w2, #(INTEGRATOR_HDR_SPDBASE_OFFSET + 4)] ; No. of Columns (CSLEE)
	;AND	$w1, $w1, #0xF		; Only want bottom 4 bits (CSLEE)
	;ORR	$w3, $w3, $w1, LSL #12	; Merge into HDR_SDRAM (CSLEE)

	;LDRB	$w1, [$w2, #(INTEGRATOR_HDR_SPDBASE_OFFSET + 5)] ; No. of Banks (CSLEE)
	;AND	$w1, $w1, #0xF		; Only want bottom 4 bits (CSLEE)
	;ORR	$w3, $w3, $w1, LSL #16	; Merge into HDR_SDRAM (CSLEE)

	;STR	$w3, [$w2, #INTEGRATOR_HDR_SDRAM_OFFSET] ; Write back to HDR_SDRAM (CSLEE)
	...
	; being 1 Mbyte).
	;
	;MOV	$w1, $w3, LSR #2	; Need to extract MEMSIZE from the (CSLEE)
	;AND	$w1, $w1, #0x7		;  the value we wrote to HDR_SDRAM (CSLEE)

	;MOV	$w2, #1			; Load 1 (CSLEE)
	;ADD	$w1, $w1, #24		; Add 24 to the MEMSIZE value (CSLEE)
	;MOV	$w1, $w2, LSL $w1	; Shift 1 by (24 + MEMSIZE) (CSLEE)
	;B	%F06

	; (CSLEE) $w1 contains memory size information
;5	MOV	$w1, #0			; Could not find any good DRAM (CSLEE)

	MOV $w2, #1			; (CSLEE) Add
	MOV $w1, $w2, LSL #25	; (CSLEE) Add, 32MBytes SDRAM
;6	LDR	$w2, =INTEGRATOR_HDR_BASE ; Load base address of header registers (CSLEE)
	;LDR	$w2, [$w2, #INTEGRATOR_HDR_STAT_OFFSET] ; Load contents of HDR_STAT (CSLEE)
	;ANDS	$w2, $w2, #0xFF0000	; Clear all but bits 23:16 to get SSRAM size (CSLEE)
	;MOVEQ	$w2, #SZ_256K		; If zero then this is a old header with 256K (CSLEE)
								; i.e. older version doesn't contains the SSRAM size information
	MOV	$w2, #0x00		; (CSLEE) SSRAM size = 0x00
	CMP	$w1, $w2		; Is there less SDRAM than the SSRAM
						; (CSLEE) Compare SDRAMsize($w1) and SSRAMsize($w2)
	MOVMI	$w1, $w2	; If so then return the size of the SSRAM
						; (CSLEE) return value -> $w1 = uHALiv_TopOfMemory
	MOV	$w3, $w1		; Need to return size in both these registers
						; (CSLEE) return value -> $w3 = ramsize
...
$label	LDR	$w2, =INTEGRATOR_HDR_BASE
	...
	;
	;LDR	$w1, [$w2, #INTEGRATOR_HDR_STAT_OFFSET] ; Get contents of HDR_STAT (CSLEE)
	MOV $w1, #0x00	; (CSLEE)
...
$label  READ_INT	$w1, $w2, $w3
	...
	;
	;LDR	$w2, =INTEGRATOR_HDR_BASE (CSLEE)
	;LDR	$w1, [$w2, #(INTEGRATOR_HDR_IC_OFFSET+IRQ_STATUS)] (CSLEE)
	;MOVS	$w1, $w1, LSL #INTEGRATOR_CM_INT0 (CSLEE)
	;BNE	%F1 (CSLEE)
	; (CSLEE) In Case Of CEUSAR, All interrupt is directly read from INTEGRATOR_IC_BASE
	...
	;
	;LDR	$w1, [$w2, #INTEGRATOR_HDR_STAT_OFFSET] ; Get contents of HDR_STAT (CSLEE)
	MOV $w1, #0x00	; (CSLEE) Select Core Module 0
	LDR	$w2, =INTEGRATOR_IC_BASE	; Get base address of the interrupt controller
	AND	$w1, $w1, #3			; Mask off header number from HDR_STAT
...
$label	DISABLE_INTS	$w1, $w2
	...
	;
	;LDR	$w1, =INTEGRATOR_HDR_BASE (CSLEE)
	;LDR	$w2, =0xFFFFFFFF (CSLEE)
	;STR	$w2, [$w1, #(INTEGRATOR_HDR_IC_OFFSET+IRQ_ENABLE_CLEAR)] (CSLEE)
	;STR	$w2, [$w1, #(INTEGRATOR_HDR_IC_OFFSET+FIQ_ENABLE_CLEAR)] (CSLEE)

	;
	; Now we need to calculate the address of the
	; interrupt controller for this header.
	;
	;LDR	$w1, [$w1, #INTEGRATOR_HDR_STAT_OFFSET] ; Get contents of HDR_STAT (CSLEE)
	MOV $w1, #0x00	; (CSLEE)
	LDR	$w2, =INTEGRATOR_IC_BASE	; Get base address of the interrupt controller
	AND	$w1, $w1, #3				; Mask off header number from HDR_STAT
	...
	;STR	$w1, [$w2, #FIQ_ENABLE_CLEAR]	; Clear all FIQ bits	(CSLEE)
	LDR $w2, =INTEGRATOR_IC_FIQBASE	; Get base address of FIQ interrupt controller (CSLEE)
	;INTEGRATOR_IC_FIQBASE is newly defined as 0x14400000 in platform.h (CSLEE)
	STR	$w1, [$w2, #0x0c]					; Clear all FIQ bits (CSLEE)
	...
	STRB	$w2, [$w1, #AMBA_UARTCR]	;AMBA_UARTCR definition changed
...
$label 	DO_DEBUG	$w1, $w2, $w3
 ...
 IF :DEF: DEBUG
	...
	;SET_LEDS	$w1, $w2	; Simple LED lighting (CSLEE)
