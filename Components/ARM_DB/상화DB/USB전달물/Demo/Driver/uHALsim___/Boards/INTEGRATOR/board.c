...
{
#ifdef	INTEGRATOR_CASE		// (CSLEE) Block
    U32 mask;
    volatile U32 *LedBank = (U32 *) LED_BANK;
...
#else
	IRQBase = (unsigned int *)(INTEGRATOR_IC_BASE);
	IRQBase[(IRQ_ENABLE_CLEAR / sizeof(unsigned int))] = (1 << irq);
#endif
...
{
    // unsigned int *headerP = (unsigned int *)INTEGRATOR_HDR_PROC; (CSLEE)
	unsigned int *headerP = 0;	// (CSLEE)
    return (*headerP);
}
...
#ifdef DEBUG
    /* Debug - write to LEDs */
    // uHALir_SetLEDs(RED_LED + GREEN_LED);	(CSLEE)
#endif
...
#if uHAL_PCI
    // uHALir_PCIInit();	(CSLEE)
#endif

#ifdef DEBUG
    // uHALir_SetLEDs(YELLOW_LED + GREEN_LED);	(CSLEE)
#endif
}
 ...
 * uHALir_InitSerial(OS_COMPORT, DEFAULT_OS_BAUD, DEFAULT_OS_BAUD_F)
 */
// void uHALir_InitSerial(unsigned int port, unsigned int baudRate) (CSLEE)
void uHALir_InitSerial(unsigned int port, unsigned int baudRate, unsigned int baudRate_F)
{
#ifdef SEMIHOSTED
...

    {
        ...
        // IO_WRITE(port + AMBA_UARTCR, 0x0); (CSLEE)
        IO_WRITE(port + AMBA_UARTCFG, 0x0);		// (CSLEE) Disable UART Block (0x04)
        IO_WRITE(port + AMBA_UARTCR, 0x00);		// (CSLEE) Disable All Interrupt (0x18)
        

        /* Set baud rate */
#ifdef	RYOO	// (Word Access)
        IO_WRITE(port + AMBA_UARTLCR_M, ((baudRate & 0xff00) >> 8));// (CSLEE) 0x09
        IO_WRITE(port + AMBA_UARTLCR_L, (baudRate & 0xff));			// (CSLEE) 0x08
#else
		IO_WRITE(port + AMBA_UARTLCR_L, (baudRate &0xffff));
#endif
        IO_WRITE(port + AMBA_UARTBAUDF, (baudRate_F & 0x3f));	// (CSLEE) Add New Code

        /* ----------v----------v----------v----------v---------- */
        /* NOTE: MUST BE WRITTEN LAST (AFTER UARTLCR_M & UARTLCR_L) */
        /* ----------^----------^----------^----------^---------- */
        /* set the UART to be 8 bits, 1 stop bit, no parity, fifo enabled */
        // IO_WRITE(port + AMBA_UARTLCR_H, (AMBA_UARTLCR_H_WLEN_8 | AMBA_UARTLCR_H_FEN)); (CSLEE)
        IO_WRITE(port + AMBA_UARTLCR_H, AMBA_UARTLCR_H_WLEN_8);	// (CSLEE)
        IO_WRITE(port + AMBA_UARTCFG, (AMBA_UARTCFG_ENB | AMBA_UARTCFG_FEN | AMBA_UARTCFG_TENB | AMBA_UARTCFG_RENB));

        /* finally, enable the uart */
        // IO_WRITE(port + AMBA_UARTCR, AMBA_UARTCR_UARTEN);	(CSLEE)
        // UART is enabled by above instruction
    }
}
