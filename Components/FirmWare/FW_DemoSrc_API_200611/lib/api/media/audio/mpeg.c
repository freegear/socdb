 ...
 *                     \/            \/     \/    \/            \/
 * $Id: mpeg.c,v 1.344 2005/09/15 05:29:26 amiconn Exp $
 *
...
static void drain_dma_buffer(void)
{
    while (PBDRH & 0x40)
    {
        //xor_b(0x08, &PADRH); // To avoid compile error by DJKIM 2006/11/30
...
void rec_tick(void)
{
    ...
    if(is_recording && (PBDRH & 0x40))
    {
        ...
        {
            //xor_b(0x08, &PADRH);  /* Set PR active, independent of polarity */ // To avoid compile error by DJKIM 2006/11/30
            ...
            while (PBDRH & 0x80)  /* Wait until /RTW becomes active */
            {
                ...
                {                 /* i.e. the MAS doesn't want to talk to us */
                    //xor_b(0x08, &PADRH);         /* Set PR inactive */ // To avoid compile error by DJKIM 2006/11/30
...
#if CONFIG_CODEC == MAS3587F
    #if 0 // To avoid compile error by DJKIM 2006/11/30
