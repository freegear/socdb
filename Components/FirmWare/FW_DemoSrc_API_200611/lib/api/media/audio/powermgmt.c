 ...
 *                     \/            \/     \/    \/            \/
 * $Id: powermgmt.c,v 1.90 2005/09/14 09:08:26 linus Exp $
 *
    ...
    while (ticks > 0) {
        ...
        if (!ata_disk_is_active() || usb_inserted()) {
            //avgbat = avgbat - (avgbat / BATT_AVE_SAMPLES) +
                     //adc_read(ADC_UNREG_POWER); // To avoid compile error by DJKIM 2006/12/01
            /*
...
static void power_thread(void)
{
    ...
    //avgbat = adc_read(ADC_UNREG_POWER) * BATT_AVE_SAMPLES; // To avoid compile error by DJKIM 2006/12/01
    ...
    while (1)
    {
        ...
        if(charger_inserted()) {
		// To avoid compile error by DJKIM 2006/12/01
		/*
...
{
    //logf("sys_poweroff()"); // by DJKIM 2006/12/04
