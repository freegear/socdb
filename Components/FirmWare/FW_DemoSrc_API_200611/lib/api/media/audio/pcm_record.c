 ...
 *                     \/            \/     \/    \/            \/
 * $Id: pcm_record.c,v 1.4 2005/08/13 17:28:59 linus Exp $
 *
...
#include "config.h"
#include "debug.h"
//#include "panic.h" // by DJKIM 2006/12/04
#include "thread.h"
...
#include "cpu.h"
//#include "mcf5249.h" // To avoid compile error by DJKIM 2006/12/01
#include "i2c.h"
#include "system.h"
#include "usb.h"
...
#include "button.h"
#include "file.h"
//#include "sprintf.h"// by DJKIM 2006/12/04
//#include "logf.h"// by DJKIM 2006/12/04
//#include "button.h"// by DJKIM 2006/12/04
#include "lcd.h"
//#include "lcd-remote.h"// by DJKIM 2006/12/04
...
void pcm_open_recording(void)
{
    ...
    //logf("pcm_open_rec"); // by DJKIM 2006/12/04
    ...
    //logf("pcm_open_rec done"); // by DJKIM 2006/12/04
}
...
void pcm_stop_recording(void)
{
    ...
    //logf("pcm_stop_recording"); // by DJKIM 2006/12/04
    ...
    //logf("pcm_stop_recording done"); // by DJKIM 2006/12/04
}
...
void pcmrec_callback(bool flush)
{
    ...
    if (wav_file != -1)
    {
        if (num_ready >= WRITE_THRESHOLD || flush)
        {
            ...
            {
                //*ptr = SWAB32(*ptr); // To avoid compile error by DJKIM 2006/12/01
                ptr++;
            }
...
void pcmrec_dma_start(void)
{
    ...
    //SAR1 = (unsigned long)&PDIR2;                      /* Source address */ // To avoid compile error by DJKIM 2006/12/01
    //BCR1 = EACH_BUFFER_SIZE;                           /* Bytes to transfer */ // To avoid compile error by DJKIM 2006/12/01

    /* Start the DMA transfer.. */
    //DCR1 = DMA_INT | DMA_EEXT | DMA_CS | DMA_DINC | DMA_START; // To avoid compile error by DJKIM 2006/12/01

    //logf("dma1 started"); // by DJKIM 2006/12/04
}
...
{
    //int res = DSR1;
    int res = 0;// To avoid compile error by DJKIM 2006/12/01

    //DSR1 = 1;    /* Clear interrupt */ // To avoid compile error by DJKIM 2006/12/01

    int_count++;

    if (res & 0x70)
    {
        //DCR1 = 0;   /* Stop DMA transfer */ // To avoid compile error by DJKIM 2006/12/01
        error_count++;
        is_recording = 0;

        //logf("dma1 err 0x%x", res); // by DJKIM 2006/12/04
    ...
    } else 
    {
        ...
        if (is_stopping || !is_recording)
        {
            ...
            //logf("dma1 stopping"); // by DJKIM 2006/12/04
        ...
        } else if (write_index == read_index)
        {
            ...
            //logf("dma1 overrun"); // by DJKIM 2006/12/04

        } else
        {
            //DAR1 = (unsigned long)rec_buffers[write_index];  /* Destination address */ // To avoid compile error by DJKIM 2006/12/01
            //BCR1 = EACH_BUFFER_SIZE; // To avoid compile error by DJKIM 2006/12/01
    ...
    //IPR |= (1<<15); /* Clear pending interrupt request */ // To avoid compile error by DJKIM 2006/12/01
}
...
static int start_wave(void)
{
    ...
    {
        wav_file = -1;
        //logf("create failed: %d", wav_file); // by DJKIM 2006/12/04
        return -1;
    }
    ...
    if (sizeof(header) != write(wav_file, header, sizeof(header)))
    {
        close(wav_file);
        wav_file = -1;
        //logf("write failed"); // by DJKIM 2006/12/04
        return -2;
    }
...
static void close_wave(void)
{
    ...
    //l = SWAB32(num_rec_bytes + 36); // To avoid compile error by DJKIM 2006/12/01
    lseek(wav_file, 4, SEEK_SET);
    write(wav_file, &l, 4);

    //l = SWAB32(num_rec_bytes); // To avoid compile error by DJKIM 2006/12/01
...
{
    //logf("pcmrec_start"); // by DJKIM 2006/12/04
    ...
    //logf("rec: %s", recording_filename); // by DJKIM 2006/12/04
...
static void pcmrec_stop(void)
{
    ...
    //logf("pcmrec_stop"); // by DJKIM 2006/12/04
    ...
    //logf("pcmrec_stop done"); // by DJKIM 2006/12/04
...
static void pcmrec_open(void)
{
    ...
    //IIS1CONFIG = 0x800;             /* Stop any playback                              */ // To avoid compile error by DJKIM 2006/12/01
    //AUDIOGLOB |= 0x180;             /* IIS1 fifo auto sync = on, PDIR2 auto sync = on */ // To avoid compile error by DJKIM 2006/12/01
    //DATAINCONTROL = 0xc000;         /* Generate Interrupt when 6 samples in fifo      */ // To avoid compile error by DJKIM 2006/12/01
    //DATAINCONTROL |= 0x20;          /* PDIR2 source = IIS1recv                        */ // To avoid compile error by DJKIM 2006/12/01

    //DIVR1 = 55;                     /* DMA1 is mapped into vector 55 in system.c      */ // To avoid compile error by DJKIM 2006/12/01
    //DMACONFIG = 1;                  /* DMA0Req = PDOR3, DMA1Req = PDIR2               */ // To avoid compile error by DJKIM 2006/12/01
    //DMAROUTE = (DMAROUTE & 0xffff00ff) | DMA1_REQ_AUDIO_2; // To avoid compile error by DJKIM 2006/12/01
    //ICR4 = (ICR4 & 0xffffff00) | 0x0000001c;  /* Enable interrupt at level 7, priority 0 */ // To avoid compile error by DJKIM 2006/12/01
    //IMR &= ~(1<<15);                /* bit 15 is DMA1 */ // To avoid compile error by DJKIM 2006/12/01
...
static void pcmrec_close(void)
{
    ...
    //DMAROUTE = (DMAROUTE & 0xffff00ff); // To avoid compile error by DJKIM 2006/12/01
    //ICR4 = (ICR4 & 0xffffff00);     /* Disable interrupt */ // To avoid compile error by DJKIM 2006/12/01
    //IMR |= (1<<15);                 /* bit 15 is DMA1 */ // To avoid compile error by DJKIM 2006/12/01
...
static void pcmrec_thread(void)
{
    ...
    //logf("thread pcmrec start"); // by DJKIM 2006/12/04
    ...
    //logf("thread pcmrec done"); // by DJKIM 2006/12/04
}
...
void pcmrec_set_mux(int source)
{
    ...
    //or_l(0x00800000, &GPIO_ENABLE); // To avoid compile error by DJKIM 2006/12/01
    //or_l(0x00800000, &GPIO_FUNCTION); // To avoid compile error by DJKIM 2006/12/01
}
