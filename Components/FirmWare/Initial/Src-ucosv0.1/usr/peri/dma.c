...
volatile smtUint32 gDMAStatus = 0, gDMAInt =0;
...
static smtBoolean CheckPattern(smtUint32 seed, smtUint32 buffer, smtUint32 size)
{
	...
	for(i = 0; i < size/4; i++)
	{
		...
		{
			EDMADPRINTF(11, "error 0x%08x[0x%08x]:0x%08x\n", 
...
{
	gDMAStatus = smt2EDMAGetStatus(EDMA_TEST_CH);
	gDMAInt = 1;
	if(gDMAStatus & DMA_STOPINT_MASK)//DMA stop interrupt
	{
		smt2DMACDisable(EDMA_TEST_CH);
		return;
	}
	smt2EDMASetStatus(EDMA_TEST_CH, gDMAStatus|0xFUL);
}

/*----------------------------------------------------------
	Function name	: EDMADefaultConf()
	Prototype		: void EDMADefaultConf(EDMA_STRUCT *edma)
	Return			: 
	Argument		:
	Comments		: 
-----------------------------------------------------------*/
void EDMADefaultConf(EDMA_STRUCT *edma)
{
...
/*----------------------------------------------------------
	...
	Prototype		: smtUint32 EDMAStartNoDescPolling(smtUint8 edmaCh, EDMA_STRUCT edma)
	Return			: 
	Argument		:
	Comments		: 
-----------------------------------------------------------*/
smtUint32 EDMAStartNoDescPolling(smtUint8 edmaCh, EDMA_STRUCT edma)
{
	smtUint32 errCode = EDMAT_EC_NONE;
	smtUint32 timeOut, edmaSts;
	extern void DCacheFlushing(void);
	
	errCode = smt2DMACDisable(EDMA_TEST_CH);
	...
	smt2DMACNoDescrp(EDMA_TEST_CH, edma);
	
	// polling EDMA stop status
	timeOut = 0;
	while(1)
	{
		edmaSts = smt2EDMAGetStatus(EDMA_TEST_CH);
		if(edmaSts & DMA_STOPINT_MASK)
		{
			...
			EDMADPRINTF(11,"[EDMA TEST] polling catch EDMA stop status\n");
			break;
		}
...
/*----------------------------------------------------------
	...
	Prototype		: smtUint32 EDMAStartNoDescInterrupt(smtUint8 edmaCh, EDMA_STRUCT edma)
	Return			: 
	Argument		:
	Comments		: 
-----------------------------------------------------------*/
smtUint32 EDMAStartNoDescInterrupt(smtUint8 edmaCh, EDMA_STRUCT edma)
{
	smtUint32 errCode;
	smtUint32 timeOut;\
	extern void DCacheFlushing(void);
	...
	while(1)
	{
		if(gDMAInt)
		{
			if(gDMAStatus & DMA_ENDINT_MASK)	//EDMA end
			{
				EDMADPRINTF(11, "[EDMA TEST] Interrupt asserted	END\n");
			}
			if(gDMAStatus & DMA_STOPINT_MASK)	//EDMA stop
			{
				EDMADPRINTF(11, "[EDMA TEST] Interrupt asserted	STOP\n");
				break;
			}
			if(gDMAStatus & DMA_ERRORINT_MASK)	//EDMA error
			{
				EDMADPRINTF(11, "[EDMA TEST] Interrupt asserted	ERROR\n");
				break;
			}
			if(gDMAStatus & DMA_STARTINT_MASK)	//EDMA  start
			{
				EDMADPRINTF(11, "[EDMA TEST] Interrupt asserted	START\n");
			}
			gDMAInt =0;
			timeOut = 0;
		}
		else if(timeOut++ == EDMAT_TO_WAITINT)
		{
...
/*----------------------------------------------------------
	...
	Prototype		: smtUint32 EDMAStartUseDescPolling(smtUint8 edmaCh, EDMA_STRUCT edma0)
	Return			: 
	Argument		:
	Comments		: 
-----------------------------------------------------------*/
smtUint32 EDMAStartUseDescPolling(smtUint8 edmaCh, EDMA_STRUCT *edma)
{
	smtUint32 i;
	...
	DCacheFlushing();
	smt2DMACUseDescrp(edmaCh, DMA_DAT_LOOP_SIZE, edma);
...
/*----------------------------------------------------------
	...
	Prototype		: smtUint32 EDMAStartUseDescInterrupt(smtUint8 edmaCh, EDMA_STRUCT edma0)
	Return			: 
	Argument		:
	Comments		: 
-----------------------------------------------------------*/
smtUint32 EDMAStartUseDescInterrupt(smtUint8 edmaCh, EDMA_STRUCT *edma)
{
...
smtUint32 EDMAM2MNoDescTest(void)
{
	...
	//--------------------------------------------------------------------------
	EDMADPRINTF(11,"==============================================================\n");
	EDMADPRINTF(11,"[EDMA TEST] no descriptor polling test\n");
	EDMADPRINTF(11,"==============================================================\n");
	...
	EDMADPRINTF(11,"[EDMA TEST] SRC  ADDR[0x%08x] BUSWIDTH[%d] ADDRINC[0x%01x]\n",
					edma.src,edma.srcWidth,edma.srcInc);
	EDMADPRINTF(11,"[EDMA TEST] DEST ADDR[0x%08x] BUSWIDTH[%d] ADDRINC[0x%01x]\n",
					edma.dst,edma.dstWidth,edma.dstInc);
	EDMADPRINTF(11,"[EDMA TEST] BURST SIZE[0x%01x] TOTOAL SIZE[0x%05x]\n",
					edma.transSize,edma.totSize);
		
	ret = EDMAStartNoDescPolling(EDMA_TEST_CH, edma);
	if(ret)
	{
		EDMADPRINTF(11,"[EDMA TEST] EDMAStartNoDescPolling ERRORCODE=[0x%08x] \n",ret);
		return SMT_ERROR;
	}
	...
	EDMADPRINTF(11,"[EDMA TEST] pattern check ok\n");	
	EDMADPRINTF(11,"[EDMA TEST] no descriptor polling test--------------[PASS]\n");
	...
	EDMADPRINTF(11,"==============================================================\n");
	EDMADPRINTF(11,"[EDMA TEST] no descriptor interrupt test\n");
	EDMADPRINTF(11,"==============================================================\n");
	...
	if(ret)
	{
		EDMADPRINTF(11,"[EDMA TEST] EDMAStartNoDescInterrupt ERRORCODE=[0x%08x] \n",ret);
		return SMT_ERROR;
	}
	...
	EDMADPRINTF(11,"[EDMA TEST] pattern check ok\n");	
	EDMADPRINTF(11,"[EDMA TEST] no descriptor interrupt test------------[PASS]\n");
...
smtUint32 EDMAM2MUseDescTest(void)
{
	smtUint32 ret,i;
	smtUint32 seed = 0xAAAA0000;
	EDMA_STRUCT edma[DMA_DAT_LOOP_SIZE];
	memset((void*)EDMA_M2M_SRC_ADDR,0x0 ,4*MB);
	memset((void*)EDMA_M2M_DES_ADDR,0x0 ,4*MB);
	...
	EDMADPRINTF(11,"==============================================================\n");
	EDMADPRINTF(11,"[EDMA TEST] using descriptor polling test\n");
	EDMADPRINTF(11,"==============================================================\n");
	...
	for(i = 0; i < DMA_DAT_LOOP_SIZE; i++)	
	{
		...
		EDMADPRINTF(11,"[EDMA TEST] --------------DESCRIPTER #%d------------------\n",i);
		EDMADPRINTF(11,"[EDMA TEST] SRC  ADDR[0x%08x] BUSWIDTH[%d] ADDRINC[0x%01x]\n",
	...
	EDMADPRINTF(11,"[EDMA TEST] pattern check ok\n");
	EDMADPRINTF(11,"[EDMA TEST] using descriptor polling test-----------[PASS]\n");
	...
	EDMADPRINTF(11,"==============================================================\n");
	EDMADPRINTF(11,"[EDMA TEST] using descriptor interrupt test\n");
	EDMADPRINTF(11,"==============================================================\n");
	...
	if(!CheckPattern(seed,EDMA_M2M_DES_ADDR, 512*1024*(DMA_DAT_LOOP_SIZE)))
		return SMT_ERROR;
	EDMADPRINTF(11,"[EDMA TEST] pattern check ok\n");
	EDMADPRINTF(11,"[EDMA TEST] using descriptor interrupt test---------[PASS]\n");
	return SMT_SUCCESS;
}
...
smtUint32 GDMATest(void)
{
	...
	EDMADPRINTF(11,"==============================================================\n");
	EDMADPRINTF(11,"[GDMA TEST] GDMA MEMCPY Polling test\n");
	EDMADPRINTF(11,"==============================================================\n");
	...
	EDMADPRINTF(11,"[GDMA TEST] SRC_ADDR[0x%08x] SRC_SIZE[%d*%d] CLIP[%d,%d] \n"
				,gdma.src,gdma.sWidth,gdma.sHeight, gdma.sClipX,gdma.sClipY);
	EDMADPRINTF(11,"[GDMA TEST] DES_ADDR[0x%08x] DES_SIZE[%d*%d] CLIP[%d,%d] \n"
				,gdma.dst,gdma.dWidth,gdma.dHeight, gdma.dClipX,gdma.dClipY);
	EDMADPRINTF(11,"[GDMA TEST] CLIP SIZE[%d*%d] \n"
	...
	EDMADPRINTF(11,"[GDMA TEST] pattern check ok\n");
	EDMADPRINTF(11,"[GDMA TEST] GDMA MEMCPY Polling test----------------[PASS]\n");

	EDMADPRINTF(11,"==============================================================\n");
	EDMADPRINTF(11,"[GDMA TEST] GDMA MEMCPY Interrupt test\n");
	EDMADPRINTF(11,"==============================================================\n");
	...
	EDMADPRINTF(11,"[GDMA TEST] pattern check ok\n");
	EDMADPRINTF(11,"[GDMA TEST] GDMA MEMCPY Interrupt test--------------[PASS]\n");
...
smtUint32 DMATest(void)
{
	...
	if(ret != SMT_SUCCESS)
	{
		EDMADPRINTF(11,"EDMAM2M No Descriptor test failed!!\n");
		return ret;
	}
	...
	if(ret != SMT_SUCCESS)
	{
		EDMADPRINTF(11,"EDMAM2M Using Descriptor test failed!!\n");
		return ret;
	}
	...
	if(ret != SMT_SUCCESS)
	{
		EDMADPRINTF(11,"EDMAM2M GDMA test failed!!\n");
		return ret;
	}
	return SMT_SUCCESS;
}
