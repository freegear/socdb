...
void CheckPCMRXStatus(void)
{
	...
	if( readData != 0 )
	{
		;//smt2UartPrint(11,"PCM Rx FIFO DATA : %d\n", readData);
	} else
	{
		;//smt2UartPrint(11,"PCM Rx FIFO has no Data\n");
	}
}
...
void CheckPCMTXStatus(void)
{
	...
	if( readData != 0 )
	{
		;//smt2UartPrint(11,"PCM Tx FIFO DATA : %d\n", readData);
	} else
	{
		;//smt2UartPrint(11,"PCM Tx FIFO has no Data\n");
	}	
}
void SEIP_DMA_RX_Enable(void)
{
	...
	smt2UartPrint(11,"SEIP DMA RX Init.....\n");
...
void SEIP_DMA_TX_Enable(void)
{
	...
	smt2UartPrint(11,"SEIP DMA TX Init.....\n");
...
void New_WriteRec(smtBoolean start, smtBoolean end, smtBoolean stop)
{
	...
	smt2UartPrint(11,"RX channel = %d\n",rec_rx_dmach);

	//SMT_WRITE(DMACSta(rec_rx_dmach),0x00000009);

	smt2DMACUseDescrp(rec_rx_dmach, DMA_DAT_LOOP_SIZE, edma);	
}
...
void New_WriteRecTest(smtBoolean interruptFlag)
{
	...
	if(interrupt==SMT_TRUE)
	{
		...
		while(1)
		{
			if(REC_InterruptFlag==SMT_TRUE)
			{
				...
				{
					smt2UartPrint(11,"[INT] Error\n");
				}

				if(REC_InterruptValue & 0x2)
				{
					smt2UartPrint(11,"[INT] End\n");
				}

				if(REC_InterruptValue & 0x4)
				{
					smt2UartPrint(11,"[INT] Start\n");
				}
				
				if(REC_InterruptValue & 0x08)
				{
					smt2UartPrint(11,"[INT] Stop\n");
		...
		recStatus.recSize = smt2DMAGetTransLength(rec_rx_dmach, AUDIO_TESTADDR);

		smt2EDMAChAsign(DMA_CHDALLOC, DMA_SEIPTX, &rec_rx_dmach);

		smt2DMACDisable(rec_rx_dmach);  
	...
	while(1)
	{
		...
		if((i%100) == 0)
		{
			//smt2UartPrint(11,"Write Size : %08x\n", 0x80000 -(DMACCon(rec_rx_dmach)&0xfffff));
			smt2UartPrint(11,"Des Addr : %08x\n", (DMACDAdr(rec_rx_dmach)));		
			CheckPCMRXStatus();
			CheckPCMTXStatus();
		}
		...
		if(smtUartDataAvailable())
		{
			...
			if(uart_input == '0')
			{
				// calculate the transfered data size
				{
					...
					recStatus.recSize = smt2DMAGetTransLength(rec_rx_dmach, AUDIO_TESTADDR);  
					smt2UartPrint(11,"record Size = %08x\n",recStatus.recSize);
					SMT_WRITE(DMACDAdr(rec_rx_dmach),loopback_start_addr); 						
				}
...
void New_ReadRecTest(void)
{
	...
	while(1)
	{
		...
		*/
		...
		if(reg&0x02)
		{
			smt2UartPrint(11, "EndInterrupt\n");
		}
		if(reg&0x04)
		{
			smt2UartPrint(11, "StartInterrupt\n");
		}
		if(reg&0x08)
		{
			smt2UartPrint(11, "StopInterrupt\n");
			smt2DMACDisable(rec_tx_dmach);
			break;			
		}
		...
		if((i%100) == 0)
		{	
			smt2UartPrint(11,"Src Addr: %08x\n", DMACSAdr(rec_tx_dmach));
			//smt2UartPrint(11,"Read Size : %08x\n", 0x80000 - (DMACCon(rec_tx_dmach)&0xfffff));			
			CheckPCMRXStatus();
			CheckPCMTXStatus();
		}
		i++;
...
void New_ReadRec(void)
{
	...
	for(i = 0; i < recStatus.des_cnt; i++)	
	{
		...
		edma[i].descListBase = (smtUint32)descBuffer;
		smt2UartPrint(11,"Desc Addr : %08x\n",edma[i].src);				
	}
	...
	ret = smt2DMACDisable(rec_tx_dmach);
	
	if(ret)
	{
		smt2UartPrint(11,"disable EDMA error\n");
	}
	
	smt2DMACUseDescrp(rec_tx_dmach, recStatus.des_cnt + 1, edma);	
}
...
void Old_WriteRecTest(smtBoolean interruptFlag)
{
	...
	while(1)
	{
		...
		if(reg & 0x00000009)
			smt2UartPrint(11, "DMACSta(rec_tx_dmach) = 0x%08X\n, reg");
