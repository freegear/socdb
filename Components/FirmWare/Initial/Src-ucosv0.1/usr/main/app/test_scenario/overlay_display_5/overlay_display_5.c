...
static smtUint32 CGVOverlay(void)
{
	...
	smt2UartPrint(11,"press 'c' or 'g' or 'v' to on/off each plane\n");
	smt2UartPrint(11,"press 'y' or 'h' to change sturation Y level\n");
	smt2UartPrint(11,"press 'u' or 'j' to change sturation C level\n");
	smt2UartPrint(11,"press 'f' or 'd' to change luminance(f) chrominance(d) filter\n");
	#endif
		...
		if(UARTDataAvailable())
		{
			...
			switch(uart_input)
			{
				...
				case 'y':
					...
					smt2UartPrint(11, "saturationYLevel = [0x%08x]\n",sturY);
					break;
				case 'h':
					smtGetVideoImageCtrl(&imageCtrl);
					imageCtrl.saturationYLev = --sturY;
				...
				case 'u':
					...
					smtSetVideoImageCtrl(&imageCtrl);
					smt2UartPrint(11, "saturationCLevel = [0x%08x]\n",sturC);
					break;
				case 'j':
					smtGetVideoImageCtrl(&imageCtrl);
					imageCtrl.saturationCLev = --sturC;
					...
					break;
				case 'f':
					smtGetVideoEncode(0x0, &vInternal);
					vInternal.lFilter = lFilterIdx%4;
					smtSetVideoEncode(0x0, &vInternal);
					smt2UartPrint(11,"[%10s] luminance filter seleted\n ",lFilterString[lFilterIdx%4]);
					lFilterIdx++;
					break;
				case 'd':
					smtGetVideoEncode(0x0,&vInternal);
					...
					smtSetVideoEncode(0x0,&vInternal);
					smt2UartPrint(11,"[%10s] chrominance filter selected\n",cFilterString[cFilterIdx%2]);
					cFilterIdx++;
					break;
