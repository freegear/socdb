...
void smtNANDSetOPT0(NandOpt0 *popt0)
{
	...
	/*
	NANDDPRINTF(CFG_UART_CH, "smtNANDSetOPT0: 0x%08x\n",
...
void smtNANDSetOPT1(NandOpt1 *popt1)
{
	...
	/*
	NANDDPRINTF(CFG_UART_CH, "smtNANDSetOPT1: 0x%08x\n",
		 ((popt1->CMDADDR[3] & 0xFF) << 24)
		|((popt1->CMDADDR[2] & 0xFF) << 16)
		|((popt1->CMDADDR[1] & 0xFF) << 8)
		|((popt1->CMDADDR[0] & 0xFF) << 0)
	);
	*/
...
{
	/*
	NANDDPRINTF(CFG_UART_CH, "smtNANDSetOPT2: 0x%08x\n",
		 ((popt2->CMDADDR[3] & 0xFF) << 24)
		|((popt2->CMDADDR[2] & 0xFF) << 16)
		|((popt2->CMDADDR[1] & 0xFF) << 8)
		|((popt2->CMDADDR[0] & 0xFF) << 0)
	);
	*/	
...
{
	*pdata = (smtUint32)SMT_READ(NANDDATA);
}
...
void smtNANDSetStatus(NandStat *pstatus)
{
	...
	//NANDDPRINTF(CFG_UART_CH, "smtNANDSetStatus: 0x%08x\n", reg);
		
	SMT_WRITE(NANDSTAT,reg);
...
void smtNANDGetECC(smtUint8 eccIdx, smtUint32 *pecc, smtUint32 *psecc)
{
	...
	switch(eccIdx)
	{
		case 0:
			...
			if(psecc) 	*psecc	= SMT_READ(SECCSECTOR0);
			break;
			
		case 1:
			if(pecc) 	*pecc 	= SMT_READ(ECCSECTOR1);
			if(psecc)	*psecc	= SMT_READ(SECCSECTOR1);
			break;
			
		case 2:
			if(pecc) 	*pecc 	= SMT_READ(ECCSECTOR2);
			if(psecc) 	*psecc	= SMT_READ(SECCSECTOR2);
			break;
			
		case 3:
			if(pecc) 	*pecc 	= SMT_READ(ECCSECTOR3);
			if(psecc) 	*psecc	= SMT_READ(SECCSECTOR3);
			break;
			
		case 4:
			if(pecc) 	*pecc 	= SMT_READ(ECCSECTOR4);
			if(psecc) 	*psecc	= SMT_READ(SECCSECTOR4);
			break;
			
		case 5:
			if(pecc) 	*pecc 	= SMT_READ(ECCSECTOR5);
			if(psecc)	*psecc	= SMT_READ(SECCSECTOR5);
			break;
			
		case 6:
			if(pecc) 	*pecc 	= SMT_READ(ECCSECTOR6);
			if(psecc) 	*psecc	= SMT_READ(SECCSECTOR6);
			break;
			
		case 7:
			if(pecc) 	*pecc 	= SMT_READ(ECCSECTOR7);
			if(psecc) 	*psecc	= SMT_READ(SECCSECTOR7);
			break;
...
void smtNANDGetFIFOStatus(NandFifoStat *pfifoStatus)
{
	...
	pfifoStatus->DFIFOLevel1	= ((reg >> 8) & 0xF);
	pfifoStatus->DFIFOLevel0	= ((reg >> 4) & 0xF);
	pfifoStatus->QLevel			= ((reg >> 0) & 0xF);
}
