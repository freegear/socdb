...
smtBoolean HostSetInt(smtBoolean enable);
...
static volatile smtBoolean	gNoBusy			= 0;
static volatile smtBoolean	gRspCRC			= 0;
static volatile smtBoolean	gCmdSent		= 0;
static volatile smtBoolean	gCmdTout		= 0;
static volatile smtBoolean	gRspEnd			= 0;
static volatile smtBoolean	gFIFOFail		= 0;
static volatile smtBoolean	gCrcSta			= 0;
static volatile smtBoolean	gDatCrc			= 0;
static volatile smtBoolean	gDatTout		= 0;
static volatile smtBoolean	gDatFin			= 0;
static volatile	smtBoolean	gBusyFin		= 0;

static volatile smtBoolean	gSDMMCIntEnable	= 0;
	...
	for(bi = 0; bi < blkNum; bi++)
	{
		...
		{	
			...
			while(1)
			{
				...
				*/
				SMT_WRITE(SDIntMsk, SMT_READ(SDIntMsk) | (1<<2));
				if(gTXFIFOEmpty)
				{
					gTXFIFOEmpty = 0;
					break;
	...
	for(bi = 0; bi < blkNum; bi++)
	{
		...
		{	
			...
			*/
			...
			SMT_WRITE(SDIntMsk, SMT_READ(SDIntMsk) | (1<<1));
...
static smtUint32 SDMMCDPRINTF(smtInt8 *format, ...)
{
	...
	smt2UARTPutStr(CFG_UART_CH, str);
...
SD_CMD9:	
	...
	smt2UARTPrint(CFG_UART_CH, "0x%08X, 0x%08X, 0x%08X, 0x%08X\n", 
...
{
	SdmmcIntEnable		sdmmcIntEnb;
	SdmmcIntSta			sdmmcStatus;
	SdmmcCmdCtrl			sdmmcCtrl;
	SdmmcCmdRsp			sdmmcRsp;
