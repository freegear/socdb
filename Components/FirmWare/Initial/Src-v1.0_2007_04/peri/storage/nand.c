...
//#define NANDDPRINTF(CFG_UART_CH, f, args...)		 smt2UARTPrint(CFG_UART_CH, f, ##args)
...
smtBoolean CheckPattern(smtUint32 seed, smtUint32 buffer, smtUint32 size)
{
	...
	for(i = 0; i < size/4; i++)
	{
		...
		{
			NANDDPRINTF(CFG_UART_CH, "error 0x%08x:0x%08x\n", 
				pbuffer[i], pattern);
			return false;
		}
	}	
...
void HexDump(smtUint32 buffer, smtUint32 size)
{
	...
	for(i = 0; i < size/16; i++)
	{
		NANDDPRINTF(CFG_UART_CH, "0x%08x : ", &pbuffer[i*16+j]);
		for(j = 0; j < 16; j++) 
		 	NANDDPRINTF(CFG_UART_CH, "%02x ", pbuffer[i*16+j]);
		 
		NANDDPRINTF(CFG_UART_CH, " | ");
		...
		for(j = 0; j < 16; j++) 
		{
			if( (pbuffer[i*16+j] >= 0x00)
			 && (pbuffer[i*16+j] <= 0x1F))
				...
				NANDDPRINTF(CFG_UART_CH, "%c", pbuffer[i*16+j]);
		}
...
smtUint32 ReadID(smtUint32 id[5])
{
    ...
    NANDDPRINTF(CFG_UART_CH, "ReadID:: get NAND ID code data\n");
    ...
    NANDDPRINTF(CFG_UART_CH, "ID code: 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
    	id[0], id[1], id[2], id[3], id[4]);
 
    
  	NANDDPRINTF(CFG_UART_CH, "-ReadID\n");
		...
		smtUint32 ppagebuf, smtUint8 transSize, smtUint32 sizeInByte)
{
	...
	while(true)
	{
		if(gNANDRdRnB) 
		{
			//NANDDPRINTF(CFG_UART_CH, "check RnB detected!!!\n");
			gNANDRdRnB = 0;
			break;
		}
    ...
    while(true)
	{
		if(gNANDRdEnd) 
		{
			//NANDDPRINTF(CFG_UART_CH, "Read end interrupt!!!\n");
			gNANDRdEnd = 0;
			break;
		}
		...
		smtUint32 ppagebuf, smtUint8 transSize, smtUint32 sizeInByte)
{
	...
	regValue	&= ~(0xF			 <<(NANDT_DMA_CH*4));
	regValue	|= (DMA_NAND 		 <<(NANDT_DMA_CH*4));	
		...
		smtUint32 ppagebuf, smtUint32 transSize, smtUint32 sizeInByte)
{
    ...
    while(true)
    {
    	...
    	{
			//NANDDPRINTF(CFG_UART_CH, "status: 0x%02x\n", nandStatus.NFStatus);
			memset(&nandStatus, 0, sizeof(nandStatus));
...
smtBoolean	NANDRWTest(void)
{
	...
	//--------------------------------------------------------------------------	
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] CPU read/write test start!!!\n");
	...
	{
		NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
		NANDDPRINTF(CFG_UART_CH, "[NAND TEST] CPU --%s!!!\n", accessTypeStr[atIdx]);
		NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
		...
		{
			// erase
			NANDDPRINTF(CFG_UART_CH, "[NAND TEST] block(b:%05d) erase!!!\n", blkAddr);
			BlockErase(blkAddr);
			...
			for(pageAddr = 0; pageAddr < 4; pageAddr++) // is large block
			{
				...
				{
					NANDDPRINTF(CFG_UART_CH, "[NAND TEST] test nand(b:%05d, p:%05d, c:%05d)...\n"
					...
					if(ret)
					{
						NANDDPRINTF(CFG_UART_CH, "NANDRWTest::PageProgram(e)0x%08x\n", 
							ret);
						return false;
					}							
					...
					if(ret)
					{
						NANDDPRINTF(CFG_UART_CH, "NANDRWTest::Read(e)0x%08x\n", 
							ret);
						return false;
					}									
					...
					{
						NANDDPRINTF(CFG_UART_CH, "[NAND TEST] (%d) test fail!!!\n", atIdx);
						return false;
					}
	...
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] CPU read/write test done!!!\n");

	
	//--------------------------------------------------------------------------
	// 2. DMA read/write test for 1 Block
	//--------------------------------------------------------------------------
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] DMA read/write test start!!!\n");
	// set default setting
	NandDefaultSetting();
	...
	{
		NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
		NANDDPRINTF(CFG_UART_CH, "[NAND TEST] DMA --%s!!!\n", accessTypeStr[atIdx]);
		NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");		
			...
			blkAddr++) 	// for one block
		{
			...
			for(pageAddr = 0; pageAddr < 4; pageAddr++) // is large block
			{
				for(colAddr = 0; colAddr < (512*1); colAddr += (512+16)) // all page
				{
					...
					if(ret)
					{
						NANDDPRINTF(CFG_UART_CH, "NANDRWTest::PageProgramDMA(e)0x%08x\n", 
							ret);
						return false;
					}	
					...
					if(ret)
					{
						NANDDPRINTF(CFG_UART_CH, "NANDRWTest::ReadDMA(e)0x%08x\n", 
							ret);
						return false;
					}	
	...
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] DMA read/write test done!!!\n");
	//--------------------------------------------------------------------------
	// 3. CPU read/write test for 1 Block by interrupt
	//--------------------------------------------------------------------------	
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] interrupt test start!!!\n");
	...
	{
		NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
		NANDDPRINTF(CFG_UART_CH, "[NAND TEST] interrupt --%s!!!\n", accessTypeStr[atIdx]);
		NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");		
			...
			blkAddr++) 	// for one block
		{
			...
			for(pageAddr = 0; pageAddr < 4; pageAddr++) // is large block
			{
				for(colAddr = 0; colAddr < (512+16); colAddr += (512+16)) // all page
				{
					...
					if(ret)
					{
						NANDDPRINTF(CFG_UART_CH, "NANDRWTest::PageProgramInt(e)0x%08x\n", 
							ret);
						return false;
					}	
					...
					if(ret)
					{
						NANDDPRINTF(CFG_UART_CH, "NANDRWTest::ReadInt(e)0x%08x\n", 
							ret);
						return false;
					}	
	...
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] interrupt read/write test!!!\n");		
...
smtBoolean NANDECCTest(void)
{
	...
	//--------------------------------------------------------------------------	
	NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] main ECC 256 bytes test start!!!\n");
	NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
		...
		blkAddr++) 	// for one block
	{
		...
		for(pageAddr = 0; pageAddr < 4; pageAddr++) // is large block
		{
			...
			NANDDPRINTF(CFG_UART_CH, "[NAND TEST] write ecc test(b:%05d, p:%05d, c:%05d)...\n"
								,blkAddr, pageAddr, 0);
			// program
			...
			if(ret)
			{
				NANDDPRINTF(CFG_UART_CH, "NANDECCTest::PageProgram(e)0x%08x\n", 
					ret);
				return false;
			}
			...
			for(mainIdx = 0; mainIdx < 8; mainIdx++) 
			{
				...
				{
					NANDDPRINTF(CFG_UART_CH, "error p[%d] sw:hw-0x%08x:0x%08x\n", 
			...
			for(spareIdx = 0; spareIdx < 4; spareIdx++)
			{
				...
				{
					NANDDPRINTF(CFG_UART_CH, "error sp[%d] hw:sw-0x%08x:0x%08x\n", 
			...
			NANDDPRINTF(CFG_UART_CH, "[NAND TEST]  read ecc test(b:%05d, p:%05d, c:%05d)...\n"
								,blkAddr, pageAddr, 0);
			...
			if(ret)
			{
				NANDDPRINTF(CFG_UART_CH, "NANDECCTest::Read(e)0x%08x\n", 
					ret);
				return false;
			}		
			...
			for(spareIdx = 0; spareIdx < 4; spareIdx++)
			{
				...
				if((HWSpareECC&0xFFFF) != (SWSpareECC&0xFFFF)) 
				{
					NANDDPRINTF(CFG_UART_CH, "error sp[%d] sw:hw-0x%08x:0x%08x\n", 
						spareIdx, HWSpareECC, SWSpareECC);
					return false;
				}
			}			
	...
	NANDDPRINTF(CFG_UART_CH, "NANDECCTest::main 256byte ECC success!!!\n");	
	//--------------------------------------------------------------------------
	// main/spare 512byte ECC test
	//--------------------------------------------------------------------------

	NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] main ECC 512 bytes test start!!!\n");
	NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");	
	...
	NANDDPRINTF(CFG_UART_CH, "NANDECCTest::main 512byte ECC success!!!\n");	
	...
	//--------------------------------------------------------------------------	
	NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
	NANDDPRINTF(CFG_UART_CH, "[NAND TEST] main/spare ECC auto write test start!!!\n");
	NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
		...
		blkAddr++) 	// for one block
	{
		...
		for(pageAddr = 0; pageAddr < 4; pageAddr++) // is large block
		{
			...
			NANDDPRINTF(CFG_UART_CH, "[NAND TEST] auto write ecc test(b:%05d, p:%05d, c:%05d)...\n"
								,blkAddr, pageAddr, 0);
			// program
			ret = PageProgram(0, pageAddr, blkAddr, 
			...
			for(mainIdx = 0; mainIdx < 8; mainIdx++) 
			{
				...
				{
					NANDDPRINTF(CFG_UART_CH, "error p[%d] sw:hw:au-0x%08x:0x%08x:0x%08x\n", 
			...
			for(spareIdx = 0; spareIdx < 4; spareIdx++)
			{
				...
				if((HWSpareECC&0xFFFF) != (SWSpareECC&0xFFFF)) 
				{
					NANDDPRINTF(CFG_UART_CH, "error p[%d] sw:hw:au-0x%08x:0x%08x\n", 
						mainIdx, HWSpareECC, SWSpareECC);
					return false;
				}
			}
	...
	NANDDPRINTF(CFG_UART_CH, "NANDECCTest::main auto write ECC success!!!\n");	
...
smtUint32 NANDTest(void)
{
	...
	NANDDPRINTF(CFG_UART_CH, "+NANDTest\n"); 
	...
	if(result)
	{
		NANDDPRINTF(CFG_UART_CH, "NANDTest::NAND READ ID fail!!!\n");
		return FMC_ERROR;
	}
	NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
	NANDDPRINTF(CFG_UART_CH, " Nand ID Read \n");
	NANDDPRINTF(CFG_UART_CH, "-------------------------------------------------------------\n");
	NANDDPRINTF(CFG_UART_CH, "NANDTest::Nand 1st ID : 0x%02X\n",idCode[0]);
	NANDDPRINTF(CFG_UART_CH, "NANDTest::Nand 2nd ID : 0x%02X\n",idCode[1]);
	NANDDPRINTF(CFG_UART_CH, "NANDTest::Nand 3rd ID : 0x%02X\n",idCode[2]);
	NANDDPRINTF(CFG_UART_CH, "NANDTest::Nand 4th ID : 0x%02X\n",idCode[3]);
    NANDDPRINTF(CFG_UART_CH, "NANDTest::Nand 5th ID : 0x%02X\n",idCode[4]);    
	...
	if(result == false)
	{
		NANDDPRINTF(CFG_UART_CH, "NANDTest::NAND read/write test fail!!!\n");
		return FMC_ERROR;
	}
	NANDDPRINTF(CFG_UART_CH, "NANDTest::NAND read/write test success!!!\n");
	...
	if(result == false)
	{
		NANDDPRINTF(CFG_UART_CH, "NANDTest::NAND ECC test fail!!!\n");
		return FMC_ERROR;
	}
	NANDDPRINTF(CFG_UART_CH, "NANDTest::NAND ECC test success!!!\n");
...
smtBoolean	NANDU_128kWriter(smtUint32 addr128k, smtUint32 buffer)
{
	...
	smtUint32	ret;
	static NandStat	nandStatus;
	static NandCtrl	nandCtrl;	
	

	// get index
	blockIdx = addr128k;
	
	//NANDDPRINTF(CFG_UART_CH, "NANDU_128kWriter::Write block: %04d\n", blockIdx);
		
	// erase block 
	ret = BlockErase(blockIdx);
	if(ret)
	{
		NANDDPRINTF(CFG_UART_CH, "NANDU_128kWriter::BlockErase(e))x%08x\n", ret);
		return false;
	}
	...
	for(pageIdx = 0; pageIdx < 64; pageIdx++)
	{
		...
		if(ret)
		{
			NANDDPRINTF(CFG_UART_CH, "NANDU_128kWriter::PageProgramDMA(e))x%08x\n", ret);
			return false;
		}		
...
smtBoolean	NANDU_128kReader(smtUint32 addr128k, smtUint32 buffer)
{
	...
	//NANDDPRINTF(CFG_UART_CH, "NANDU_128kWriter::Read block: %04d\n", blockIdx);	
	...
	for(pageIdx = 0; pageIdx < 64; pageIdx++)
	{
		...
		if(ret)
		{
			NANDDPRINTF(CFG_UART_CH, "NANDU_128kReader::ReadDMA(e))x%08x\n", ret);
			return false;
		}	
	}
