...
smtUint32 DDRSDRAMTest(void)
{
	...
	DDRTDPRINTF(CFG_UART_CH, "[DDR SDRAM TEST] START!!!\n");
	...
	for(loopCnt = 0; loopCnt < 4; loopCnt++) {
		...
		while(true) 
		{
			...
			if(testRegion[regionIdx].startAddress == testRegion[regionIdx].endAddress)
			{
				...
				{
					DDRTDPRINTF(CFG_UART_CH, "[DDR SDRAM TEST] %d loop test END!!!\n", loopCnt);
					break;
				}
				else
				{
					DDRTDPRINTF(CFG_UART_CH, 
			...
			//--------------------------------------------------------
			DDRTDPRINTF(CFG_UART_CH, "------------------------------------------------------\n");
			DDRTDPRINTF(CFG_UART_CH, "[DDR SDRAM TEST] region: 0x%08x-0x%08x\n",
