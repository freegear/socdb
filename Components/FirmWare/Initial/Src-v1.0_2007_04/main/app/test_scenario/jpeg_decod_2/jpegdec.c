...
BYTE Hmax; 			// Maximum Horizontal Sampling Factor for all component
BYTE Vmax; 			// Maximum Vertical Sampling Factor for all component
...
// jpeg -> RGB24 bitmap
void JpegDecode0(BYTE * jpeg, BYTE * bmp, int * width, int * height)
{
...
/////////////////////////////////////////////////////////////////////
    ...
    BYTE jpeg_HuffDecode( int Th , BIT_STREAM * pBS)
{
	int i = 1, j, limit = 0;
	WORD CODE;
	BYTE Value;
...
// change inline funct
//inline short jpeg_Extend(WORD V, BYTE T)
__inline short jpeg_Extend(WORD V, BYTE T)
{
	WORD Vt = 1 << (T-1) ;
			  ...
			  for( n = 0 ; n < Rv ; n++ )    
			    {
			      switch(Rh)
				{
				case 4 : MCU[k][ ( idx1 + n ) * mWidth + ( idx2 + 3 ) ] = ( BYTE )ZZ_OUT[ idx3 ] ;
				case 3 : MCU[k][ ( idx1 + n ) * mWidth + ( idx2 + 2 ) ] = ( BYTE )ZZ_OUT[ idx3 ] ;
				case 2 : MCU[k][ ( idx1 + n ) * mWidth + ( idx2 + 1 ) ] = ( BYTE )ZZ_OUT[ idx3 ] ;
				case 1 : MCU[k][ ( idx1 + n ) * mWidth + ( idx2 + 0 ) ] = ( BYTE )ZZ_OUT[ idx3 ] ;
				}
			    }
#else
			  for( n = 0 ; n < Rv ; n++ )    
			    {
			      for( o = 0 ; o < Rh ; o++ )    
				{
				  MCU[k][ ( idx1 + n ) * mWidth + ( idx2 + o ) ] = ( BYTE )ZZ_OUT[ idx3 ] ;
				}
			    }
#endif
   ...
   void jpeg_DecodeMCU(int mx, int my, BIT_STREAM * pBS)
{
	...
	{
		#include <stdarg.h>
		extern unsigned int smt2UARTPrint(char uartCh, char *format, ...);
		
		//smt2UARTPrint(CFG_UART_CH, "w,h mw, mh %d %d %d %d \n", mWidth, mHeight, mx, my);
