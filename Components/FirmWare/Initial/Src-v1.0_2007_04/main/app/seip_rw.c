...
void SEIPReadWriteTest(void)
{
	...
	smt2UARTPrint(CFG_UART_CH, "SEIP Read/Write Test...");
	...
	SEIPNORWrite(0x00, 0x90);
	SEIPNORWrite(0x00, 0x00);
	for(i = 0; i < 4; i++)
		smt2UARTPrint(CFG_UART_CH,"Addr(0x%02x) : 0x%02x\n", i, SEIPNORRead(i));	
	...
	return;
error:
	smt2UARTPrint(CFG_UART_CH,"Error(Read %08x when %08x expected)\n", data, expected_data);
	return;
}
