...
smtUint32 ADCTest(void)
{
	...
	// Need to modified by DJKIM 2006/12/20	
	//setting ACLKDIV
	sysConReg = SMT_READ(PM_CLKDIV);
	sysConReg &= ~PM_ACLK_DIV;
	SMT_WRITE(PM_CLKDIV, sysConReg &
		(0x00<<SHIFT_DN_FROM_MASK(PM_ACLK_DIV)) ); 
	
	//READ_START mode test
	ReadStartSet();
