...
{
	SMT_WRITE(SDDatCon, 
		 ( (pdatCtrl->DMASize		& 0x7F ) << 25 )
		|( (pdatCtrl->MMCPlus		& 0x1  ) << 21 )
		|( (pdatCtrl->TARSP			& 0x1  ) << 20 )
		|( (pdatCtrl->RACMD			& 0x1  ) << 19 )
		|( (pdatCtrl->ByteOrder		& 0x1  ) << 18 )
		|( (pdatCtrl->BlkMode		& 0x1  ) << 17 )
		|( (pdatCtrl->WideBusEnable & 0x1  ) << 16 )
		|( (pdatCtrl->DMAEnable		& 0x1  ) << 15 )
		|( (pdatCtrl->TransStart	& 0x1  ) << 14 )
...
{
	SMT_WRITE(SDFSTA,
		 ( (pFIFOStatus->Reset 		& 0X1 ) << 16 )
		|( (pFIFOStatus->FIFOError 	& 0X1 ) << 14 )
		|( (pFIFOStatus->TXAvail 	& 0X1 ) << 13 )
		|( (pFIFOStatus->RXAvail 	& 0X1 ) << 12 )
		|( (pFIFOStatus->TXHalfFull & 0X1 ) << 11 )
		|( (pFIFOStatus->TXEmpty	& 0X1 ) << 10 )
		|( (pFIFOStatus->RXFull		& 0X1 ) << 8  )
		|( (pFIFOStatus->RXHalfFull & 0X1 ) << 7  )
...
{
	SMT_WRITE(SDIntMsk,
		 ( (pIntMask->AutoReadComplete		& 0x1) << 18 )
		|( (pIntMask->AutoReadCMD12Error 	& 0x1) << 17 )
		|( (pIntMask->AutoReadCMD18Error 	& 0x1) << 16 )
		|( (pIntMask->NoBusy 				& 0x1) << 15 )
		|( (pIntMask->RspCRC 				& 0x1) << 14 )
		|( (pIntMask->CmdSent 				& 0x1) << 13 )
		|( (pIntMask->CmdTout 				& 0x1) << 12 )
		|( (pIntMask->RspEnd 				& 0x1) << 11 )
		|( (pIntMask->FIFOFail 				& 0x1) << 10 )
		|( (pIntMask->CRCSta 				& 0x1) << 9  )
		|( (pIntMask->DatCRC 				& 0x1) << 8  )
		|( (pIntMask->DatTOut 				& 0x1) << 7  )
		|( (pIntMask->DatFin 				& 0x1) << 6  )
		|( (pIntMask->BusyFin 				& 0x1) << 5  )
		|( (pIntMask->TFHalf 				& 0x1) << 3  )
		|( (pIntMask->TFEmpty	 			& 0x1) << 2  )
		|( (pIntMask->RFFull 				& 0x1) << 1  )
		|( (pIntMask->RFHalf 				& 0x1) << 0  )
	);
}
