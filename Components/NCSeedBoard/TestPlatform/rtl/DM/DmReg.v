...
output [ 0:0] 	CPlanePixFormat;
output [ 1:0] 	CPlaneAlphaMode;
output [ 7:0] 	CPlaneAlphaValue;
output [XW:0] 	CPlaneXStart;
output [YW:0] 	CPlaneYStart;
output [ 9:0]	CPlaneXRef;
output [XW:0] 	CPlaneXSize;
output [YW:0] 	CPlaneYSize;
output [AW:0] 	CPlaneStartAddr;
...
output [XW:0] 	GPlaneXStart;
output [YW:0] 	GPlaneYStart;
output [ 9:0]	GPlaneXRef;
output [XW:0] 	GPlaneXSize;
output [YW:0] 	GPlaneYSize;
output [AW:0] 	GPlaneStartAddr;
output			GPlaneGammaEn;

// Palette Memory
output		  	PalMemWrite;
output [ 7:0] 	PalMemWrAddr;
output [23:0] 	PalMemWrData;
output			PalMemRead;
input  [23:0] 	PalMemRdData;
...
output [XW:0] 	VPlaneXStart;
output [YW:0] 	VPlaneYStart;
output [ 9:0]	VPlaneXRef;
output [XW:0] 	VPlaneXSize;
output [YW:0] 	VPlaneYSize;
...
output 			DnScaleEn;
output 			UpScaleEn;
output [XW:0]	ScaleInXSize;
output [YW:0]	ScaleInYSize;
...
reg  [ 0:0]	CPlanePixFormat;	// 0: 8bit; 1: 16bit
reg  [ 1:0]	CPlaneAlphaMode;	// 0: No Alpah; 2: Global Alpha; 3: Per-Pixel Alpha
reg  [ 7:0] CPlaneAlphaValue;
reg  [XW:0] CPlaneXStart;	// Cursor Plane
reg  [YW:0] CPlaneYStart;
reg  [ 9:0] CPlaneXRef;
reg  [XW:0] CPlaneXSize;	// max 64x64
reg  [YW:0] CPlaneYSize;
reg  [AW:0] CPlaneStartAddr;
reg			CPlaneAddrSwEn;
...
reg  [XW:0] GPlaneXStart;	// Cursor Plane
reg  [YW:0] GPlaneYStart;
reg  [ 9:0] GPlaneXRef;
reg  [XW:0] GPlaneXSize;	// max 64x64
reg  [YW:0] GPlaneYSize;
...
reg  	 	PalMemWrite;
reg 		PalMemRead;
reg  [ 7:0] PalMemWrAddr;
reg  [23:0] PalMemWrData;
`endif
...
reg  [XW:0] VPlaneXStart;	// Cursor Plane
reg  [YW:0] VPlaneYStart;
reg  [ 9:0] VPlaneXRef;
reg  [XW:0] VPlaneXSize;
reg  [YW:0] VPlaneYSize;
...
reg 		DnScaleEn;
reg 		UpScaleEn;
reg [XW:0]	ScaleInXSize;
reg [YW:0]	ScaleInYSize;
...
wire rCPOSSel   	= rSelect & (PADDR == CPOS[9:2]);  
wire rGPOSSel   	= rSelect & (PADDR == GPOS[9:2]);  
wire rVPOSSel   	= rSelect & (PADDR == VPOS[9:2]);  
...
wire rGPALDSel   	= rSelect & (PADDR == GPALD[9:2]);  
...
wire rLECONSel		= rSelect & (PADDR == LECON[9:2]);
...
wire rCPOSWr    	= rCPOSSel     	& rWrite;
wire rGPOSWr    	= rGPOSSel     	& rWrite;
wire rVPOSWr    	= rVPOSSel     	& rWrite;
...
wire rGPALDWr   	= rGPALDSel    	& rWrite;
...
wire rLECONWr		= rLECONSel    	& rWrite;
...
wire rCPOSRd    	= rCPOSSel     	& rRead;
wire rGPOSRd    	= rGPOSSel     	& rRead;
wire rVPOSRd    	= rVPOSSel     	& rRead;
...
always @(negedge PRESETB or posedge PCLK)
	...
	else if (rDMCONWr) 	SWReset <= PWDATA[0];
...
always @(negedge PRESETB or posedge PCLK)
	...
	else if (rCPOSWr) begin
	    CPlaneXPos 		<= PWDATA[21:11];
	    CPlaneYPos  	<= PWDATA[10:0];
	end
...
always @(negedge PRESETB or posedge PCLK)
	...
	else if (rGPOSWr) begin
	    GPlaneXPos 		<= PWDATA[21:11];
	    GPlaneYPos  	<= PWDATA[10:0];
	end
...
always @(negedge PRESETB or posedge PCLK)
	...
	else if (rVPOSWr) begin
	    VPlaneXPos 		<= PWDATA[21:11];
	    VPlaneYPos  	<= PWDATA[10:0];
	end
//------------------------------------------------------------------------------
// Cursor Plane
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
	    ...
	    CPlaneEn			<= PWDATA[31];
	    CPlanePixFormat		<= PWDATA[28];
	    CPlaneXSize 		<= PWDATA[21:11];
	    CPlaneYSize 		<= PWDATA[10:0];
	end
...
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
		...
		CPlaneXRef			<= PWDATA[31:22];
	    CPlaneXStart 		<= PWDATA[21:11];
	    CPlaneYStart  		<= PWDATA[10:0];
	end
...
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
	    ...
	    GPlaneXSize 		<= PWDATA[21:11];
	    GPlaneYSize 		<= PWDATA[10:0];
	end
...
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
		...
		GPlaneXRef			<= PWDATA[31:22];
	    GPlaneXStart 		<= PWDATA[21:11];
	    GPlaneYStart  		<= PWDATA[10:0];
	end
//------------------------------------------------------------------------------
// Palette Memory
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) GPalMemWrite	<= 0;
	else		  GPalMemWrite  <= rGPALDWr;

always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) GPalMemRead	<= 0;
	else		  GPalMemRead   <= rGPALDRd;
...
always @(negedge PRESETB or posedge PCLK)
	...
	else if (rGPALDWr) begin
	    GPalMemWrData  		<= PWDATA[23:0];
	end

`ifdef PCLKisACLK
assign PalMemWrAddr = GPalMemWrAddr;
assign PalMemWrData = GPalMemWrData;
assign PalMemRead   = GPalMemRead;
assign PalMemWrite  = GPalMemWrite;
`else
always @(negedge ARESETn or posedge ACLK)
	if (!ARESETn) begin
		...
		PalMemRead  <= GPalMemRead;
		PalMemWrite <= GPalMemWrite;
	end
...
always @(negedge ARESETn or posedge ACLK)
	if (!ARESETn) begin
		...
		PalMemWrAddr <= GPalMemWrAddr;
		PalMemWrData <= GPalMemWrData;
	end
`endif
//------------------------------------------------------------------------------
// Video Plane
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
	    ...
	    VPlaneXSize 		<= PWDATA[21:11];
	    VPlaneYSize 		<= PWDATA[10:0];
	end
...
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
		...
		VPlaneXRef			<= PWDATA[31:22];
	    VPlaneXStart 		<= PWDATA[21:11];
	    VPlaneYStart  		<= PWDATA[10:0];
	end
...
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
	    ...
	    ScaleInXSize 		<= PWDATA[21:11];
	    ScaleInYSize 		<= PWDATA[10:0];
	end
...
`endif
//------------------------------------------------------------------------------
reg  [31:0]  LCDTiming0PCLK;
reg  [31:0]  LCDTiming1PCLK;
reg  [31:0]  LCDTiming2PCLK;
reg  [31:0]  LCDTiming3PCLK;
reg  [31:0]  LCDControlPCLK;
reg  [31:0]  LcdLineEndPCLK;

always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
	    LCDTiming0PCLK <= {32{1'b0}};
	    LCDTiming1PCLK <= {21'b0, 11'd2047};
	    LCDTiming2PCLK <= {32{1'b0}};
	    LCDTiming3PCLK <= {21'b0, 11'd2047};
	    LcdLineEndPCLK <= {32{1'b0}};
	    LCDControlPCLK <= {32{1'b0}};
	end
	else if (rHSYNC0Wr) begin
	    LCDTiming0PCLK <= PWDATA;
	end
	else if (rHSYNC1Wr) begin
	    LCDTiming1PCLK <= PWDATA;
	end
	else if (rVSYNC0Wr) begin
	    LCDTiming2PCLK <= PWDATA;
	end
	else if (rVSYNC1Wr) begin
	    LCDTiming3PCLK <= PWDATA;
	end
	else if (rLCDCONWr) begin
	    LCDControlPCLK <= PWDATA;
	end
	else if (rLECONWr) begin
	    LcdLineEndPCLK <= PWDATA;
	end
//------------------------------------------------------------------------------
// LCD TG
// MetaStability 2 FF
LcdReg LcdSyncLcdClk(
 				...
 				.LCDControlPCLK (LCDControlPCLK),
 				.LcdLineEndPCLK	(LcdLineEndPCLK),
...
assign		DmErrInt = DmIntEn & PlaneFIFOError;
...
always @(negedge PRESETB or posedge PCLK)
	if 	(!PRESETB)		  PRDATA <= 32'b0;
		...
		case(1'b1) // synopsys parallel_case
    	  	rDMCONRd	: PRDATA <= {PrioritySel, DmIntEn, 1'b0};
    	  	rDMSTSRd	: PRDATA <= {CDMAOverRunSts, GDMAOverRunSts, VDMAOverRunSts,
    	  							 CurFIFOUnderRunSts, GraFIFOUnderRunSts, VidFIFOUnderRunSts, MixFIFOUnderRunSts};

			rCPOSRd		: PRDATA <= {10'b0, CPlaneXPos, CPlaneYPos};
			rGPOSRd		: PRDATA <= {10'b0, GPlaneXPos, GPlaneYPos};
			rVPOSRd		: PRDATA <= {10'b0, VPlaneXPos, VPlaneYPos};

			rCCONRd		: PRDATA <=  {CPlaneEn, 2'b0, CPlanePixFormat, 10'b0, CPlaneXSize, 4'b0, CPlaneYSize};
			rCBLNDRd	: PRDATA <= {CPlaneAlphaValue, CPlaneChromaKey};
			rCBMODRd	: PRDATA <= {1'b0, CPlaneChromaKeyEn, 1'b0, CPlaneAlphaMode, 3'b0, 24'b0};
			rCBASERd	: PRDATA <= {CPlaneXRef, CPlaneXStart, CPlaneYStart};
			rCADDRRd	: PRDATA <= {CPlaneStartAddr};
			rCBLINKRd	: PRDATA <= {CPlaneAddrSwEn, 25'b0, CPlaneAddrSwVal};
			
			rGCONRd		: PRDATA <= {GPlaneEn, 1'b0, GPlanePixFormat, GPlaneGammaEn, 4'b0, GPlaneXSize, GPlaneYSize};
			rGBLNDRd	: PRDATA <= {GPlaneAlphaValue, GPlaneChromaKey};
			rGBMODRd	: PRDATA <= {1'b0, GPlaneChromaKeyEn, 1'b0, GPlaneAlphaMode, 3'b0, 24'b0};
			rGBASERd	: PRDATA <= {GPlaneXRef, GPlaneXStart, GPlaneYStart};
			rGADDRRd	: PRDATA <= {GPlaneStartAddr};
			rGPALARd	: PRDATA <= {GPalMemWrAddr};
			rGPALDRd	: PRDATA <= {PalMemRdData};
			
			rVCONRd		: PRDATA <= {VPlaneEn, VPlaneInterlaceEn, 1'b0, VPlanePixFormat, VPlaneGammaEn, 4'b0, VPlaneXSize, VPlaneYSize};
			rVBLNDRd	: PRDATA <= {VPlaneAlphaValue, VPlaneChromaKey};
			rVBMODRd	: PRDATA <= {1'b0, VPlaneChromaKeyEn, 1'b0, VPlaneAlphaMode, 3'b0, 24'b0};
			rVBASERd	: PRDATA <= {VPlaneXRef, VPlaneXStart, VPlaneYStart};
			rVADDRRd	: PRDATA <= {VPlaneStartAddr};
			rVADDR2Rd	: PRDATA <=  VPlaneStartAddr2;
...
//			rLECONRd	: PRDATA <= LcdLineEndPCLK;
