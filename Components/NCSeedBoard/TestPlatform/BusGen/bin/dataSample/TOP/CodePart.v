...
//STATE03_START
    ...
    //For Slave ??
    //Write address channel
    .AWIDmi??2s??    (AWID_2_?NAME?    ),
    .AWADDRmi??2s??  (AWADDR_2_?NAME?  ),
    .AWLENmi??2s??   (AWLEN_2_?NAME?   ),
    .AWSIZEmi??2s??  (AWSIZE_2_?NAME?  ),
    .AWBURSTmi??2s?? (AWBURST_2_?NAME? ),
//ENABLE_LOCK
    .AWLOCKmi??2s??  (AWLOCK_2_?NAME?  ),
//ENABLE_CACHE
    .AWCACHEmi??2s?? (AWCACHE_2_?NAME? ),
//ENABLE_PROT
    .AWPROTmi??2s??  (AWPROT_2_?NAME?  ),

    .AWVALIDmi??2s?? (AWVALID_2_?NAME? ),
    .AWREADYs??2mi?? (AWREADY_?NAME? ),

    //Write data channel
    .WIDmi??2s??     (WID_2_?NAME?     ),
    .WDATAmi??2s??   (WDATA_2_?NAME?   ),
//ENABLE_WSTRB
    .WSTRBmi??2s??   (WSTRB_2_?NAME?   ),
    .WLASTmi??2s??   (WLAST_2_?NAME?   ),
    .WVALIDmi??2s??  (WVALID_2_?NAME?  ),
    .WREADYs??2mi??  (WREADY_?NAME?  ),

    //Write response channel
    .BIDs??2mi??     (BID_?NAME?     ),
    .BRESPs??2mi??   (BRESP_?NAME?   ),
    .BVALIDs??2mi??  (BVALID_?NAME?  ),
    .BREADYmi??2s??  (BREADY_2_?NAME?  ),
...
//STATE07_START
    ...
    //For Slave ??
    //Read address channel
    .ARIDmi??2s??    (ARID_2_?NAME?    ),
    .ARADDRmi??2s??  (ARADDR_2_?NAME?  ),
    .ARLENmi??2s??   (ARLEN_2_?NAME?   ),
    .ARSIZEmi??2s??  (ARSIZE_2_?NAME?  ),
    .ARBURSTmi??2s?? (ARBURST_2_?NAME? ),
//ENABLE_LOCK
    .ARLOCKmi??2s??  (ARLOCK_2_?NAME?  ),
//ENABLE_CACHE
    .ARCACHEmi??2s?? (ARCACHE_2_?NAME? ),
//ENABLE_PROT
    .ARPROTmi??2s??  (ARPROT_2_?NAME?  ),

    .ARVALIDmi??2s?? (ARVALID_2_?NAME? ),
    .ARREADYs??2mi?? (ARREADY_?NAME? ),

    //Read data channel
    .RIDs??2mi??     (RID_?NAME?     ),
    .RRESPs??2mi??   (RRESP_?NAME?   ),
    .RDATAs??2mi??   (RDATA_?NAME?   ),
    .RLASTs??2mi??   (RLAST_?NAME?   ),
    .RVALIDs??2mi??  (RVALID_?NAME?  ),
    .RREADYmi??2s??  (RREADY_2_?NAME?  ),
//STATE_END
