...
processNode(xmlTextReaderPtr reader) {
    ...
    printf("depth :: %d nodeType :: %d Name :: %s  EmptyElement::%d HasVal::%d[%d]", 
    ...
    //if(name == (const xmlChar *)"name")//xmlTextReaderHasAttributes(reader)==1 && name=="name" )
    if(xmlTextReaderHasAttributes(reader)==1)// && name=="name" )
    {
        printf("--> [%s]", xmlTextReaderGetAttribute(reader, "slavenum"));
    }
    if (value == NULL)
	printf("\n");
    else {
