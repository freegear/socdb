...
;
	MACRO
	CHECK_UNIFIED_740T	$tmp
	MOVS	$tmp, #1		; Set tmp & clear Z (TRUE)
	MEND

;Compare (uHAL supplied) processor ID with ARM 740
;
	MACRO
	CHECK_CPUID_740T	$id, $tmp
	RDCPU_ID_740T	$id, $tmp
	CMP	$tmp, #0x740		; set Z if it is an ARM 740
	MEND

;Compare (uHAL supplied) processor Vendor ID with 'A'RM.
;
	MACRO
	CHECK_VENDOR_740T	$id, $tmp
	RDCPU_VENDOR_740T	$id, $tmp
...
;
	...
	MACRO
	TEST_MMU_740T	$tmp
	RDMMU_STATE_740T	$tmp
	AND	$tmp, $tmp, #EnableMMU
	TEQ	$tmp, #EnableMMU
	MEND

	MACRO
	TEST_BIGEND_740T	$tmp
	RDMMU_STATE_740T	$tmp
...
;
	MACRO
	WRMMU_FlushTB_740T $reg_number
	MEND

;Flush Instruction TLB 
;
	MACRO
	WRMMU_FlushITB_740T $reg_number
	WRMMU_FlushTB_740T $reg_number
	MEND
...
;
	MACRO
	WRMMU_FlushDTB_740T $reg_number
	WRMMU_FlushTB_740T $reg_number
	MEND
...
1      
        WRCACHE_CleanDCentry_740T $reg1
        ADD     $reg1, $reg1, #32
        CMP     $reg1, $reg2
        BLT     %B1
	MEND
...
;
	MACRO
	WRCLK_EnableClockSW_740T	$reg
	RDMMU_STATE_740T	$reg
	ORR	$reg, $reg, #0xC0000000
	WRMMU_STATE_740T	$reg
	MEND
