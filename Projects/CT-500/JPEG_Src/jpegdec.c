...
WORD RInterval; 		// Restart Interval
SET MCU[(4*4*64)]; 		// MCU 단위의 블럭
...
BYTE * Jptr, * Bptr; // JPEG, BMP pointer
int RstCount; // 디코딩 중일때 Restart Interval 카운터
ULONG XMBlockNum, YMBlockNum; // X, Y 블럭 갯수
ULONG CurXMBlock, CurYMBlock; // 디코딩 중인 X, Y 블럭
...
/////////////////////////////////////////////////////////////////////
#define GET_MARKER()		((ULONG)((*Jptr << 8) | *(Jptr+1)))
#define GET_JLENGTH(ptr)	((ULONG)((*(ptr) << 8) | *((ptr)+1)))
...
#define RESET_BS() { BS.buf=0; BS.cnt=0; }
#define CHECK_BS(n) { \
	while((n) > BS.cnt) { \
	BS.buf = (BS.buf << 8) | *Jptr++; \
	BS.cnt += 8; \
	if((BS.buf & 0xff) == 0xff) Jptr++; } }
#define GET_BIT(n) (((int) (BS.buf >> (BS.cnt -= (n)))) & ((1<<(n))-1))
...
#else
	if(jpeg == (BYTE *)NULL || bmp == (BYTE *)NULL) return;
#endif
...
void JpegDecodeRun()
{
	...
	jpeg_DecodeMCU(CurXMBlock, CurYMBlock);
...
void jpeg_LoadDQT(BYTE * Qptr)
{
	...
	while(1)
	{
		...
		Qptr++;
		i &= 0xF;
		memcpy(TbQ[i].Q, Qptr, 64) ;	// 해당 테이블에 64개의 양자화 테이블 복사
		Qptr += 64;
	}
}
...
{ 
	...
	while(1) {
		...
		printf("%02X %02X %02X %02X %02X %02X %02X %02X\n", 
		       BITS[0], BITS[1], BITS[2], BITS[3], BITS[4], BITS[5], BITS[15], BITS[16]);
		printf("%02X %02X %02X %02X %02X %02X %02X %02X\n", 
		       Hptr[0], Hptr[1], Hptr[2], Hptr[3], Hptr[4], Hptr[5], Hptr[15], Hptr[16]);
#endif
		...
		memcpy( TbH[ Th ].HUFFVAL, Hptr, Num ) ;     //Huffman Value 값을 Num크기만큼 Hptr에서 읽는다.
		Hptr += Num ;                                // Hptp가 증가
...
void jpeg_LoadSOF(BYTE * Fptr)
{
	...
	for(i=0 ; i<FrameHeader.Nf ; i++)
	{
		p = Fptr + (3*i);
		FrameHeader.C[i]  = *(p) ;
		FrameHeader.H[i]  = *(p + 1) >> 4 ;
		FrameHeader.V[i]  = *(p + 1) & 0x0F;
		FrameHeader.Tq[i] = *(p + 2);
	}
}
...
{ // N = Component ID 0/1/2
	...
	jpeg_DecodeDC(ScanHeader.Td[N]);   
	jpeg_DecodeAC(ScanHeader.Ta[N] + 16);
...
void jpeg_Zigzag()
{
	...
	memcpy(ZZ_SCAN, ZZ, 64 * sizeof(short));
	for(i=0 ; i<64 ; i++)
		ZZ[i] = ZZ_SCAN[ZIGZAG_TABLE[i]];
}
...
void jpeg_InitCLUT()
{
	...
	{
		v = i - 128;
		ColorTable.rV[i] = (v * 1436) >> 10;
		ColorTable.gU[i] = (v * 352) >> 10;
		ColorTable.gV[i] = (v * 731) >> 10;
		ColorTable.bU[i] = (v * 1814) >> 10;
	}
}
...
void jpeg_DecodeMCU(int mx, int my)
{
	...
	int R, G, B;	
	int pos, stride = STRIDE(bWidth, DISPLAY_DIB_BPP);
	...
	for( k=0 ; k < Ns ; k++ ) 
	{
		...
		Rv = Vmax / V ;
#ifdef __TEST__
		printf("FrameHeader.H[ %d ] = %d\n", k, FrameHeader.H[ k ]);
		printf("FrameHeader.V[ %d ] = %d\n", k, FrameHeader.V[ k ]);
				...
				for( i=0 ; i<8 ; i++ ) 
				{
					for( j=0 ; j<8 ; j++ ) 
					{
						...
						for( n = 0 ; n < Rv ; n++ )    
						{
							for( o = 0 ; o < Rh ; o++ )    
							{
								MCU[ ( idx1 + n ) * mWidth + ( idx2 + o ) ].C[ k ] = ( BYTE )ZZ[ idx3 ] ;
							}
						}
					}
				}
	...
	for( i = 0 ; i < mHeight ; i++ )
	{
		...
		for( j = 0 ; j < mWidth ; j++ )    
		{
			...
			if (Ns > 1)
			{
				iY  = (BYTE)MCU[idx3].C[0];
				iCb = (BYTE)MCU[idx3].C[1];
				iCr = (BYTE)MCU[idx3].C[2];

				iR = iY + ColorTable.rV[iCr];
				iG = iY - (ColorTable.gU[iCb] + ColorTable.gV[iCr]);
				iB = iY + ColorTable.bU[iCb];
