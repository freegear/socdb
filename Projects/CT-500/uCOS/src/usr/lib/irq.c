...
void InterruptHandler(void)
{
	...
	while(1)
	{
		...
		{
			smt2UartPrint(11, "Unknown IRQ[0x%08x]\n",irq);
			smt2UartPrint(11, "INTMASK[0x%08x]\n",SMT_READ(INTMSK));
			smt2UartPrint(11, "INTMODE[0x%08x]\n",SMT_READ(INTMOD));
...
{
	extern void smt2GDMASetIntOn(smtBoolean);
//	gDmaVariable.status[irq] = SMT_READ(DMA2D_STATUS);
	smt2GDMASetIntOn(SMT_TRUE);
