...
void smtDMSetMasterCtrl(DMCtrl maCtrl)
{
	SMT_WRITE(
			DMCON,
			  ( ( maCtrl.swReset		& 0x1 ) << 7 )	
			 |( ( maCtrl.prioritySel	& 0x1 ) << 3 )
 			 |( ( maCtrl.errIntEn		& 0x1 ) << 2 )
			 |( ( maCtrl.startIntEn		& 0x1 ) << 1 )
			 |( ( maCtrl.endIntEn		& 0x1 ) << 0 )
		);
}
/*------------------------------------------------------------------------------
...
void smtDMSetCPalette(DMPlanePalette palette)
{
	SMT_WRITE(
			CPALADDR,
			  ( (palette.palAddr & 0xFF    ) << 0  )
		);

	SMT_WRITE(
			CPALM,
			 ( (palette.palData & 0xFFFFFFFF) <<  0 )
		);
}
/*------------------------------------------------------------------------------
...
void smtDMSetGCtrl(DMPlaneCtrl ctrl)
{
	SMT_WRITE(
			GCON,
			  ( (ctrl.xEn			& 0x1  ) << 31 )
			 |( (ctrl.gColBarEn		& 0x1  ) << 30 )
			 |( (ctrl.pixFmt		& 0x7  ) << 27 )
			 |( (ctrl.gammaEn		& 0x1  ) << 26 )
			 |( (ctrl.vPlaneN2PUpEn	& 0x1  ) << 25 )
...
void smtDMSetLCDCtrl(DMLcdCtrl ctrl)
{
	SMT_WRITE(
			LCDCON,(\
			 ((ctrl.lcdEn  & 0x1)<<31)|((ctrl.lcdPwrEn&0x1)<<30)\
			|((ctrl.lcdBpp & 0x3)<< 8)|((ctrl.rbSwap & 0x1)<< 7)\
			|((ctrl.invertWrEn  & 0x1)<<6)|((ctrl.invertVSync & 0x1)<<5)\
			|((ctrl.invertHSync	& 0x1)<<4)|((ctrl.invertPixClk& 0x1)<<3)\
			|((ctrl.pDiv		& 0x7)<<0))
		);
}
/*------------------------------------------------------------------------------
