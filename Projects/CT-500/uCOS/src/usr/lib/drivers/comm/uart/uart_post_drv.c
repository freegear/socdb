...
static smtUint32 smt2UartMultiSetBaudRate(smtUint32 baudrate);
...
/*------------------------------------------------------------------------------
	Function name	: smt2UartMultiInit
	Prototype		: 
	Return			: 
	Argument		: 
	Comments		: 
...
smtUint32 smt2UartMultiInit(UartConfig uartInitData)
{
	...
	//set baud rate
	uartConfig.uartBaudGen = smt2UartMultiSetBaudRate(uartInitData.baudrate);
	smtUartSetMode(uartInitData.uartCh, &uartConfig, SET_UARTBRD);
...
/*------------------------------------------------------------------------------
	Function name	: smt2UartMultiDeInit
	Prototype		: 
	Return			: 
	Argument		: 
	Comments		: 
...
smtUint32 smt2UartMultiPutStr(smtUint8 uartCh, smtInt8 * str)
{
	...
	while( str[index] != '\000')  
	{      	
		smt2UartMultiPutCh(uartCh, (char) str[index] ) ;      
		if(str[index] == '\n')
			smt2UartMultiPutCh(uartCh, '\r');
		index++;  
	}
	return SMT_SUCCESS;
}
...
/*------------------------------------------------------------------------------
	Function name	: smt2UartMultiGetStr()
	Prototype		: smtBoolean smtUartStrRead(smtUint8 *pData, smtUint8 dataCnt)
	Return			: error code
	Argument		: Received data, receive data count
	Comments		:
...
smtUint32 smt2UartMultiGetStr(smtUint8 uartCh,smtUint8 *pData, smtUint8 dataCnt)
{
    ...
    while(len < dataCnt-1)
    {
        input = smt2UartMultiGetCh(uartCh,0);
        ...
        if(input == '\b')
        {
            if(len)
            {
                len--;
                pData[len] = '\0';
                smt2UartMultiPutStr(uartCh,"\b \b");
            }
        }
        else if(input == '\r')
        {
			smt2UartMultiPutStr(uartCh,"\r\n");
        ...
        else
        {
            pData[len] = input;
            len++;
            smt2UartMultiPutCh(uartCh,input);
        }
...
/*------------------------------------------------------------------------------
	Function name	: smt2UartMultiPrint()
	Prototype		: void smt2UartMultiPrint(smtUint8 *fmt, ...);
	Return			: void
	Argument		: fmt  -> format operator
	Comments		: print out though console
------------------------------------------------------------------------------*/
smtUint32 smt2UartMultiPrint(smtUint8 uartCh, smtInt32 dispLvl, smtInt8 *format, ...)
{
	...
	smt2UartMultiPutStr(uartCh, str);
	return SMT_SUCCESS;
}

/*------------------------------------------------------------------------------
	Function name	: smt2UartMultiPutCh()
	Prototype		: smtBoolean smt2UartMultiPutCh(smtUint8 data)
	Return			: error code
	Argument		: data
	Comments		:
		Transmit character data
...
/*------------------------------------------------------------------------------
	Function name	: smt2UartMultiGetCh()
	Prototype		: smtUint8 smt2UartMultiGetCh(smtUint32 waitTime)
	Return			: Received data
	Argument		:   waitTime
	Comments		:
		Receive character data
...
smtUint8 smt2UartMultiGetCh(smtUint8 uartCh, smtUint32 waitTime)
{
	...
	while(1)
	{
		...
		if(uartConfig.uartStatus.rxFifoCnt != 0x00)
		{
			return smtUartReadRxFifo(uartCh);
		}
	}
}
/*------------------------------------------------------------------------------
	Function name	: smt2UartMultiDataValid()
	Prototype		: smtUint8 smt2UartMultiDataValid(smtUint8 uartCh)
...
smtUint8 smt2UartMultiDataValid(smtUint8 uartCh)
{
	UartStruct uartConfig;
	smtUartGetMode(uartCh, &uartConfig, GET_UARTSTATUS);
	return uartConfig.uartStatus.rxFifoCnt;
}

/*----------------------------------------------------------
	Function name	: smt2UartMultiSetBaudRate()
	Prototype		: static smtUint32 smt2UartMultiSetBaudRate(smtUint32 baudrate)
	Return		: error code
	Argument	:
	Comments	: Return a error-code
	get uart baudrate gen register value
