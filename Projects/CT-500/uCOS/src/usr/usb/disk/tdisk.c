...
{ /* Body */
   ...
   smt2UartPrint(11,"press any key when you ready!!\n");
   smt2UartGetCh(0);
   ...
   if(CARD_DETECT==CARD_INSERTED)
   {
	if(SDMMCIdentify() == -1) 
	{
		smt2UartPrint(11,"[usb_main()]: has no card... insert card\n");
	}
   }
   ...
   while (SMT_TRUE) 
   {
			...
			else
			{
				if(CARD_DETECT  == CARD_EXTRACT)
				{
					if(SDMMCIdentify() == -1) 
					{
						smt2UartPrint(11,"[SDMMC TEST]: has no card... insert card\n");
					}
					...
					smt2UartPrint(11, "[SDMMC TEST]:  Remove the usb device before finishing the usb test\n");
					smt2UartPrint(11, "[SDMMC TEST]:  then, press '0'\n");
			  }
		  }
	  #else
			smt2UartPrint(".\n");
...
void card_detect_handler(smtUint32 IRQ)
{
	...
	else
	{
		...
		if(SDMMCIdentify() == -1) 
		{
			smt2UartPrint(11,"[card_detect_handler()]: has no card... insert card\n");
		}
		else
		{
			CARD_DETECT = CARD_INSERTED;
