...
smtBoolean smtNorWriteBlock(smtUint32 address,smtUint8 *data,smtUint32 datalen)
{
	...
	while(1)
	{
		...
		smt2UARTPrint(CFG_UART_CH, "Sector Write Complete\n");
...
smtBoolean smtNorWriteSector(smtUint32 address,smtUint8 *data,smtUint32 datalen)
{
	...
	smt2UARTPrint(CFG_UART_CH,"address = 0x%08x\n",address);
	smt2UARTPrint(CFG_UART_CH,"sector  = 0x%08x\n",sector);
	...
	while(1)
	{
		...
		{
			smt2UARTPrint(CFG_UART_CH, "Status Check failure\n");
			return SMT_FALSE;
		}
		...
		for(offset=0; offset < wlen; offset++) 
		{
			...
			if(srcdata!=dstdata)
			{
				smt2UARTPrint(CFG_UART_CH, "[%d]Write Failure\n",__LINE__);
			}
		}
...
smtBoolean smtNorEraseSector(smtUint32 sector)
{
	...
	smt2UARTPrint(CFG_UART_CH, "Nor Erase Failure\n");
