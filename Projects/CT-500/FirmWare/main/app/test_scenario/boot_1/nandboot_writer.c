...
smtUint32 NandBootWriter(void)
{
	...
	smt2UARTPrint(CFG_UART_CH, "Press any key to write nandboot!!\n");
	...
	// write test pattern
	{
		...
		// write boot up
		smt2UARTPrint(CFG_UART_CH, "Boot up code write start ");
		NANDU_128kWriter(0, (smtUint32)bootCode);	// 0 : 1st block #, bootCode : Block size
		smt2UARTPrint(CFG_UART_CH, "Boot up code write complete!!\n");
		
		
		smt2UARTPrint(CFG_UART_CH, "ct500 binary image code write start from 0x60100000");
		// write image 2MB
		{
			int i;
			
			for(i = 0; i < 0x600000/0x20000; i++)//shkim-20070404 : load 6MB image	// 0x200000/0x20000 = 2MB/128KB
			{
				NANDU_128kWriter(i+1, (smtUint32)0x60100000+i*0x20000);
				smt2UARTPrint(CFG_UART_CH, "NANDU_128kWriter: %d sector\n", i);
			}
		}
		smt2UARTPrint(CFG_UART_CH, "ct500 binary image code write complete!!!\n");
	}
	while(1);
