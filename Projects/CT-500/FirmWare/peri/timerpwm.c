...
void TimerIntervalTest(void)
{
	...
	for (timerCh = TIMER_SEL0; timerCh < TIMER_SEL3+1; timerCh++)
		for (prescale = PRESCALE_MIN; prescale < PRESCALE_MAX; prescale+=0xA0)
		{
			...
			smt2UARTPrint(CFG_UART_CH, "TIMER Interval Test : %d, 0x%x\n", timerCh, prescale);
		}
	...
	smt2UARTPrint(CFG_UART_CH, "TIMER Interval Test done\n");
}
...
void TimerOverFlowTest(void)
{
	...
	for (timerCh = TIMER_SEL0; timerCh < TIMER_SEL3+1; timerCh++)
		for (prescale = PRESCALE_MIN; prescale < PRESCALE_MAX; prescale+=0xA0)
		{
			...
			smt2UARTPrint(CFG_UART_CH, "TIMER Match & Overflow Test : %d, 0x%x\n", timerCh, prescale);
		}
	...
	smt2UARTPrint(CFG_UART_CH, "TIMER Match & Overflow Test done\n");
}
...
static void ISRTIMER(smtUint32 irq)
{
	...
	smt2UARTPrint(CFG_UART_CH, "Timer ISR\n");
	timerFlag = 1;
}
