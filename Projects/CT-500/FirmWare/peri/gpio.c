...
smtUint32 GPIOInOut(void)
{
	...
	smt2UARTPrint(CFG_UART_CH, "Press the GPIO key.. Enter '0' to escape\n");
...
gpioError:
	smt2UARTPrint(CFG_UART_CH, "GPIO error\n");
	return SMT_ERROR;
}
...
static void GPIOISR(smtUint32  GPIOPin)
{
	...
	if((gpio.intBothEdge == 1) && (gpio.intLevel == 0))
	{
		...
		smt2UARTPrint(CFG_UART_CH, "GPIO(%d) Active high edge detected\n", GPIOPin);
	else if((gpio.intLevel == 0) && (gpio.intBothEdge == 0) && (gpio.intPolarity == 0))
		smt2UARTPrint(CFG_UART_CH, "GPIO(%d) Active low edge detected\n", GPIOPin);
