...
smtUint32 DMTest(void)
{
	...
	//	Init Video Encoder
	smt2UARTPrint(CFG_UART_CH,"enter to enable video encoder\n");
	smt2UARTGetCh(CFG_UART_CH, &temp, 0);
	VideoEncoderEnable();

	//	Init Display Module - master register
	smt2UARTPrint(CFG_UART_CH,"enter to enable display module\n");
	smt2UARTGetCh(CFG_UART_CH, &temp, 0);
	errCode = DMInit();
	//	register Display module ISR
	smt2UARTPrint(CFG_UART_CH,"Request IRQ !! \n");
	smt2UARTGetCh(CFG_UART_CH, &temp, 0);
	RequestIRQ(IRQ_DM, ISRDM);	// DM irq : 18

	//cursor plane test
	smt2UARTPrint(CFG_UART_CH,"cursor plane test \n");
	bProgressive = smt2UARTGetCh(CFG_UART_CH, &temp, 0);
	...
	//graphic plane test
	smt2UARTPrint(CFG_UART_CH,"graphic plane test \n");
	bProgressive = smt2UARTGetCh(CFG_UART_CH, &temp, 0);
	...
	//video plane test
	smt2UARTPrint(CFG_UART_CH,"video plane test \n");
	bProgressive = smt2UARTGetCh(CFG_UART_CH, &temp, 0);
	...
	//Overlay test
	smt2UARTPrint(CFG_UART_CH,"Overlay test \n");
	bProgressive = smt2UARTGetCh(CFG_UART_CH, &temp, 0);
...
static smtUint32 DMCursorPlaneTest(smtBoolean bProgressive)
{
	...
	smt2UARTPrint(CFG_UART_CH,"press enter to start even/odd switching\n");
	smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);

	smt2UARTPrint(CFG_UART_CH,"press '0' to exit cursor plane test\n");
	...
	while(1)
	{
		...
		if(uart_input)
		{
			uart_input = smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);
			if(uart_input == '0')
			{
				break;
			}
...
static smtUint32 DMGraphicPlaneTest(smtBoolean bProgressive)
{
	...
	smt2UARTPrint(CFG_UART_CH,"enter to switch even/odd frame !\n");
	smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);
	smt2UARTPrint(CFG_UART_CH,"press '0' to exit graphic plane test\n");
	while(1)
	{
		if(bChgEvenFrame)
		{
...
static smtUint32 DMVideoPlaneTest(smtBoolean bProgressive)
{
	...
	smt2UARTPrint(CFG_UART_CH,"enter to switch even/odd \n");
	smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);
...
static smtUint32 DMOverlayTest(smtBoolean bProgressive)
{
	...
	smt2UARTPrint(CFG_UART_CH,"start to enable cursor plane !! \n");
	smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);
	...
	while(1)
	{
		...
		for(i = 0 ; i < 240 ; i++)
		{
			...
			cPlaneBlndMode.blendMod = 0x0; //0:no alpha , 2: global alpha, 3: pixel alpha
			smtDMSetCBlndMode(&cPlaneBlndMode);
		smt2UARTPrint(CFG_UART_CH,"enter to change x coodination to [%d]! \n", i);
		inputChar = smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);
		if (inputChar == '0')
			break;
		}
		if(inputChar == '0')
	...
	smt2UARTPrint(CFG_UART_CH,"start to enable graphic plane !! \n");
	smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);
	...
	smt2UARTPrint(CFG_UART_CH,"start to enable video plane !! \n");
	//smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);
	...
	smt2UARTPrint(CFG_UART_CH,"start to swich even/odd field !! \n");
	//smt2UARTGetCh(CFG_UART_CH, &uart_input, 0);
...
void ISRDM(smtUint32 irq)
{
	...
	//smt2UARTPrint(CFG_UART_CH,"Entered DM ISR status [0x%08x]\n", regData);
}
