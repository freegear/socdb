...
uint_32 port_change_counter=0;
uint_32 port_change_log[100];
uint_32 sof_after_suspend =0;
...
{ /* Body */
   ...
   ****************************************************************/   
   ...
   #else
   usb_dev_ptr->EP_QUEUE_HEAD_BASE = (VUSB20_EP_QUEUE_HEAD_STRUCT_PTR)\
                                     USB_DRIVER_QH_BASE;/* 0x6050.0840 */
   usb_dev_ptr->DTD_BASE_PTR = (VUSB20_EP_TR_STRUCT_PTR) \
                                USB_DRIVER_DTD_BASE; /* 0x6050.1140 */
   usb_dev_ptr->SCRATCH_STRUCT_BASE = (SCRATCH_STRUCT_PTR)\
                                USB_DRIVER_SCRATCH_STRUCT_BASE; /* 0x6050.1560 */
...
{ /* Body */
   ...
   dev_ptr->REGISTERS.OPERATIONAL_DEVICE_REGISTERS.USB_CMD = 0x00080000 | EHCI_CMD_RUN_STOP;
...
*
*END*-----------------------------------------------------------------*/
void _usb_dci_vusb20_isr(uint_32 IRQ)
{ /* Body */
   USB_DEV_STATE_STRUCT_PTR                     usb_dev_ptr;
   VUSB20_REG_STRUCT_PTR                        dev_ptr;
   uint_32                                      status;
   ...
   for (;;) {
      ...
      if (status & EHCI_STS_PORT_CHANGE) {
      	//smt2UARTPrint(CFG_UART_CH, "EHCI_STS_PORT_CHANGE\n");
      ...
      if (status & EHCI_STS_ERR) {
      	//smt2UARTPrint(CFG_UART_CH, "EHCI_STS_ERR\n");
