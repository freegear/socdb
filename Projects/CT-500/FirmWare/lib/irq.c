...
#ifdef __NESTED_INT__
/*----------------------------------------------------------
	Function name	: InterruptHandler
	Prototype		: void InterruptHandler(smtUint32 irq)
	Return			: void
	Argument		: 
	Comments		:
----------------------------------------------------------*/
#define INTMASK_PRIO(x) (0xFFFFFFFFUL << (x))

void InterruptHandler(smtUint32 irq, smtUint32 intMaskVal)
{
	if(ISRTable[irq] != 0)
	{
		ISRTable[irq](irq);
	...
	{
		smt2UARTPrint(CFG_UART_CH, "Unknown IRQ[0x%08x]\n",irq);
		smt2UARTPrint(CFG_UART_CH, "INTMASK[0x%08x]\n",SMT_READ(INTMSK));
		smt2UARTPrint(CFG_UART_CH, "INTMODE[0x%08x]\n",SMT_READ(INTMOD));
	}

	SMT_WRITE(INTMSK, intMaskVal);
	EnableIRQ(irq);
}

/*----------------------------------------------------------
	Function name	: AckIRQNested
	Prototype		: void AckIRQNested(void)
	Return			: void
	Argument		: 
	Comments		:
----------------------------------------------------------*/
smtUint32 AckIRQNested(void)
{
	smtUint32 irq;
	...
	AckIRQ(irq);
	DisableIRQ(irq);
	SMT_WRITE(INTMSK, INTMASK_PRIO(irq));
	return irq;
}
#else
/*----------------------------------------------------------
...
{
	smt2UARTPrint(CFG_UART_CH, "UndefHandler()\n");
}
...
void UnknownGpioIRQ(smtUint32 GPIOPin)
{
	...
	smt2UARTPrint(CFG_UART_CH, "UnknownGpioIRQ\n");
...
{
	extern void smtGDMASetIntOn(smtBoolean);
	smtGDMASetIntOn(SMT_TRUE);
...
{
	smt2UARTPrint(CFG_UART_CH, "HandlerUndef \n");
}
...
{
	smt2UARTPrint(CFG_UART_CH, "HandlerPabort \n");
}
...
{
	smt2UARTPrint(CFG_UART_CH, "HandlerDabort \n");
}
...
{
	smt2UARTPrint(CFG_UART_CH, "HandlerFIQ \n");
}
