...
module     cardctrlwrap
  (
   ...
   si_addr             ,
	...
	si_size				, // for mmc ram setting 
	si_wr				, // indicate command
...
//   reg 					   si_newopb; // new operation 2007.6.21
   reg [1:0] 			   si_csb;
   ...
   parameter 			   CI_SM_NUM = CI_END_CMD + 1;
   ...
   always@(ci_cs or ci_adsb or rdyb_hsk or lastb_hsk or ads_fgb) begin
	  case(1'b1)
		ci_cs[CI_IDLE] : begin
		   if(ci_adsb == 1'b0 || ads_fgb == 1'b0)
	  ...
	  endcase // case(1'b1)
   end // always@ (ci_cs or ci_adsb or rdyb_hsk or lastb_hsk)
   ...
   always@(si_cs or adsb_hsk or si_rdyb or wr_hsk or 
		   rdyb_hsk or lastb_hsk or si_err) begin
	  case(1'b1)
		si_cs[SI_IDLE] : begin
		   if(adsb_hsk == 1'b0) begin
				// revision by duck new operation no need ready sign
				si_ns <= SI_ST_NEW_OP;
   ...
   //---------------------------------------------------------------
   // End command state 동안에 새로운 operation이 시작 될 경우.
   always@(posedge clk_card or negedge rstb) begin
   ...
   always@(posedge clk_sys or negedge rstb) begin
	...
	// indicate write read operation (in new operation time)
