...
module     cardctrlwrap
  (
   ...
   //-------------------------------------------------------------------
   // Handshake signals
   wire 				   abortb_hsk_1; // 2007.7.20   // card domain
	reg						abortb_hsk;
	reg						abortb_hsk_sync;

	always @(posedge clk_sys or negedge rstb) 
	begin
	if (!rstb)
		begin	
		abortb_hsk_sync <= 1'b1;
		abortb_hsk<= 1'b1;
		end
	else	
		begin
		abortb_hsk_sync <= abortb_hsk_1;
		abortb_hsk		<= abortb_hsk_sync;
		end
	end
   ...
   reg [1:0] 			   csb_hsk;
   reg [1:0] 			   csb_hsk_sync;
	always @(posedge clk_sys or negedge rstb) 
	begin
	if (!rstb)
   		csb_hsk_sync <= 2'b00;
	else
		csb_hsk_sync	<= csb_hsk;
	end

   wire 				   rdyb_hsk;
   reg	 				   rdyb_hsk_sync;
	always @(posedge clk_card or negedge rstb) 
	begin
	if (!rstb)
   		rdyb_hsk_sync <= 1'b1;
	else
   		rdyb_hsk_sync <= rdyb_hsk;
	end




   wire 				   adsb_hsk;   // casd domain
	reg						adsb_hsk_sync;
	always @(posedge clk_sys or negedge rstb) 
	begin
	if (!rstb)
...
//   reg                     f_sm_loop; // first state machine loop
   ...
   always@(ci_abortb or ci_cs or ci_adsb or rdyb_hsk_sync or
	  ...
	  case(1'b1)
		...
		ci_cs[CI_END_CMD] : begin
		   if(ci_abortb == 1'b0)
			 ci_ns <= CI_ST_ABORT;
		   else if(rdyb_hsk_sync == 1'b1) begin
			  if(lastb_hsk == 1'b1)
				ci_ns <= CI_ST_SEND_CMD;
			  else ci_ns <= CI_ST_IDLE;
		   end
		...
		ci_cs[CI_ABORT] : begin
		   if(rdyb_hsk_sync == 1'b1 && adsb_hsk == 1'b1)
			 ci_ns <= CI_ST_IDLE;
   ...
   always@(si_cs or adsb_hsk_sync or si_rdyb or wr_hsk_sync or si_rdb or 
		   rdyb_hsk or lastb_hsk_sync or si_err or abortb_hsk) begin
	  case(1'b1)
		si_cs[SI_IDLE] : begin
		   if(abortb_hsk != 1'b0 && adsb_hsk_sync == 1'b0) begin
				si_ns <= SI_ST_NEW_OP;
/*			  if(si_rdyb == 1'b0) begin
		...
		si_cs[SI_END_CMD] : begin
		   if(abortb_hsk == 1'b0)
			 si_ns <= SI_ST_IDLE;
		   else if(adsb_hsk_sync == 1'b1) begin
			  if(lastb_hsk_sync == 1'b0 || si_err == 1'b1) // last or error.
				si_ns <= SI_ST_IDLE;
		...
		si_cs[SI_WAIT_ADS] : begin
		   if(abortb_hsk == 1'b0)
			 si_ns <= SI_ST_IDLE;
		   else if(adsb_hsk_sync == 1'b0 && si_rdyb == 1'b0 && 
                   si_rdb == 1'b1 )
			  si_ns <= SI_ST_SEND_RDY;
		   else si_ns <= SI_ST_WAIT_ADS;
		end
   ...
   assign abortb_hsk_1 = ci_abortb & (~ci_sm_abort); // 2007.7.20
   
   always@(posedge clk_card or negedge rstb) begin
	  if(~rstb) ads_fgb <= 1'b1;
	  else if(abortb_hsk_1 == 1'b0) ads_fgb <= 1'b1;
   ...
   always@(posedge clk_card or negedge rstb) begin
	  ...
	  else if(abortb_hsk_1 == 1'b0) lastb_hsk <= 1'b1;
   ...
   always@(posedge clk_card or negedge rstb) begin
	  if(~rstb) wr_hsk <= 1'b0;
	  else if(abortb_hsk_1 == 1'b0) wr_hsk <= 1'b0;
	  else if(ci_adsb == 1'b0)
		wr_hsk <= ci_wr;
   end // always wr_hsk
   ...
   always@(posedge clk_card or negedge rstb) begin
	...
	always@(posedge clk_sys or negedge rstb) begin
		...
		else si_abortb <= abortb_hsk_sync;
	end
   ...
   always@(posedge clk_card or negedge rstb) begin
	  ...
	  else if(abortb_hsk_1 == 1'b0) ci_error <= 1'b0;
	  else if(ci_sm_send_cmd) ci_error <= si_err;
	  else if(ci_sm_end_cmd) ci_error <= 1'b0;
   end // always ci_error


   assign si_wrb = (si_sm_send_rdy)? ~wr_hsk_sync : 1'b1;
   ...
   assign si_rdb = ((~si_sm_end_cmd & si_sm_end_cmd_dly) | si_sm_send_rd)? wr_hsk_sync : 1'b1;
   ...
   always@(posedge clk_card or negedge rstb) begin
	  ...
	  else if(abortb_hsk_1 == 1'b0) csb_hsk <= 2'h3;
   ...
   always@(posedge clk_sys or negedge rstb) begin
	  ...
	  else if(si_sm_idle) si_csb <= csb_hsk_sync;
   end // always si_csb
...
*/
   ...
   always@(posedge clk_sys or negedge rstb) begin
	  ...
	  else if(adsb_hsk_sync == 1'b0 && (si_sm_idle | si_sm_wait_ads)) begin
		 si_wdata <= wdata_hsk;
		 si_beb <= beb_hsk;
	  end
