...
*/
    ...
    nNFCE3Out       ,       //07_06_19 : Register Revision
    nNFCE2Out       ,
    nNFCE1Out       ,
    nNFCE0Out       ,
...
//Control reg
wire        NFCtrlRst   = NFCTRL1In[5];   

//wire[1:0]   NFIDByte    = NFCTRLIn[9:8];
wire        Ecc512En    = NFCTRL1In[1];
wire        AutoEccWr   = NFCTRL1In[0];
wire        DMAEn       = NFCTRL0In[7];
wire        WrEndIntEn  = NFCTRL0In[6];
wire        RdEndINTEn  = NFCTRL0In[5];
//wire        EccErrIntEn = NFCTRLIn[3]; //2.9
wire        FIFOintEn   = NFCTRL0In[4];
wire        RnBIntEn3   = NFCTRL0In[3];
wire        RnBIntEn2   = NFCTRL0In[2];
wire        RnBIntEn1   = NFCTRL0In[1];
wire        RnBIntEn0   = NFCTRL0In[0];


//configuration reg
wire        NFBootEn = NFCONF2In[2];
wire        IOWidth  = NFCONF2In[1];
wire        NandWidth= NFCONF2In[0];
wire[ 1:0]  BootCfg  = NFCONF1In[7:6];
wire        OutDtmn  = NFCONF1In[5];
wire[ 3:0]  TADLTWB  = NFCONF1In[4:1];
wire[ 2:0]  TCALS    = {NFCONF1In[0],NFCONF0In[ 7:6]};
wire[ 2:0]  TRWLP    = NFCONF0In[ 5:3];
wire[ 2:0]  TRWHP    = NFCONF0In[ 2:0];
...
wire[1:0]   ChipSel          = FirstOper[31:30];

wire        RnBWait          = (FirstOper[29:28]==2'b01) ? 1'b1 : 1'b0;
wire        AutoRdStat       = (FirstOper[29:28]==2'b10) ? 1'b1 : 1'b0;
wire        Continue         = (FirstOper[29:28]==2'b11) ? 1'b1 : 1'b0;

wire[11:0]  DataSize         = FirstOper[27:16];
//wire[ 2:0]  FIFOLevelSet     = FirstOper[18:16];   
//wire[ 2:0]  OpMode           = FirstOper[15:13];
wire        TransSize        = FirstOper[12]; // byte,halfword,word select
wire[ 3:0]  CmdAddrTransByte = FirstOper[11:8];
wire[ 7:0]  CmdAddrFlag      = FirstOper[ 7:0];

wire        ReadData   = (FirstOper[15:13]==3'b000) ? 1'b1 : 1'b0;
wire        ReadStatus = (FirstOper[15:13]==3'b001) ? 1'b1 : 1'b0;
wire        ReadID     = (FirstOper[15:13]==3'b010) ? 1'b1 : 1'b0;
wire        WriteData  = (FirstOper[15:13]==3'b011) ? 1'b1 : 1'b0;
//wire        CellWrite  = (FirstOper[16:14]==3'b100) ? 1'b1 : 1'b0;
//wire        Continue   = (FirstOper[16:14]==3'b101) ? 1'b1 : 1'b0;
wire        NOP        = (FirstOper[15:13]==3'b111) ? 1'b1 : 1'b0;
...
begin
    if(!nRst) begin
        ...
        if(DMAEn==1'b1) begin
            if(ReadData | ReadID) begin
               // DMAReqOut<=FIFOHalfFullIn | FIFOFullIn | DataEnd;
                DMAReqOut<=RdRdyIn | DataEnd; //2007.07.12
            end
...
assign Byte_8bitBus      = (TransSize==1'b0 && IOWidth==1'b0) ? 1'b1 : 1'b0;
assign Byte_16bitBus     = (TransSize==1'b0 && IOWidth==1'b1) ? 1'b1 : 1'b0;
assign HalfWord_8bitBus  = (TransSize==1'b1 && IOWidth==1'b0) ? 1'b1 : 1'b0;
assign HalfWord_16bitBus = (TransSize==1'b1 && IOWidth==1'b1) ? 1'b1 : 1'b0;
//assign Word_8bitBus      = (TransSize==1'b10 && IOWidth==1'b0) ? 1'b1 : 1'b0;
//assign Word_16bitBus     = (TransSize==1'b10 && IOWidth==1'b1) ? 1'b1 : 1'b0;
...
begin
    if(!nRst) begin
        ...
        /* 3.5  ******************
            ...
            if(Byte_16bitBus || HalfWord_8bitBus) begin
...
assign RnBHigh          = (ChipSel==2'b11) ? FiltRnB3In : 
                          (ChipSel==2'b10) ? FiltRnB2In :
                          (ChipSel==2'b01) ? FiltRnB1In : FiltRnB0In;
...
// Operation Read 
wire OpRdEnd;
assign  NFOpRdEnOut   = ( (NFBootEn==1'b0) && OperRdEn==1 && OperRdEnDly==0) ? 1'b1 : 1'b0;
...
begin
    if(!nRst) begin
        ...
        if(NST_Load==1'b1 && 
            ( ((QLevelIn!=0) && NFOpWordNum!=LoadCnt && OperRdEnDly!=1'b1))) begin
            OperRdEn<=~OperRdEn;    
        end
        else OperRdEn<=1'b0;
    end
...
begin
    if(!nRst) begin
        ...
        if((NST_Load && QLevelIn!=0 && NFOpRdEnOut==1'b1)) begin
            if(LoadCnt==0) FirstOper<=NFOpIn;
            else if(LoadCnt==1) CmdAddr1<=NFOpIn;
            else if(LoadCnt==2) CmdAddr2<=NFOpIn;
        end
    end
end
//**************************************************************
...
reg         nNFCE2Out;           //07_06_19 : Register Revision            
reg         nNFCE3Out;
...
begin
    if(!nRst) begin
        ...
        nNFCE2Out<=1'b1;            //07_06_19 : Register Revision 
        nNFCE3Out<=1'b1;
        ...
        if(NST_Idle==1'b1) begin
            if(Continue==1'b1 || RnBWait==1'b1) begin
                ...
                nNFCE2Out<=nNFCE2Out;           //07_06_19 : Register Revision 
                nNFCE3Out<=nNFCE3Out;
        ...
        end                                 //07_06_19 : Register Revision
