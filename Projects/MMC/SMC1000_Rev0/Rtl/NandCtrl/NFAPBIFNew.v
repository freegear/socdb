...
assign ControlOut1 = {2'b0,NFCtrlRst,Ctrl1[4:0]};
...
wire	NFOPER0_adr 	= CS[0];
wire	NFOPER1_adr 	= CS[1];
wire	NFOPER2_adr 	= CS[2];
wire	NFOPER3_adr 	= CS[3];
wire 	NFCONF0_adr 	= CS[4];
wire 	NFCONF1_adr 	= CS[5];
wire 	NFCONF2_adr 	= CS[6];
wire 	NFCTRL0_adr 	= CS[7];
wire 	NFCTRL1_adr 	= CS[8];
wire 	NFSTAT0_adr 	= CS[9];
wire 	NFSTAT1_adr 	= CS[10];
wire 	NFSTAT2_adr 	= CS[11];
wire 	NFSTAT3_adr 	= CS[12];
wire 	NFFIFOSTAT0_adr = CS[13];
wire 	NFFIFOSTAT1_adr = CS[14];

always @(posedge CLK or negedge RESETn)
    if(!RESETn)
        QWrCnt <= 0;
    else if((NFOPER0_adr&EXT_SFR_WR)||(NFOPER1_adr&EXT_SFR_WR)||(NFOPER2_adr&EXT_SFR_WR)||(NFOPER3_adr&EXT_SFR_WR)) 
        QWrCnt <= QWrCnt + 1;
    else if(QWrCnt==4)
        QWrCnt <=0;
...
always @(posedge CLK or negedge RESETn)
    ...
    begin
        ...
        if     ((NFOPER0_adr) & (EXT_SFR_WR)) NFOPER0 <= EXT_SFR_DOUT;
        else if((NFOPER1_adr) & (EXT_SFR_WR)) NFOPER1 <= EXT_SFR_DOUT;
        else if((NFOPER2_adr) & (EXT_SFR_WR)) NFOPER2 <= EXT_SFR_DOUT;
        else if((NFOPER3_adr) & (EXT_SFR_WR)) NFOPER3 <= EXT_SFR_DOUT;
...
assign FIFOWrEn   =((We && TransSize==1'b1 && FIFOWrCnt==1'b1) || 
                    (We && TransSize==1'b0) || FIFOWrDataEnIn == 1'b1) ? 1'b1 : 1'b0;
/*
assign FIFOWrData = (We && TransSize==1'b1) ? {WDATA,FIFOWrCollect[15:8]} : 
                    (We && TransSize==1'b0) ? {8'h00,WDATA} : FIFOWrDataIn;
*/
assign FIFOWrData = (We && TransSize==1'b1) ? {FIFOWrCollect[7:0],WDATA} : 
                    (We && TransSize==1'b0) ? {8'h00,WDATA} : FIFOWrDataIn;
assign FIFORdEn  = ((Oe && TransSize==1'b1 && FIFORdCnt==1'b1) || 
...
always @(posedge CLK or negedge RESETn)
    if(!RESETn)
        ...
        if(We && TransSize==1'b1) 
            ...
            FIFOWrCollect <= {FIFOWrCollect[7:0],WDATA};
    end

always @(posedge CLK or negedge RESETn)
    if(!RESETn)
        FIFOWrCnt <= 0;
    else if(We && TransSize==1'b1)
   // else if(FIFOWrCnt==2)
   //         FIFOWrCnt <= 0;
   // else if(We)
...
begin
    if(!RESETn) begin
        ...
        if(EXT_SFR_WR && NFSTAT0_adr && EXT_SFR_DOUT[0]==1'b1) NFStatValid<=1'b0;
...
begin
    if(!RESETn) begin
        ...
        if(EXT_SFR_WR && NFSTAT0_adr && EXT_SFR_DOUT[7]==1'b1) WrEnd<=1'b0;
...
begin
    if(!RESETn) begin
        ...
        if(EXT_SFR_WR && NFSTAT0_adr && EXT_SFR_DOUT[6]==1'b1) RdEnd<=1'b0;
...
begin
    if(!RESETn) begin
        ...
        RnB3_Dly<=FiltRnB3In;
        if(EXT_SFR_WR && NFSTAT3_adr && EXT_SFR_DOUT[7]==1'b1) RnBDetect3<=1'b0;
...
begin
    if(!RESETn) begin
        ...
        RnB2_Dly<=FiltRnB2In;
        if(EXT_SFR_WR && NFSTAT3_adr && EXT_SFR_DOUT[6]==1'b1) RnBDetect2<=1'b0;
...
begin
    if(!RESETn) begin
        ...
        RnB1_Dly<=FiltRnB1In;
        if(EXT_SFR_WR && NFSTAT3_adr && EXT_SFR_DOUT[5]==1'b1) RnBDetect1<=1'b0;
...
begin
    if(!RESETn) begin
        ...
        RnB0_Dly<=FiltRnB0In;
        if(EXT_SFR_WR && NFSTAT3_adr && EXT_SFR_DOUT[4]==1'b1) RnBDetect0<=1'b0;
...
begin
    if(!RESETn) begin
        ...
        NFCtrlRst<=1'b0;
            ...
            if((NFCONF0_adr) & (EXT_SFR_WR))	Timing_conf0<=EXT_SFR_DOUT;
            else if((NFCONF1_adr) & (EXT_SFR_WR))
					begin
					Timing_conf1<=EXT_SFR_DOUT[4:0];
					BootCfgPinIn <=EXT_SFR_DOUT[7:6];
					OutDtmnPinIn <= EXT_SFR_DOUT[5];
					end
			else if((NFCONF2_adr)& (EXT_SFR_WR))
					begin
					IOWidthPinIn <= EXT_SFR_DOUT[1];
					NandWidthPinIn <= EXT_SFR_DOUT[0];
					end
            else if((NFCTRL0_adr) & (EXT_SFR_WR))	Control0   <=EXT_SFR_DOUT;
            else if((NFCTRL1_adr)& (EXT_SFR_WR))
                begin
