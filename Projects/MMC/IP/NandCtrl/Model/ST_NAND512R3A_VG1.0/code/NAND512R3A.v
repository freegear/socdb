/****************************************************************************************
  ...
  NAND512R3A Parameters Definitions

  512Mbits (x8, 4096 Blocks, 32 Pages, 528 Bytes) 1.8V NAND Flash Memory
...
// Memory control
//-----------------------------------------------------------------------------
`define MEMORY_ALLOC        0  //1:whole 0:partial
...
`define DEVICE_CODE         8'h36

//=======================================================================================
// NAND512R3A 512Mbits(x8 bus width), 528 Bytes Page, 1.8V Device Simulation Model
//=======================================================================================
module NAND512R3A(
                   IO,
                   CE,
...
begin
  ...
  //-----------------------------------------------------------------
    if(MOD_READ_MODE || MOD_PAGE_PROGRAM_1 || MOD_COPY_BACK_PG) begin
      case(add_cycle)
        4:   REG_ADD <= {REG_ADD[`ADD_BIT-1:9],area_b,iIO};
        3:   REG_ADD <= {REG_ADD[`ADD_BIT-1:17],iIO,REG_ADD[8:0]};
        2:   REG_ADD <= {REG_ADD[25],iIO,REG_ADD[16:0]};
        1: begin
             REG_ADD <= {iIO,REG_ADD[24:0]};
  ...
  //-----------------------------------------------------------------
      ...
      case(add_cycle)
        3:   REG_ADD <= {REG_ADD[`ADD_BIT-1:17],iIO,{`ADD_COL{1'b0}}};
        2:   REG_ADD <= {REG_ADD[25],iIO,REG_ADD[16:0]};
        1: begin
             REG_ADD <= {iIO,REG_ADD[24:0]};
             ADD_COMPLETE <= 1'b1; //cycles for address input is over
...
begin
  if(power_on && MOD_READ_MODE && {iCE,iALE,iCLE,iRE,iWE,bWE}===6'b010110) begin
    if(add_cycle === 4 && area_c === 1'b0) addr_column <= {area_c,area_b,iIO};
    if(add_cycle === 4 && area_c === 1'b1) addr_column <= {area_c,area_b,4'b0000,iIO[3:0]};
  end else
...
begin
  ...
  begin
    ...
    if(COPYBACK_ID2 === 1'b1 && address_src[`CHP_BIT-1:`CHP_BIT-2] === address_tgt[`CHP_BIT-1:`CHP_BIT-2])
    begin
      for(addr_temp1 = 0 ; addr_temp1 < `COL_IN_PAGE ; addr_temp1 = addr_temp1 + 1)
      begin
    ...
    if(COPYBACK_ID2 === 1'b1 && address_src[`CHP_BIT-1:`CHP_BIT-2] !== address_tgt[`CHP_BIT-1:`CHP_BIT-2])
      $display("\n%t, Error: The most 2 significant address bits must be the same for source and target!\n",$realtime);
  end
  else if(wr_protect == 1'b1)
  begin
    //wr_protect <= 1'b0;
...
begin
  ...
  //---------------------------------------------------------------------------------------------------------
    CMD_READ_A :
      if(oRY_BY) begin CMD_CHECK = 1'b1; add_cycle = 4; end
      else begin
