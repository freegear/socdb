...
static smtUint32 	gl_dynamic_wear_info[FTL_FREEBLK_MAXNUM]; 

// bitfield for locking mechanism 
static smtUint8		gl_dynamic_lock[(FTL_FREEBLK_MAXNUM+7) >> 3];
...
) 
{
	...
	// get free table 
	free_table = gpFlush->freeBlks;
	for(idx = 0; idx < gpFlush->freeBlkNum; idx++,free_table++) 
	{
		// check if block is not used (free) 
		if (!((*free_table)&FTL_FBTYPE_BAD)) 	  
		{
...
) 
{
    ...
    if(static_lba != BLK_NA)
	{
		...
		smtBlkAddr		*free_table 	= gpFlush->freeBlks;
		smtUint8		find_index		= INDEX_NA;
		smtUint32 		find_wear		= WEAR_NA;

		for (a=0; a < gpFlush->freeBlkNum; a++)
		{
		...
		if (find_index!=INDEX_NA)
		{
			...
			{
				// incremented by 1! 
				gpWearAllocData->free_index=find_index;
				gpWearAllocData->free_wear=find_wear+1; 

				gpWearAllocData->static_lba=static_lba;
				gpWearAllocData->static_wear=static_wear;
	...
	gpWearAllocData->free_index	= INDEX_NA;
	gpWearAllocData->static_lba	= BLK_NA;
	gpWearAllocData->static_wear	= WEAR_NA;
...
/*----------------------------------------------------------
	...
	Comments		: 
		...
		gl_wear_allocstruct holds all the information                                                                  
...
)
{
	...
	num			= (smtUint8)gpFlush->freeBlkNum;
	free_table	= &gpFlush->freeBlks[index];
	...
	{	
		...
		{	
			...
			{ 
				...
				gpWearAllocData->free_index	= index; 
				gpWearAllocData->free_wear	= gl_dynamic_wear_info[index]+1; 
	...
	gl_static->dynamic_index		= index;		
 	gpWearAllocData->free_index	= INDEX_NA;
 	gpWearAllocData->free_wear	= WEAR_NA;		
