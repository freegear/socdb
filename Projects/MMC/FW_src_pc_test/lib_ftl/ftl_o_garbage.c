...
)
{
	...
	{ // get original 
	 	...
	 	wear = GET_SPARE_AREA(gTempBuffer)->wear;
		if (wear<WEAR_NA) 
		{
	...
	if(FTL_W_ChkStatic())
	{
		...
		smtBlkAddr 		s_pba;
		smtUint8  	index = gWearAllocInfo.free_index;
		...
		{
			int ret=FTL_L_Read(s_pba,po,gTempBuffer);
			if (!po)
			{
		...
		FTL_W_UpdateSInfo(lba,gWearAllocInfo.free_wear);

		gFlush.freeBlks[index] = s_pba;
		FTL_W_UpdateDInfo(index,gWearAllocInfo.static_wear);
...
) 
{
	...
	{ //  check if switchable 
		...
		if (!FTL_L_Read(orig_pba, 0, gTempBuffer))
		{
		 	wear = GET_SPARE_AREA(gTempBuffer)->wear;
		}
...
again:
	...
	for (po = 0; po < gDevInfo.pagePerBlks; po++)
	{
		...
		if(lpo == INDEX_NA)
		{
			FTL_L_Read(orig_pba, po, gTempBuffer);
		}
		else
		{
			FTL_L_Read(log->pba,lpo,gTempBuffer);	
		}
		...
		if (FTL_L_Write(pba, po, gTempBuffer) == FTL_L_ERROR) 
		{
			goto again;
		}
	}
	...
	FTL_W_UpdateSInfo(log->lba, gWearAllocInfo.free_wear);
