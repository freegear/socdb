...
reg [RasQW:0] ras_8_data;
reg [RasQW:0] ras_9_data;
reg [RasQW:0] ras_a_data;
reg [RasQW:0] ras_b_data;
reg [RasQW:0] ras_c_data;
reg [RasQW:0] ras_d_data;
reg [RasQW:0] ras_e_data;
reg [RasQW:0] ras_f_data;
    ...
    if (!ARESETB) begin
        ...
        if (CMD == bc_sras | CMD == bc_mras) begin
            if (ras_cnt != 0) begin
                ...
                RAS_0_RA   <= ras_1_data[RAW+BL+1: BL+1]; // 15:4
                RAS_0_TT   <= ras_1_data[BL: 0]; 		//  3:0
                ...
                ras_7_data <= ras_8_data;
                ras_8_data <= ras_9_data;
                ras_9_data <= ras_a_data;
                ras_a_data <= ras_b_data;
                ras_b_data <= ras_c_data;
                ras_c_data <= ras_d_data;
                ras_d_data <= ras_e_data;
                ras_e_data <= ras_f_data;
                ras_f_data <= 0;
                ras_cnt     = ras_cnt - 1;
            end
        end
        ...
        if ((BA_REQ &&
            ((BA_BA == 2'b00 && ({1'b0, BA_RA} != l_b0_last_ra))||
             ...
             (CLOSING)))) begin	// Should Fix Architecture Bug
            case(BA_BA)
            ...
            case(ras_cnt)
              		...
              		8 : ras_8_data <= {BA_BA, BA_RA, BA_TT};
              		9 : ras_9_data <= {BA_BA, BA_RA, BA_TT};
              		10: ras_a_data <= {BA_BA, BA_RA, BA_TT};
              		11: ras_b_data <= {BA_BA, BA_RA, BA_TT};
              		12: ras_c_data <= {BA_BA, BA_RA, BA_TT};
              		13: ras_d_data <= {BA_BA, BA_RA, BA_TT};
              		14: ras_e_data <= {BA_BA, BA_RA, BA_TT};
              		15: ras_f_data <= {BA_BA, BA_RA, BA_TT};
            endcase
            if (ras_cnt <= 15) begin
                ras_cnt = ras_cnt + 1;
            end
