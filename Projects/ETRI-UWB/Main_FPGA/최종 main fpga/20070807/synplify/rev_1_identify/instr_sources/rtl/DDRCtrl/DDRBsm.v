...
input     		CAS_0_NEWROW;
input     		CAS_EMPTY   ;
input			WriteToRead, BurstWTREnd, SingleWTREnd;
input  [ 2:0]	BurstWTRCnt;
...
begin
    if (!ARESETB) begin
        ...
        else if (length != 0 & (BurstWTRCnt != 0 | BurstWTRCnt != 1)) begin
            length <= length - 1;
        end
    end
end
...
always @(cbs_idle or cbs_rowopen or
         ...
         trp_cnt or trasmin_cnt or BurstWTREnd or WriteToRead or SingleWTREnd)
begin
    i_bf_reqcmd = bc_null;
    case(1'b1)
      cbs_idle :
        ...
        if    (~CAS_EMPTY && BF_NO == CAS_0_BA && ~RAS_EMPTY && BF_NO == RAS_0_BA) begin
            if (CAS_0_RA == RAS_0_RA)
                ...
                if (~BF_TCASBUSY) begin
                	if (WriteToRead) begin
                    	if (CAS_0_TT == 0 & SingleWTREnd)
                    	     i_bf_reqcmd = bc_scas;
                    	else if (CAS_0_TT != 0 & CAS_0_TT <= {BL+1{1'b1}} & BurstWTREnd)
                    	     i_bf_reqcmd = bc_mcas;
                    	else i_bf_reqcmd = bc_null;
                	end
                    else begin
...
begin
    ...
    case(1'b1)
      cbs_idle :
        if (~RAS_EMPTY && BF_NO == RAS_0_BA & ~BF_TRASBUSY & trp_cnt == 0) begin
            if (i_bf_reqcmd == BF_CMD) // request accepted
                 nextstate = bs_rasing;
...
*/
      ...
      cbs_casing :
      	if (WriteToRead)begin
        	if (BurstWTREnd)  	nextstate = bs_rowopen;
