...
output [23:0] 	CPlanePalMemWrData;
output			CPlanePalMemRead;
input  [23:0] 	CPlanePalMemRdData;
...
output [23:0] 	GPlanePalMemWrData;
output			GPlanePalMemRead;
input  [23:0] 	GPlanePalMemRdData;
...
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
		...
		CPalMemWrAddr	<= PWDATA[31:24];
	    CPalMemWrData  	<= PWDATA[23:0];
	end
...
assign CPlanePalMemRead   = CPalMemRead;
...
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
	    ...
	    LCDControlPCLK <= {1'b1, 6'b0, 2'b11, 4'b0};
	end
	    ...
	    LCDControlPCLK <= {PWDATA[31:30], PWDATA[10:0]};
	end
...
//			rCADDR2Rd	: PRDATA <= {CDMARAddr, 2'b0};
			...
			rCPALMRd	: PRDATA <= {CPalMemWrAddr, CPlanePalMemRdData};
			...
			rGPALMRd	: PRDATA <= {GPalMemWrAddr, GPlanePalMemRdData};
