...
void WDTpreScaleTest(smtUint16 min, smtUint16 max)
{
	...
	for (value = min; value < max; value+=0x50)
	{
		...
		smt2UartPrint(11, "WDT preScale Test : 0x%x\n", value);
	}

	ReleaseIRQ(IRQ_WDT);
	smt2UartPrint(11, "WDT preScale Test done\n");
}
...
void WDTReloadTest(smtUint16 min, smtUint16 max)
{
	...
	for (value = min; value < max; value+=0x50)
	{
		...
		smt2UartPrint(11, "WDT Reload Test : 0x%x\n", value);
	}

	ReleaseIRQ(IRQ_WDT);
	smt2UartPrint(11, "WDT Reload Test done\n");
}
...
void WDTDivideTest(void)
{
	...
	for (clkSel = 0; clkSel < 2; clkSel++)
		for (value = 0; value < 4; value++)
		{
			...
			smt2UartPrint(11, "WDT Divide Test : 0x%x\n", value);
		}

	ReleaseIRQ(IRQ_WDT);
	smt2UartPrint(11, "WDT Divide Test done\n");
}
