...
-- Input file was: vusb_hs_core_dev_amba_apb.vhdl
-- Date Created: Thu Dec 21 22:43:52 2006
...
// ------------------------------------------------------------------------------
//  File          : $HeadURL: svn+ssh://porus/ci/svn/USBCTRL/HSCTRL/trunk/HSCTRL/digital/design/hsctrl/rtl_vhdl_ahb/vusb_hs_core_dev.vhdl.tmpl $                                
...
// ------------------------------------------------------------------------------
//  Last modification   :                                                        
//  $Date: 2006-09-25 19:57:40 +0100 (Mon, 25 Sep 2006) $                                                                       
//  $Revision: 297 $                                                                   
module vusb_hs_core_dev_amba_apb (clk,
   rst,
   aux_clk,
   xcvr_clk,
   xcvr_ser_clk,
...
output   [2:0] m_hburst; 
output   [3:0] m_hprot; 
output   [BUS_TYPE_INFO_WIDTH - 1:0] m_htypeinfo; //  Master Packet type (not for general use; leave open)
...
wire    [2:0] m_hburst; 
wire    [3:0] m_hprot; 
wire    [BUS_TYPE_INFO_WIDTH - 1:0] m_htypeinfo; 
wire    [31:0] m_hwdata; 
wire    m_hbusreq; 
wire    m_hlock; 
...
wire    [8:0] s_paddr_i; 
wire    s_pwrite_i; 
wire    [31:0] s_pwdata_i; 
wire    [31:0] m_hrdata_i; 
wire    m_hready_i; 
wire    [1:0] m_hresp_i; 
wire    m_hgrant_i; 
// ---------------------------------------------------------------------------
...
// -------------------------------------------------------------------
assign s_penable_i = s_penable; 
assign s_psel_i = s_psel; 
assign s_paddr_i = s_paddr; 
assign s_pwrite_i = s_pwrite; 
assign s_pwdata_i = s_pwdata; 
assign m_hrdata_i = m_hrdata; 
assign m_hready_i = m_hready; 
assign m_hresp_i = m_hresp; 
assign m_hgrant_i = m_hgrant; 
...
// -------------------------------------------------------------------
vusb_hs_up_int_amba_apb #(0,1,VUSB_HS_DEV_EP_TX,VUSB_HS_DEV_EP_RX) U_up_int (.clk(clk_i),
...
// -------------------------------------------------------------------
vusb_hs_dma_dev_amba_apb #(VUSB_HS_DEV_EP_TX,VUSB_HS_DEV_EP_ADD_TX,VUSB_HS_DEV_EP_RX,VUSB_HS_DEV_EP_ADD_RX) U_dma (.clk(clk_i),
          //  do not change to clk!
          .rst_local(rst_local),
          .rst_local_a(rst_local_a),
          .m_haddr(m_haddr),
...
endmodule // module vusb_hs_core_dev_amba_apb
