...
void UartInit(void)
{
	SMT_WRITE(UART1_BRD, BAUDRATE_SETTING);
...
void UartPutch(char ch)
{
  ...
  while (1) {
    if((SMT_READ(UART1_SR) & 0x00003f00 )  != 0x00002000) // FIFO no full
//    if((SMT_READ(UART1_SR) & 0x00003f00 )  != 0x00000700) // FIFO no full
//    if((SMT_READ(UART1_SR) & 0x00003f00)  == 0x00000000) // FIFO is empty
    {
      SMT_WRITE(UART1_TX, (long)(ch & 0xff));
      return ;
    }
  }
}
...
char UartGetch()
{
    while((SMT_READ(UART1_SR) & 0x0000003f ) == 0x00000000); // FIFO no full

    return (SMT_READ(UART1_RX)& 0xff);  
}
...
{
	return ((SMT_READ(UART1_SR) & 0x0000003f) != 0x00000000);
}
