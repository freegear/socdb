/******************************************************************************
	CQ RISC評価キット/SH-3 割り込み処理ベクタ初期化
******************************************************************************/

#include	<stdio.h>
#include	"SH3.H"

/* SH-3 割り込みベクタテーブル 0x0C000000～ */
...
{
	/* VBR(VBR_ADR=0x0C000000)に割り込みベクタを転送 */
	/* 一般例外処理を転送 */
	movmem(_ex_int_start_,(void *)0x0c000100,_ex_int_end_ - _ex_int_start_);
	/* 一般割り込みベクタテーブル(VBR(0xc000000))へプログラムを転送 */
	movmem(_int_start_,(void *)0x0c000600,_int_end_ - _int_start_);
...
__ex_int_start_:
	...
	mov.l	r0,@-r15
	mov.l	r1,@-r15
	mov.l	L06,r0		/* 例外コードをチェック */
	mov.l	@r0,r1
	mov.l	L07,r0		/* 0x180 不当命令例外コード */
	cmp/eq	r0,r1		/* 不当命令 ? */
	bf	nobreak
	mov.l	@r15+,r1
	mov.l	@r15+,r0
...
__int_start_:
	stc.l	ssr,@-r15
	stc.l	spc,@-r15
/* PARTNER ではSPC,SSRを待避し、SR.BLを0に
   したあとでブレークしなければなりません */
	mov.l	r0,@-r15
	mov.l	r1,@-r15
	mov.l	INTEVT,r0		/* 例外コードをチェック */
	mov.l	@r0,r1
	mov.l	NMI,r0		/* 0x1c0 NMI例外コード */
	cmp/eq	r0,r1		/* NMI ? */
	bf	nonmi
	mov.l	@r15+,r1
	mov.l	@r15+,r0
...
nextIRL0:
	...
	.align 2
INTEVT:
	.long	0xffffffd8	/* INTEVTレジスタアドレス */
BLBIT:
	.long	0xefffffff	/* BLビットを0 */
NMI:
	.long	0x1c0		/* NMI 例外コード */
PARTNER:
	.long	0x612		/* モニタジャンプ先 */
IRL_adr:
