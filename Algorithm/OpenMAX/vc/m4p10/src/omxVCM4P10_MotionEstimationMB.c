...
static OMXResult armVCM4P10_PartitionME (
    const OMX_U8* pSrcCurrBuf,   
    OMX_S32 SrcCurrStep,
	const OMX_U8 *pSrcRefBufList[16],
	OMX_S32 SrcRefStep,
	const OMXRect *pRefRect,
	const OMXVCM4P2Coordinate *pCurrPointPos,
    void* pMESpec, 
	...
	OMX_U32 *pDstCost)
{
    ...
    const OMXVCM4P10SubMacroblockType     
                ModeSubMBType4x4 [] = {OMX_VC_SUB_4x4, OMX_VC_SUB_4x8, 
                              OMX_VC_SUB_8x4, OMX_VC_SUB_8x8};
    ...
    else
    {
        ...
        for (i = 0; i < 4; i++)
        {
            pSrcDstMBCurr->subMBType [i] = OMX_VC_SUB_8x8;
        }
    }
 ...
 * [in] pSrcRefBufList   Pointer to an array with 16 entries. Each entry points to the top-left corner of
	...
	OMX_INT *pDstCost)
{
    ...
    armRetArgErrIf(pSrcRefBufList == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(pSrcRecBuf == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(pRefRect == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(pCurrPointPos == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(pMESpec == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(pMBInter == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(pMBIntra == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(pSrcDstMBCurr == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(pDstCost == NULL, OMX_StsBadArgErr)
    armRetArgErrIf(SrcRefStep <= 0 || SrcRefStep & 15, OMX_StsBadArgErr)
    armRetArgErrIf(SrcRecStep <= 0 || SrcRecStep & 15, OMX_StsBadArgErr)
    armRetArgErrIf(SrcCurrStep <= 0 || SrcCurrStep & 15, OMX_StsBadArgErr)
    
    armRetArgErrIf(armNot16ByteAligned(pSrcCurrBuf), OMX_StsBadArgErr)    
    armRetArgErrIf(armNot16ByteAligned(pSrcRecBuf), OMX_StsBadArgErr)

    for (i = 0; i < 16; i++)
    {
        armRetArgErrIf(pSrcRefBufList [i] != NULL &&
            armNot16ByteAligned(pSrcRefBufList [i]), OMX_StsBadArgErr)
