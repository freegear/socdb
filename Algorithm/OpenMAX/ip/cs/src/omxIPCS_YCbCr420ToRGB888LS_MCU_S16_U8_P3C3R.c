/**
 *
 *  omxIPCS_YCbCr420ToRGB888LS_MCU_S16_U8_P3C3R.c
 * 
 ...
 * 
 *
 * Description  : Colour Conversion Routine - Converts YUV420 to RGB888 in floating point.
...
{   
    ...
    armRetArgErrIf(((dstStep > (-3 * JPEG_MCU_PIX_WIDTH_420)) && (dstStep < (3 * JPEG_MCU_PIX_WIDTH_420))), OMX_StsBadArgErr);
    ...
    taking care of rounding errors. The order of storing the output is BGRBGRBGR...
    Hence the RGB pointer arguments to this function, apparently appear in reverse order.
    -------------------------------------------------------------------------------------
    */
    ...
    for(i = 0; i < convHeight/2; i++)
    {
        for(j = 0; j < convWidth; j += 6)
        {
            ...
            armIPCS_ComputeRFromYUV_JPEG(Ydata, Udata, Vdata, &pDst[j+2]);
            armIPCS_ComputeGFromYUV_JPEG(Ydata, Udata, Vdata, &pDst[j+1]);
            armIPCS_ComputeBFromYUV_JPEG(Ydata, Udata, Vdata, &pDst[j]);
            
            armIPCS_ComputeRFromYUV_JPEG(Ydata2, Udata, Vdata, &pDst[j+5]);
            armIPCS_ComputeGFromYUV_JPEG(Ydata2, Udata, Vdata, &pDst[j+4]);
            armIPCS_ComputeBFromYUV_JPEG(Ydata2, Udata, Vdata, &pDst[j+3]);
            
            armIPCS_ComputeRFromYUV_JPEG(Ydata3, Udata, Vdata, &pDst2[j+2]);
            armIPCS_ComputeGFromYUV_JPEG(Ydata3, Udata, Vdata, &pDst2[j+1]);
            armIPCS_ComputeBFromYUV_JPEG(Ydata3, Udata, Vdata, &pDst2[j]);
            
            armIPCS_ComputeRFromYUV_JPEG(Ydata4, Udata, Vdata, &pDst2[j+5]);
            armIPCS_ComputeGFromYUV_JPEG(Ydata4, Udata, Vdata, &pDst2[j+4]);
            armIPCS_ComputeBFromYUV_JPEG(Ydata4, Udata, Vdata, &pDst2[j+3]);
        }
