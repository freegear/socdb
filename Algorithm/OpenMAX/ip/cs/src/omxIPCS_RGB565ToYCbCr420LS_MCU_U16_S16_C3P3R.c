/**
 *
 *  omxIPCS_RGB565ToYCbCr420LS_MCU_U16_S16_C3P3R.c
 * 
 ...
 * 
 *
 * Description  : Colour Conversion Routine - Converts RGB565 to YUV420 in floating point.
...
{
    const OMX_U16 *pSrc2 = pSrc + (srcStep >> 1);
    ...
    armRetArgErrIf(((srcStep > (-2 * JPEG_MCU_PIX_WIDTH_420)) && (srcStep < (2 * JPEG_MCU_PIX_WIDTH_420))), OMX_StsBadArgErr);
    ...
    take care of type-conversion from intermediate float values and clipping to 
    ...
    */
    ...
    for(i = 0; i < convHeight/2; i++)
    {
        ...
        {
            armIPCS_UnpackRGB565(pSrc[j],    &Rdata,  &Gdata,  &Bdata);
            armIPCS_UnpackRGB565(pSrc[j+1],  &Rdata2, &Gdata2, &Bdata2);
            armIPCS_UnpackRGB565(pSrc2[j],   &Rdata3, &Gdata3, &Bdata3);
            armIPCS_UnpackRGB565(pSrc2[j+1], &Rdata4, &Gdata4, &Bdata4);
