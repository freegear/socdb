/**
 *
 *  omxIPCS_YCbCr420RszCscRotRGB_U8_P3C3R.c
 * 
       ...
       )
{
    ...
    Each iteration of the loop creates two rows (or columns) of interpolation output 
    pixels. This fits well as the YUV420 format works in multiples of 2x2 data blocks.
    ...
    */
    ...
    if((rotation == OMX_IP_ROTATE90L) || (rotation == OMX_IP_ROTATE90R))
    {
        pDstRGBRef1 = (OMX_U8 *)pDst;
        pDstRGBRef2 = (OMX_U8 *)pDst + bytesPerOutPix;
        dstPixStep  = dstStep;
        dstRowStep  = bytesPerOutPix;
    }
    else
    {
        pDstRGBRef1 = (OMX_U8 *)pDst;
        pDstRGBRef2 = (OMX_U8 *)pDst + dstStep;
        dstPixStep  = bytesPerOutPix;
        dstRowStep  = dstStep;
    }

    if(interpolation == OMX_IP_NEAREST)
    {
        for(i = 0; i < dstSize.height; i += 2)
        {
            yPosLuma1   = armRoundFloatToS32(i * gridRatioV);
            yPosLuma2   = armRoundFloatToS32((i + 1)  * gridRatioV);
            yPosChroma  = armRoundFloatToS32((i >> 1) * gridRatioV);
            pDstRGB1    = (OMX_U8 *)pDstRGBRef1 + (dstRowStep * i);
            pDstRGB2    = (OMX_U8 *)pDstRGBRef2 + (dstRowStep * i);
            lumaPix     = chromaPix = 0;
            ...
            for(j = 0; j < dstSize.width; j += 2)
            {
                ...
                Y00data     = pSrcY[xPosLuma1  + (yPosLuma1  * srcStepY)];
                Y01data     = pSrcY[xPosLuma2  + (yPosLuma1  * srcStepY)];
                Udata       = pSrcU[xPosChroma + (yPosChroma * srcStepU)];
                Vdata       = pSrcV[xPosChroma + (yPosChroma * srcStepV)];
                Y10data     = pSrcY[xPosLuma1  + (yPosLuma2  * srcStepY)];
                Y11data     = pSrcY[xPosLuma2  + (yPosLuma2  * srcStepY)];
                
                armIPCS_RGBConvertAndPack(Y00data, Udata, Vdata, colorConversion, (void *)pDstRGB1);
                pDstRGB1 += dstPixStep;
                armIPCS_RGBConvertAndPack(Y01data, Udata, Vdata, colorConversion, (void *)pDstRGB1);
                pDstRGB1 += dstPixStep;
                
                armIPCS_RGBConvertAndPack(Y10data, Udata, Vdata, colorConversion, (void *)pDstRGB2);
                pDstRGB2 += dstPixStep;
                armIPCS_RGBConvertAndPack(Y11data, Udata, Vdata, colorConversion, (void *)pDstRGB2);
                pDstRGB2 += dstPixStep;
            }
        }
    }
    ...
    else if(interpolation == OMX_IP_BILINEAR)
    {
        for(i = 0; i < dstSize.height; i += 2)
        {
            yPosLuma1   = (OMX_INT)(i * gridRatioV);
            yPosLuma2   = (OMX_INT)((i + 1)  * gridRatioV);
            yPosChroma  = (OMX_INT)((i >> 1) * gridRatioV);
            yOffLuma1   = (i * gridRatioV) - yPosLuma1;
            yOffLuma2   = ((i + 1)  * gridRatioV) - yPosLuma2;
            yOffChroma  = ((i >> 1) * gridRatioV) - yPosChroma;
            pDstRGB1    = (OMX_U8 *)pDstRGBRef1 + (dstRowStep * i);
            pDstRGB2    = (OMX_U8 *)pDstRGBRef2 + (dstRowStep * i);
            lumaPix     = chromaPix = 0;
            ...
            for(j = 0; j < dstSize.width; j += 2)
            {
                ...
                armIPCS_InterpPixel_Bilinear(pSrcY, srcStepY, xPos, yPosLuma1, xOff, yOffLuma1, &Y00data);
                armIPCS_InterpPixel_Bilinear(pSrcY, srcStepY, xPos, yPosLuma2, xOff, yOffLuma2, &Y10data);
                lumaPix++;
                
                xPos    = (OMX_INT)(lumaPix * gridRatioH);
                xOff    = (lumaPix * gridRatioH) - xPos;
                armIPCS_InterpPixel_Bilinear(pSrcY, srcStepY, xPos, yPosLuma1, xOff, yOffLuma1, &Y01data);
                armIPCS_InterpPixel_Bilinear(pSrcY, srcStepY, xPos, yPosLuma2, xOff, yOffLuma2, &Y11data);
                lumaPix++;
                
                xPos    = (OMX_INT)(chromaPix * gridRatioH);
                xOff    = (chromaPix * gridRatioH) - xPos;
                armIPCS_InterpPixel_Bilinear(pSrcU, srcStepU, xPos, yPosChroma, xOff, yOffChroma, &Udata);
                armIPCS_InterpPixel_Bilinear(pSrcV, srcStepV, xPos, yPosChroma, xOff, yOffChroma, &Vdata);
                chromaPix++;
