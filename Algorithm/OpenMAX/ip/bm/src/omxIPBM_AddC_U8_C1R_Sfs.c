 ...
 * 
 *
 * Description  : Image arithmetic operation - Add a constant to every
 *                pixel in the ROI of a single channel source image.
 */
...
{
    OMX_INT addWidth    = roiSize.width;
    OMX_INT addHeight   = roiSize.height;
    OMX_U16 round       = 0;
    OMX_INT i, j;
    ...
    for(i=0; i<addHeight; i++, pSrc+=srcStep, pDst+=dstStep)
    {
        for(j=0; j<addWidth; j++)
        {
