...
parameters for the ddr model.  While there are many ways to define 
the speed grade, it is currently being set in tb.do.  The following 
is an example of defining the speed grade on the command line using 
the ModelSim simulator:  

    vlog +define+sg6 mobile_ddr.v
...
8 using the ModelSim simulator.
	...
	vsim -Gpart_mem_bits=8 mobile_ddr
...
every address using the ModelSim simulator.
	...
	vlog +define+FULL_MEM mobile_ddr.v
