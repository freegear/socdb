...
//
//  Implementation for smt926a is using timer 0 as system timer.
//
BOOL OALTimerInit(
    UINT32 msecPerSysTick, UINT32 countsPerMSec, UINT32 countsMargin
) {
    // msecPerSysTick   : System-tick period
    // countsPerMSec    : Timer input clock frequency divided by 1000
    // countsMargin     : Safe time range
    ...
    // Refer to PLATFORM\COMMON/TIMER/COMMON/globals.c:OAL_TIMER_STATE g_oalTimer;
    ...
    curridlehigh = 0;   // High DWORD of the 64bit idle time
    curridlehigh = 0;   // Low DWRD of the 64bit idle time
    ...
    OUTREG32(&g_pPWMRegs->TCFG0, INREG32(&g_pPWMRegs->TCFG0) & ~0x0000FF00);    // TCFG0 : Timer config reg
    OUTREG32(&g_pPWMRegs->TCFG0, INREG32(&g_pPWMRegs->TCFG0) | (245-1) <<8);		// charlie, Timer4 scale value. Timer2,3,4 prescale value
    // Select MUX input 1/2
    OUTREG32(&g_pPWMRegs->TCFG1, INREG32(&g_pPWMRegs->TCFG1) & ~(0xF << 16));   // TCFG1 : Timer config reg
    OUTREG32(&g_pPWMRegs->TCFG1, INREG32(&g_pPWMRegs->TCFG1) | (3 << 16));	// charlie, Timer4 Division (1/16 clock)
    // Set timer register
    OUTREG32(&g_pPWMRegs->TCNTB4, g_oalTimer.countsPerSysTick); // TCNTB4 : Timer4 count buffer reg

    // Start timer in auto reload mode
    tcon = INREG32(&g_pPWMRegs->TCON) & ~(0x0F << 20);  // TCON : Timer control reg
    OUTREG32(&g_pPWMRegs->TCON, tcon | (0x2 << 20) );   // Timer4 update
    OUTREG32(&g_pPWMRegs->TCON, tcon | (0x5 << 20) );   // Timer4 auto-reload & start
...
//
    ...
    // Create SYSINTR for timer
    irq = IRQ_TIMER0;
    // OALIntrRequestSysIntr() : Allocates a new SYSINTR value for a specified IRQ list
    sysIntr = OALIntrRequestSysIntr(1, &irq, OAL_INTR_FORCE_STATIC);
    ...
    // Set prescaler 1 to 1 
    OUTREG32(&g_pPWMRegs->TPRE0, INREG32(&g_pPWMRegs->TPRE0) & ~0xFF);
    OUTREG32(&g_pPWMRegs->TPRE0, INREG32(&g_pPWMRegs->TPRE0) | (245-1));

    // Select MUX input 1/2
    //OUTREG32(&g_pPWMRegs->TCFG1, INREG32(&g_pPWMRegs->TCFG1) & ~(0xF << 16));
    //OUTREG32(&g_pPWMRegs->TCFG1, INREG32(&g_pPWMRegs->TCFG1) | (3 << 16));

    // Set timer register
    OUTREG32(&g_pPWMRegs->TDAT0, g_oalTimer.countsPerSysTick);

    // Start timer in auto reload mode
    tcon = INREG32(&g_pPWMRegs->TCON0) & ~(0xFF);
    OUTREG32(&g_pPWMRegs->TCON0, tcon | (0x8 << 3) );   // Interval mode & clearing the counter
    OUTREG32(&g_pPWMRegs->TCON0, tcon | (0x1 << 7) );   // Enable
...
#ifdef ENABLE_WATCH_DOG
    {
        extern void SMTInitWatchDogTimer (void);
        SMTInitWatchDogTimer ();
    }
#endif
...
{
#if 0   // S3C2440
    return (INREG32(&g_pPWMRegs->TCNTB4) - INREG32(&g_pPWMRegs->TCNTO4)); // CNT buf(TCNTB4) - CNT observe buf(TCNTO4)
#else   // SMT926A  Modified by DJKIM 2006/10/19
    // Temporary, use the timer0
    return (INREG32(&g_pPWMRegs->TDAT0) - INREG32(&g_pPWMRegs->TCNT0)); // CNT data reg(TDAT0) - CNT RD reg(TCNT0)
#endif
}
...
VOID OALCPUIdle()
{
    ...
    smtCLKPWR->CLKCON |=  (1 << 2); // Clock control reg(CLKCON) IDLE bit  -> idle mode
    // Wait until interrupt handler set interrupt flag
    while (g_oalLastSysIntr == SYSINTR_NOP);    // Wait until timer interrupt occurs
    smtCLKPWR->CLKCON &= ~(1 << 2); // CLKCON IDLE mode disable
    INTERRUPTS_OFF();
#else   // SMT926A  Modified by DJKIM 2006/10/19
    volatile SMT926A_CLKPWR_REG *smtCLKPWR = (SMT926A_CLKPWR_REG *)OALPAtoVA(SMT926A_BASE_REG_PA_CLOCK_POWER, FALSE);
    volatile SMT926A_INTR_REG *smtINTR = (SMT926A_INTR_REG *)OALPAtoVA(SMT926A_BASE_REG_PA_INTR, FALSE);
    ...
    smtCLKPWR->CLKCON |=  (1 << 1); // Idle mode enable
    // Wait until interrupt handler set interrupt flag
    while (g_oalLastSysIntr == SYSINTR_NOP);    // Wait until timer interrupt occurs
    smtCLKPWR->CLKCON &= ~(1 << 1); // Idle mode disable
    INTERRUPTS_OFF();
#endif
}
