...
BOOL OALIntrEnableIrqs(UINT32 count, const UINT32 *pIrqs)
{
    ...
    // unmask ISR
    mask = INREG32(g_pIntrRegs->INTMSK);
    mask &= ~(1<<irq);
    OUTREG32(&g_pIntrRegs->INTMSK, mask);
#endif
    }        
...
VOID OALIntrDisableIrqs(UINT32 count, const UINT32 *pIrqs)
{
    ...
    if(g_pIntrRegs->INTPND & (1<<irq))
        SETREG32(&g_pIntrRegs->I_ISPC, (1<<irq));   // clear INTPND if IRQ
#endif
    }
...
#else   /// SMT926A Modified by DJKIM 2006/10/20
    //mask = INREG32(&g_pPortRegs->I_ISPR);
    mask = 1<<irq;
    OUTREG32(&g_pIntrRegs->I_ISPC, mask);
    CLRREG32(&g_pIntrRegs->INTMSK, mask);   // 굳이 INTMSK의 해당 bit를 clear해 줄 필요 있나???
#endif
    }
...
ULONG OEMInterruptHandler(ULONG ra)
{
    ...
    //UINT32 irq, irq2, mask;   // For S3C2440
    UINT32 irq, mask;       // For SMT926A
...
#else   // SMT926A  Modified by DJKIM 2006/10/20
    // Get pending interrupt(s)
    irq = INREG32(&g_pIntrRegs->I_ISPR);    // IRQ Interrupt Service Pending reg

    // System timer interrupt?
    if (irq == IRQ_TIMER0_TMC) {    // Timer0 match interrupt

        // Clear the interrupt
        //OUTREG32(&g_pIntrRegs->SRCPND, 1 << IRQ_TIMER4);
        //OUTREG32(&g_pIntrRegs->INTPND, 1 << IRQ_TIMER4);
        OUTREG32(&g_pIntrRegs->I_ISPC, 1 << IRQ_TIMER0_TMC);

        // Rest is on timer interrupt handler
        sysIntr = OALTimerIntrHandler();    // Refer to PLATFORM\...\SMT926A\timer.c
	}    
    // Profiling timer interrupt?
    else if (irq == IRQ_TIMER1_TMC)
    {
        // Mask and Clear the interrupt.
        mask = 1 << irq;
        SETREG32(&g_pIntrRegs->INTMSK, mask);   // Mask the interrupt
        OUTREG32(&g_pIntrRegs->I_ISPC, mask);   // Clear the IRQ interrupt
...
#ifdef OAL_ILTIMING
        if (g_oalILT.active) {
            g_oalILT.isrTime1 = OALTimerCountsSinceSysTick();   // Start time of the interrupt service routine
            g_oalILT.savedPC = 0;   // Saved program counter at the time of the INT
            g_oalILT.interrupts++;  // Number of interrupts since the last call
        }        
#endif
