...
//  File:  intr.h
//
//  This file contains SKYWALKER board specific interrupt code. The board uses
//  GPG1 pin as interrupt for LAN91C111 ethernet chip.
//
#include <bsp.h>
...
BOOL BSPIntrInit()
{
    SMT926A_IOPORT_REG *pOalPortRegs;
    ULONG value;

    OALMSG(OAL_INTR&&OAL_FUNC, (L"+BSPIntrInit\r\n"));
    
    // Then get virtual address for IO port
    pOalPortRegs = OALPAtoVA(SMT926A_BASE_REG_PA_IOPORT, FALSE);
...
BOOL BSPIntrRequestIrqs(DEVICE_LOCATION *pDevLoc, UINT32 *pCount, UINT32 *pIrqs)
{
    ...
    // Refer to kitle.c - InitKitlEtherArgs()??? by DJKIM 2006/10/10
    switch (pDevLoc->IfcType) {
    case Internal:
        switch ((ULONG)pDevLoc->LogicalLoc) {
        case BSP_BASE_REG_PA_LAN91C111_IOBASE:
            pIrqs[0] = IRQ_EINT9;
            *pCount = 1;
            rc = TRUE;
            break;
