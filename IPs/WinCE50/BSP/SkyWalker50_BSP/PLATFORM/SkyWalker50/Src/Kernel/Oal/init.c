...
void OEMInit()
{
    ...
    dwNKDrWatsonSize = 128 * 1024;  // What is it??? by DJKIM 2006/10/19
...
#else   // SMT926A  Need to insert by DJKIM 2006/10/20
#endif
}
...
void ConfigureGPIO()
{
    ...
    smtIOP->MISCCR &= ~(7<<20); // BATT_FUNC
    smtIOP->MISCCR |= (4<<20);
    smtIOP->MISCCR &= ~(7<<8);  // CLKSEL1
    smtIOP->MISCCR |= (5<<8);
    smtIOP->MISCCR &= ~(7<<4);  // CLKSEL0
    smtIOP->MISCCR |= (5<<4);
    smtIOP->MISCCR |= (3<<0);   // DATA port pull-up  enable
#else   // SMT926A  Modified by DJKIM 2006/10/20
    smtIOP->GPIO_0 = 0xFF;  // Timer output
    smtIOP->GPIO_1 = 0xAA;  // Input TCAP
    smtIOP->GPIO_2 = 0xFF;  // Timer & PWM output & push-up output
    smtIOP->GPIO_3 = 0xAA;  // External timer clock
