...
#include <keybdpdd.h>
#include <keybdist.h>
#include "smt926kbd.hpp"
#include <smt926a.h>

// Pointer to device control registers
volatile SMT926A_IOPORT_REG *v_pIOPregs;
volatile SMT926A_SPI_REG *v_pSPIregs;
	...
	)
{
	...
	if(v_pIOPregs == NULL) {
		...
		if(!VirtualCopy((PVOID)v_pIOPregs, (PVOID)(dwIOBase), sizeof(SMT926A_IOPORT_REG), PAGE_READWRITE|PAGE_NOCACHE )) 
		{
	...
	v_pSPIregs = (volatile SMT926A_SPI_REG *)VirtualAlloc(0, sizeof(SMT926A_SPI_REG), MEM_RESERVE, PAGE_NOACCESS);
	if (v_pSPIregs == NULL) {
		DEBUGMSG(1, (TEXT("[KBD] v_pSPIregs : VirtualAlloc failed!\r\n")));
		goto error_return;
	}
	else {
		if (!VirtualCopy((PVOID)v_pSPIregs, (PVOID)(dwSSPBase), sizeof(SMT926A_SPI_REG), PAGE_READWRITE | PAGE_NOCACHE)) 
		{
