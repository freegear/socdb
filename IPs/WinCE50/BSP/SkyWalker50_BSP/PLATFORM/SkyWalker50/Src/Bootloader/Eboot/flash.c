...
{
    // This function is called by DownloadImage() (blcommon.c)

#if 0    // Inserted by DJKIM 2006/09/30
    // ¾ó¸¶Å­ flash »çÀÌÁî·Î Àâ¾Æ¾ß ÇÏ³ª??? Å« ¿ë·® ÀüÃ¼·Î ??Î?by DJKIM 2006/10/09
...
{
	if (g_ImageType & IMAGE_TYPE_STEPLDR) // stepldr.bin - When steppingstone loader for NANDbooting
	{
		dwAddr = (FILE_CACHE_START + (dwAddr - STEPLDR_RAM_IMAGE_BASE));
	    return (LPBYTE)dwAddr;
	}
    else if (g_ImageType & IMAGE_TYPE_LOADER) // eboot.bin
    {
		dwAddr = (FILE_CACHE_START + (dwAddr - EBOOT_RAM_IMAGE_BASE));
	    return (LPBYTE)dwAddr;
	}
	else if (g_ImageType & IMAGE_TYPE_RAWBIN) // raw bin.nb0
    {
...
{
#if 0 // Inserted by DJKIM 2006/10/09. Further work
