...
SD_API_STATUS CSDIOControllerBase::Initialize()
{
    ...
    vm_pIOPreg = (SMT926A_IOPORT_REG *)VirtualAlloc(0, sizeof(SMT926A_IOPORT_REG), MEM_RESERVE, PAGE_NOACCESS);
    if (vm_pIOPreg == NULL) 
    {
    ...
    if (!VirtualCopy((PVOID)vm_pIOPreg, (PVOID)(SMT926A_BASE_REG_PA_IOPORT >> 8), sizeof(SMT926A_IOPORT_REG), PAGE_PHYSICAL | PAGE_READWRITE | PAGE_NOCACHE)) {
    ...
    vm_pSDIReg = (SMT926A_SDI_REG *)VirtualAlloc(0, sizeof(SMT926A_SDI_REG), MEM_RESERVE, PAGE_NOACCESS);
    if (vm_pSDIReg == NULL) 
    {
    ...
    if (!VirtualCopy((PVOID)vm_pSDIReg, (PVOID)(SMT926A_BASE_REG_PA_SDI >> 8), sizeof(SMT926A_SDI_REG), PAGE_PHYSICAL | PAGE_READWRITE | PAGE_NOCACHE)) {
    ...
    vm_pCLKPWR = (SMT926A_CLKPWR_REG *)VirtualAlloc(0, sizeof(SMT926A_CLKPWR_REG), MEM_RESERVE, PAGE_NOACCESS);
    if (vm_pCLKPWR == NULL) 
    {
    ...
    if (!VirtualCopy((PVOID)vm_pCLKPWR, (PVOID)(SMT926A_BASE_REG_PA_CLOCK_POWER >> 8), sizeof(SMT926A_CLKPWR_REG), PAGE_PHYSICAL | PAGE_READWRITE | PAGE_NOCACHE)) {
    ...
    if( m_dwDMAChannel != 0xffffffff )
    {
        ...
        vm_pDMAreg = (SMT926A_DMA_REG *)VirtualAlloc(0, sizeof(SMT926A_DMA_REG), MEM_RESERVE, PAGE_NOACCESS);
        if (vm_pDMAreg == NULL) 
        {
        ...
        if (!VirtualCopy((PVOID)vm_pDMAreg, (PVOID)(SMT926A_BASE_REG_PA_DMA >> 8), sizeof(SMT926A_DMA_REG), PAGE_PHYSICAL | PAGE_READWRITE | PAGE_NOCACHE)) {
...
DWORD CSDIOControllerBase::SetClockRate(DWORD dwClockRate)
{
    ...
    //
    // The newest datasheet for the SMT926A states that the maximum reliable speed for MMC cards is 10Mhz.
