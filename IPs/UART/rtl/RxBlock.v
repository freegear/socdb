...
parameter UART_FIFO_WIDTH = 5;	// 32 depth FIFO(default)
parameter UART_FIFO_MAX_CNT = {(UART_FIFO_WIDTH){1'b1}};	// 31(default)
...
output       parityerror;
output       rxbusy;
output [UART_FIFO_WIDTH:0] rxfifocnt;
output       rxfifoempty;
input        frameerr_clear;
input        parityerr_clear;
input        overrunerr_clear;
...
wire [UART_FIFO_WIDTH:0] rxfifocnt;
   ...
   reg        fifowrb;
   wire [UART_FIFO_WIDTH:0]  fifocnt;
   reg [UART_FIFO_WIDTH-1:0]  fifo_wrpos;
   reg [UART_FIFO_WIDTH-1:0]  fifo_rdpos;
   reg [7:0]  rxfifo [UART_FIFO_MAX_CNT:0] ;
   reg [7:0]  rxsftreg ;
   reg        rxclk    ;
   reg        rxsftenb ;
   reg        framechkb ;
...
//   reg        dataphase   ;
   ...
   reg        overrunerror_int ;
   
   reg        rxbusy_int ;

   assign     bytereceivedb = fifowrb;
   assign     rxbusy = rxbusy_int ;
...
/*
   always @ ( posedge clk or negedge rstb )
	...
	if ( rstb == 1'b0 )
	  fifocnt <= 0 ;
	else if ( fifowrb == 1'b0 && fifordb == 1'b1 && fifocnt[UART_FIFO_WIDTH] != 1'b1)
	  fifocnt <= fifocnt + 1;
	else if ( fifowrb == 1'b1 && fifordb == 1'b0 && fifocnt != 0)
	  fifocnt <= fifocnt - 1;
     end
*/
   reg fifofull;
   always @ (posedge clk or negedge rstb )
     begin
	if (rstb == 1'b0)
	  overrunerror_int <= 1'b0 ;
	else if ( swrst == 1'b1 || overrunerr_clear == 1'b1 )
	  overrunerror_int <= 1'b0 ;
//	else if ( fifowrb == 1'b0 && fifordb == 1'b1 && fifofull == 1'b1)
	else if ( fifowrb == 1'b0 && fifofull == 1'b1)
	  overrunerror_int <= 1'b1 ;
	else
	  overrunerror_int <= overrunerror_int;
     end
   
   assign overrunerror = overrunerror_int ;
   
   wire fifoempty;
   assign fifocnt[UART_FIFO_WIDTH-1:0] = fifo_wrpos - fifo_rdpos;
   assign fifocnt[UART_FIFO_WIDTH] = fifofull;
   assign fifoempty = (fifocnt == 0)? 1'b1 : 1'b0;

   assign rxfifoempty = fifoempty;

   assign rxfifocnt = fifocnt ;
   ...
   always @ ( posedge clk or negedge rstb )
	...
	if ( rstb == 1'b0 )
	  ...
	  fifo_wrpos <= 0 ;
	else if ( fifowrb == 1'b0 && fifofull == 0)
   ...
   always @ ( posedge clk or negedge rstb )
	...
	if ( rstb == 1'b0 )
	  ...
	  fifo_rdpos <= 0;
	else if ( fifordb == 1'b0 && fifoempty == 0)
   ...
   always @ ( posedge clk or negedge rstb )
	...
	if ( rstb == 1'b0 )
	  ...
	  fifofull <= 0;
	else if(fifocnt[UART_FIFO_WIDTH-1:0] == {(UART_FIFO_WIDTH){1'b1}} && fifowrb == 1'b0 && fifordb == 1'b1)
	  fifofull <= 1;
	else if(fifofull == 1'b1 && fifowrb == 1'b1 && fifordb == 1'b0)
	  fifofull <= 0;
     end
   ...
   always @ ( posedge clk or negedge rstb )
     begin
		if ( rstb == 1'b0 )
		  for ( i = 0 ; i <= UART_FIFO_MAX_CNT ; i=i+1)
		    rxfifo[i] <= { 8{1'b0}};
		...
		begin
			if(databit == 1)
		    	...
		    	rxfifo[fifo_wrpos] <= {1'b0, rxsftreg[7:1]} ; // 1 more shift when data bit is 7bit
		end
     end
   ...
   // RX lowpass filter
	   ...
	   assign rxd_loopback = (loopbacken == 1'b1 )? txd : rxd;
	   ...
	   always@(in_dly or rxd_loopback) begin
		  sum = rxd_loopback;
   ...
   always @ (posedge clk or negedge rstb)
	...
	if ( rstb == 1'b0 )
	  frameerror_int <= 1'b0 ;
	else if ( swrst == 1'b1 || frameerr_clear == 1'b1)
	  frameerror_int <= 1'b0;
	else if ( rxclk == 1'b1 && framechkb == 1'b0 )
	  if ( rx_lpfd == 1'b0 )
	    frameerror_int <= 1'b1;
	  else
	    frameerror_int <= frameerror ;
     end

   assign frameerror = frameerror_int;
   ...
   always @ ( posedge clk or negedge rstb )
     begin
	if ( rstb == 1'b0 )
	  parityerror_int <= 1'b0 ;
	else if ( swrst == 1'b1  || parityerr_clear == 1'b1)
	  parityerror_int <= 1'b0 ;
	else if ( rxclk == 1'b1 && paritychkb == 1'b0 )
	  if ( rx_lpfd != parityint )
	    parityerror_int <= 1'b1;
	  else
	    parityerror_int <= parityerror_int;
     end // always @ ( posedge clk )
   

   assign parityerror = parityerror_int ;
   ...
   // next state
   always @ ( clk or uarten or databit or baudx16clk or rx_lpfd or rxclk or parity or
