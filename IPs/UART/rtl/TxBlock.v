...
output       txfifo_fillb;
output       txfifo_fullb;
output [UART_FIFO_WIDTH:0] txfifocnt;
...
wire [UART_FIFO_WIDTH:0] txfifocnt;
   ...
   reg [7:0] txfifo [UART_FIFO_MAX_CNT:0];
//   reg [UART_FIFO_WIDTH:0] fifocnt;
   ...
   //////////////////////////////////////////////
   // select txd out
   always @ ( txdoutsel or txclk or rstb or shiftout or  parity or even_parity or odd_parity)
     begin
	case ( txdoutsel ) /* synopsys parallel_case full_case */
   ...
   always @ ( posedge clk or negedge rstb )
	...
	else if ( txclkcntenb == 1'b1 || swrst == 1'b1 )
	  begin
	     txclkcnt <= 4'b0000;
	     txclk <= 1'b0 ;
	  end
   ...
   // next state
   always @ ( clk or txclk or rstb or uarten or databit or  fifo_fillb or cst_idle or cst_loaddata 
	...
	case(1'b1)  /* synopsys parallel_case full_case */
	  cst_idle:
	     if( uarten == 1'b1 && fifo_fillb == 1'b0 )
...
*/
   ...
   always @ (posedge clk or negedge rstb )
	...
	if ( rstb == 1'b0 )
	  ...
	  fifo_wrpos <= 0 ;
	else if ( fifowrb == 1'b0 && fifofull == 1'b0)
   ...
   always @ (posedge clk or negedge rstb )
	...
	if (rstb == 1'b0 )
	  ...
	  fifo_rdpos <= 0 ;
	else if ( fifordb == 1'b0 && fifoempty == 1'b0)
   ...
   wire   fifo_fullb;
   wire   fifo_emptyb;
   assign fifo_fullb   = (fifocnt[UART_FIFO_WIDTH] == 1'b1)? 0:1;
   assign fifo_emptyb  = ~fifoempty;
   assign fifo_fillb   = fifoempty;
   
   assign txfifocnt    = fifocnt;
