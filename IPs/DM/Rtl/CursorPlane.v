...
input  [31:0] 	PalMemWrData;
input			PalMemRead;
output [31:0]	PalMemRdData;

// Plane X/Y Cordinate
input  [ 1:0]	CPlanePixFormat;	// 0: 8bit, 1: 16bit
...
wire  nCWD00  = NxtStC[2];
wire  nCWD01  = NxtStC[3];
wire  nCWD10  = NxtStC[4];
wire  nCWD11  = NxtStC[5];
...
always @(tCIdle or tCDReq or tCWD00 or tCWD01 or tCWD10 or tCWD11 or 
  	...
  	case(1'b1)	// synopsys parallel_case
  	  	...
  	  	tCWD01  : 	if (CPlanePixFormat[1]) begin // 16bit
...
//										/2							/4
assign CPlaneXSizeRef = CPlanePixFormat[1] ? {1'b0, CPlaneXSize[CW:1]} : {2'b0, CPlaneXSize[CW:2]};
...
assign PalMemReadInt = (CPlanePixFormat==0) & (nCWD00 | nCWD01 | nCWD10 | nCWD11);
assign PalMemReadAll = PalMemReadInt | PalMemRead;
assign PalMemRdAddr  = PalMemRead ? PalMemWrAddr : PalMemReadInt ? PalMemRdAddrInt : 0;

always @(nCWD00 or nCWD01 or nCWD10 or nCWD11 or LatchCDMAData or CDMADataIn)
	case(1'b1) // synopsys parallel_case full_case
		nCWD00 : PalMemRdAddrInt = CDMADataIn[7:0];
		nCWD01 : PalMemRdAddrInt = LatchCDMAData[15:8];
		nCWD10 : PalMemRdAddrInt = LatchCDMAData[23:16];
		nCWD11 : PalMemRdAddrInt = LatchCDMAData[31:24];
	endcase

PaletteMem #(8, 32) CPaletteMem(
			.Clk 		(Clk), 
			.nRST 		(nRST), 
			.Csb  		(1'b0),
			.Web  		(~PalMemWrite), 
...
always @(negedge nRST or posedge Clk)
	if  (!nRST)	CurFIFOWrData <= 0;
		...
		if (CPlanePixFormat[1]) begin // 16bit
			if (CPlanePixFormat[0]) begin // RGB565
				case(1'b1) // synopsys parallel_case
					tCWD00 : begin
							 CurFIFOWrData[23:0] <= {LatchCDMAData[15:11], 3'b0, 
							 						 LatchCDMAData[10:5],  2'b0, 
							 						 LatchCDMAData[4:0],   3'b0};
							 case(CPlaneAlphaMode) // synpsys parallel_case
							 	2'b00   : CurFIFOWrData[31:24] <= 8'hFF;					// No Alpha
							 	default : CurFIFOWrData[31:24] <= CPlaneAlphaValue;			// Global Alpha
							 endcase
							 end
					tCWD01 : begin	
							 CurFIFOWrData[23:0] <= {LatchCDMAData[31:27], 3'b0, 
							 						 LatchCDMAData[26:21], 2'b0, 
			...
			case(1'b1) // synopsys parallel_case
				tCWD00 : begin
						 if (CPlanePixFormat[0])
						 CurFIFOWrData[23:0] <= {LatchCDMAData[7:5], 5'b0, 
						 						 LatchCDMAData[4:2], 5'b0, 
						 						 LatchCDMAData[1:0], 6'b0};
						 else CurFIFOWrData[23:0] <= PalMemRdData;
						 case(CPlaneAlphaMode) // synpsys parallel_case
						 	2'b00   : CurFIFOWrData[31:24] <= 8'hFF;					// No Alpha
						 	2'b10   : CurFIFOWrData[31:24] <= CPlaneAlphaValue;			// Global Alpha
						 	default : CurFIFOWrData[31:24] <= PalMemRdData[31:24];		// Per-Pixel Alpha
						 endcase
						 end
				tCWD01 : begin	
						 if (CPlanePixFormat[0])
...
ScFIFO32x32 #(5, 5, 32) CursorFIFO(		// Insert 2007/02/03
			.Clk			(Clk), 
			.nRST			(nRST), 
			.FIFOFlush		(FIFOClear), 
