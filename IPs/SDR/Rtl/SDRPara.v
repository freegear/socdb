...
// Bus Define
parameter AW  = 26 -1;  		// Address Width
parameter DW  = 32 -1;			// Data Width
parameter BW  = 4 -1;			// Byte Width
parameter BL  = 4 -1;   		// Burst Length
parameter ID  = 5 -1;   		// ID Width

// Write Data Que Define
parameter WQCD  = 6 -1;			// Q Count Depth
parameter WQD   = 64 -1;			// Q Depth (Command Depth * Maximum Burst(16) * 2)
parameter WQW   = (DW+1) + (BW+1) -1;	// Q Width

// Read Data Que Define
parameter RQCD  = 6 -1;			// Q Count Depth
parameter RQD   = 64 -1;			// Q Depth(Command Depth * Maximum Burst(16) * 2)
parameter RQW   = (DW+1) + (ID+1) -1;	// Q Width

// Burst Length Que Define
parameter WBLQCD  = 4 -1;
parameter WBLQD   = 16 -1;
parameter WBLQW   = 2*(BL+1) +2 -1;	// Q Width

parameter BBLQCD  = 3 -1;
parameter BBLQD   = 8 -1;

parameter RBLQCD  = 3 -1;
parameter RBLQD   = 8 -1;
...
parameter TL  = 5 -1;   		// Memory Burst Length

// Ras Que Define
parameter RasQW = (BAW+1) + (RAW+1) + (TL+1) -1; // RAS Q Width

// Cas Que Define
parameter RWW = 1;
parameter CasQW = (BAW+1) + (RAW+1) + (CAW+1) + (TL+1) + (RWW) + (ID+1) + 1 -1; // CAS Q Width

parameter CD = 4  -1;			// Q Count Depth
