...
// Write Command Que Define
parameter AWQCD  = 1 -1;
parameter AWQD   = 2 -1;
parameter AWQW   = (AW+1) + (BL+1) + (ID+1) + 2 -1;

// Read Command Que Define
parameter ARQCD  = 1 -1;
parameter ARQD   = 2 -1;
parameter ARQW   = (AW+1) + (BL+1) + (ID+1) + 2 -1;

// Write Data Que Define
parameter WQCD  = 7 -1;			// Q Count Depth
parameter WQD   = 128 -1;			// Q Depth (Command Depth * Maximum Burst(16) * 2)
//parameter WQW   = (DW+1) + (BW+1) + (ID+1) -1;	// Q Width
parameter WQW   = (DW+1) + (BW+1) -1;	// Q Width

parameter BLQCD  = 4 -1;
parameter BLQD   = 16 -1;
parameter BLQW   = 2*(BL+1) +2 -1;	// Q Width

// Read Data Que Define
parameter RQCD  = 7 -1;			// Q Count Depth
parameter RQD   = 128 -1;			// Q Depth(Command Depth * Maximum Burst(16) * 4)
parameter RQW   = (DW+1) + (ID+1) + 1 -1;	// Q Width, Last
