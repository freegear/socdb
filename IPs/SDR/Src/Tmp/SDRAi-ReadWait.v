...
parameter R_RREQ = 4'b0010;
parameter R_RSPI = 4'b0100;
parameter R_WAIT = 4'b1000;

reg [3:0]  CurStR, NxtStR;

wire  tRIdle  = CurStR[0];
wire  tRRReq  = CurStR[1];
wire  tRRSpi  = CurStR[2];
wire  tRWait  = CurStR[3];
...
assign  iAWReady = ~QueErr & AWValid & WLast & iWReady; // remove tWIdle 2006/06/16
//assign  iAWReady =  ~WQFull & ~QueErr & AWValid & WLast & iWReady;
...
//assign  iWReady = (WrMaskCnt <= (WQD-15)) & WValid & AckMem & ~tWWAck & ~tWWReq & ~WBLQFull;
...
//assign  iARReady = tRIdle & ~QueErr & ARValid & (RdMaskCnt <= (RQD-15)) & ~RBLQFull;
assign  iARReady = tRIdle & ~QueErr & ARValid & ~RQHFull & ~RBLQFull;
assign  ARReady  = iARReady;

reg  [1:0] RWaitCnt;
wire RWaitEnd = (RWaitCnt == 0);
...
always @(tRIdle or tRRReq or tRRSpi or tRWait or RWaitEnd or iARReady or ReadAck or RPageMisCon) begin
  	NxtStR = R_IDLE;
  	case(1'b1)	// synopsys parallel_case
  	  	...
  	  	tRRSpi  : 	if (ReadAck)			NxtStR = R_WAIT;
  	  	          	else        			NxtStR = R_RSPI;
 	  	                            		

		tRWait 	: 	if (RWaitEnd)			NxtStR = R_IDLE;
  	  	          	else					NxtStR = R_WAIT;
...
always @(negedge ARESETB or posedge ACLK)
  	if 		(!ARESETB) 	RWaitCnt <= 0;
  	else if (tRRReq | tRRSpi)
  						RWaitCnt <= 3;
  	else if (tRWait)   	RWaitCnt <= RWaitCnt-1;
