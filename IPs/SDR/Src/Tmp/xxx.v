...
reg [BL:0]   RdWrapAddrSt1;

wire [RQCD:0] RdWrapCnt;

wire [RQCD:0] RQIncAddr0;
reg  [RQCD:0] RQIncAddr1;
...
always@(negedge ARESETB or posedge ACLK) 
	if   	(!ARESETB) 	RQIncAddr1 <= 0;
	else if (RQRead)	RQIncAddr1 <= RQIncAddr1+1;
...
always@(negedge ARESETB or posedge ACLK) 
	if   	(!ARESETB) 	RBLCnt1 <= 0;
	else if (iRBLQRead)	RBLCnt1 <= RBLQRdData[BL:0];
	else begin
		if (RdWrapAddrSt1==0)	
						RBLCnt1 <= 0;
		else begin
			if (RQRead) RBLCnt1 <= RBLCnt1 - 1;
			else		RBLCnt1 <= RBLCnt1;
		end
	end
...
always @(negedge ARESETB or posedge ACLK)
  	if      (!ARESETB) begin
		...
		if (RBLQRdData[RBLQW:RBLQW-1] == 2) begin
			case(RBLQRdData[BL:0]) // synopsys parallel_case
				4'd1    :  	RdWrapAddrSt1 = {3'b0, RBLQRdData[BL+1]}; 		// RdWrap Burst 2
				4'd3    :  	RdWrapAddrSt1 = {2'b0, RBLQRdData[BL+2:BL+1]}; 	// RdWrap Burst 4
				4'd7    :  	RdWrapAddrSt1 = {1'b0, RBLQRdData[BL+3:BL+1]}; 	// RdWrap Burst 8
				default :  	RdWrapAddrSt1 =        RBLQRdData[BL+4:BL+1];	// RdWrap Burst 16
			endcase
		end
		else		   		RdWrapAddrSt1 = 0;								// Increment
		if (RdWrapAddrSt1!=0)RdWrapCnt1  <= RQIncAddr1 + RdWrapAddrSt1;	// Wrap Start
		else				RdWrapCnt1 	 <= RQIncAddr1;
	end
  	else if (RQRead) begin
  		if (RdWrapAddrSt1!=0 & (RdWrapAddrSt1 == RBLCnt1)) 					// Wrap Arround
  							RdWrapCnt1   <= RdWrapCnt1 - LatchedRBL;
  		else   	  			RdWrapCnt1   <= RdWrapCnt1 + 1;
  	end

assign RdWrapCnt = ~RReady & (RdWrapCnt0==RdWrapCnt1) ? RdWrapCnt0 : RdWrapCnt1;

always@(negedge ARESETB or posedge ACLK) 
	if   	(!ARESETB) 	RBurstCnt <= 0;
	else if (iRBLQRead)	RBurstCnt <= RBLQRdData[BL:0];
	else begin
		if  (RQRead)	RBurstCnt <= RBurstCnt - 1;
