...
parameter W_WDAT = 5'b00010;
parameter W_WACK = 5'b00100;
parameter W_WREQ = 5'b01000;
parameter W_WSPI = 5'b10000;

reg [4:0]  CurStW, NxtStW;

wire  tWIdle  = CurStW[0];
wire  tWWDat  = CurStW[1];
wire  tWWAck  = CurStW[2];
wire  tWWReq  = CurStW[3];
wire  tWWSpi  = CurStW[4];
...
assign  iAWReady = ~QueErr & (tWIdle | tWWSpi);
`else
assign  iAWReady =  ~WQFull & ~QueErr & (tWIdle | tWWSpi);
`endif
...
always @(tWIdle or tWWDat or tWWAck or tWWReq or tWWSpi or WDatAck or RespAck or iAWValid or WriteAck or WPageMisCon) begin
  	NxtStW = W_IDLE;
  	case(1'b1)	// synopsys parallel_case
  	  	tWIdle  : 	if (iAWValid)			NxtStW = W_WDAT;	// Write Command Ack.
  	  	          	else        			NxtStW = W_IDLE;

  	  	tWWDat  : 	if (WDatAck)			NxtStW = W_WACK;	// All WData Received
  	  	          	else        			NxtStW = W_WDAT;

  	  	tWWAck  : 	if (RespAck)            NxtStW = W_WREQ;	// Write Response to Master
  	  	          	else        			NxtStW = W_WACK;
...
// Page Miss Condition
always @(Port16Bit or AddrSiz or WriteLatAA or WriteLatTT)
	if(Port16Bit) begin
        case (AddrSiz) // synopsys parallel_case
...
always @(Port16Bit or AddrSiz or WPageMisCal)
	if(Port16Bit) begin
        case (AddrSiz) // synopsys parallel_case
...
`ifdef LIMIT
assign  iWReady = (WrMaskCnt <= ((WQD+1)/2)) & AckMem & tWWDat & ~WBLQFull;
`else
...
assign  iARReady = tRIdle & ~QueErr & (RdMaskCnt <= ((RQD+1)/2)) & ~RBLQFull;
`else
...
always @(Port16Bit or AddrSiz or ReadLatAA or ReadLatTT)
    if(Port16Bit) begin
        case (AddrSiz) // synopsys parallel_case
...
always @(Port16Bit or AddrSiz or RPageMisCal)
    if(Port16Bit) begin
        case (AddrSiz) // synopsys parallel_case
