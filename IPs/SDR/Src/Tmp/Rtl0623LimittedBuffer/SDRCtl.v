...
wire [31:0] Reg1Data = {30'b0, AddrSwapEn, SDREnStatus};

wire [31:0] Reg2Data = {SRPwDnFlag, 14'b0, PwDnFlag, PwDnCnt};

wire [31:0] Reg3Data = {1'b0, RQFull, WQFull, selfref_cnt, SDRBusy, stable_cnt, refreshno, trefresh};
...
wire [BAW:0] BA_BA = AddrSwapEn ? BA_AA[BAW+CAW+1: CAW+1] : BA_AA[AW:AW-BAW];
wire [RAW:0] BA_RA = AddrSwapEn ? BA_AA[AW:AW-RAW] 		  : BA_AA[AW-BAW-1: CAW+1];
wire [CAW:0] BA_CA =              BA_AA[CAW: 0];
//-----------------------------------------------------------------------------------
// Power Down
...
//                    if (last_ba == ras_0_ba) rs_state <= rs_idle;
                  ...
                  default :
                    if (~bf_tcasbusy) rs_state <= rs_state - 1;
//                    if (~bf_tcasbusy & ~read_bstop) rs_state <= rs_state-1; // 2006/05/04 Revised
                    else              rs_state <= rs_state;
                endcase
            end
...
begin
    if (!ARESETB) begin
        ...
        else if (rs_state == (rs_write + 2) | (rs_state == rs_idle))
            rw_bstop0 <= 1'b0;
        if (rw_stop1)
            rw_bstop1 <= 1'b1;
        else if (rs_state == (rs_write + 2) | (rs_state == rs_idle))
...
begin
    if (!ARESETB) begin
        ...
        case(tcl)
          3'b001  : do_valid <= do_valid_2;
          3'b010  : do_valid <= do_valid_3;
...
begin
    if (!ARESETB) begin
        ...
        case(tcl)
          3'b001  : do_id <= do_id_2;
          3'b010  : do_id <= do_id_3;
          default : do_id <= 0;
        endcase
    end
end
...
begin
    if (!ARESETB) begin
        ...
        case(tcl)
          3'b001  : bf_last <= bf_last_2;
          3'b010  : bf_last <= bf_last_3;
	...
	if (!PORESETB) begin
    	...
    	case(ercmd)
    	  ...
    	  begin
    	    pttn    = cmd_moderegset;
    	    tclp    = tcl + 1;
    	    SD_ADDR <= {5'b00000, tclp, 4'b0111}; // page mode
    	  end
    	  default :
    	  begin
    	    case(ecmd)
...
begin
	...
	else if (SDREnStatus & SDRBusy & do_valid & ((tcl==1 & di_request_d1) | (tcl==2 & di_request_d2))) begin
		RWCollision <= 1'b1;
