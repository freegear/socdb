...
wire RBLQFull;
wire RBLQEmpty;
wire [RBLQW:0]	RBLQWrData = {ARQRdData[ARQW:ARQW-1], ARQRdData[BL:0], ARQRdData[AW+BL+1:AW+1]};
...
//-------------------------------------------------------------
// Write Command Path
wire    iAWReady = ~WQFull & ~WBLQFull & AckQue & AWValid & WLast & iWReady;
...
always @(tWIdle or tWWAck or tWWReq or tWWSpi or BReady or iAWReady or WriteAck or WPageMisCon) begin
  	NxtStW = W_IDLE;
  	case(1'b1)	// synopsys parallel_case
  	  	tWIdle  : 	if (iAWReady)			NxtStW = W_WACK;	// Write Command Ack.
  	  	          	else        			NxtStW = W_IDLE;

  	  	tWWAck  : 	if (BReady)            	NxtStW = W_WREQ;	// All WData Received
  	  	          	else        			NxtStW = W_WACK;
...
assign  iWReady = ~WQFull & WValid & AckQue & ~WBLQFull & ~tWWAck & ~tWWReq;
...
//-------------------------------------------------------------
// Read Command Path
wire    iARReady = tRIdle & ~ARQFull & ~RQFull & AckQue;
...
always @(tRIdle or tRRReq or tRRSpi or RdReqEn or ReadAck or RPageMisCon) begin
  	NxtStR = R_IDLE;
  	case(1'b1)	// synopsys parallel_case
...
assign ReadAck    =  AckQue & ~(tWWReq | tWWSpi); // Read Mask When Write Request
