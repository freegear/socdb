...
parameter R_DIDLE = 3'b001;
parameter R_DREQ  = 3'b010;
parameter R_DRDY  = 3'b100;

reg [2:0]  CurStRD, NxtStRD;

wire  tRDIdle = CurStRD[0];
wire  tRDReq  = CurStRD[1];
wire  tRDRdy  = CurStRD[2];
...
assign  iAWReady = ~QueErr & WLast & iWReady;//iWValid; // remove tWIdle 2006/06/16
//assign  iAWReady =  ~WQFull & ~QueErr & WLast & iWReady;//iWValid; remove WValid 2006/06/29
assign  AWReady  =  iAWReady;

assign  iAWValid = iAWReady & AWValid & WLast;
...
always @(tWIdle or tWWAck or tWWReq or tWWSpi or RespAck or iAWValid or WriteAck or WPageMisCon) begin
  	NxtStW = W_IDLE;
  	case(1'b1)	// synopsys parallel_case
  	  	tWIdle  : 	if (iAWValid)			NxtStW = W_WACK;	// Write Command Ack.
  	  	          	else        			NxtStW = W_IDLE;
  	  	...
  	  	tWWSpi  : 	if (iAWValid)			NxtStW = W_WACK;
...
always @(negedge ARESETB or posedge ACLK)
  	if      (!ARESETB) begin
		...
		if (WrWrapAddrSt!=0 & ((LatchedWBL+1 - WrWrapAddrSt) == WBLCnt)) 
...
assign  iWReady = ~WQFull & AckMem & ~tWWAck & ~tWWReq & ~WBLQFull;
//assign  iWReady = (WrMaskCnt <= (WQD-15)) & AckMem & ~tWWAck & ~tWWReq & ~WBLQFull;
...
always @(negedge ARESETB or posedge ACLK)
  	...
  	else if (iAWValid) WrMaskCnt <= WrMaskCnt + AWLen + 1;
...
always @(negedge ARESETB or posedge ACLK)
  	...
  	else if (RQRead)   RdMaskCnt <= RdMaskCnt - 1;

//assign  iARReady = tRIdle & ~QueErr & (RdMaskCnt <= (RQD-15)) & ~RBLQFull;
assign  iARReady = tRIdle & ~QueErr & ~RQHFull & ~RBLQFull;
...
reg [BL:0]   RBLCnt0;
reg [RQCD:0] RdWrapCnt0;
reg [BL:0]   RdWrapAddrSt0;

reg [RQCD:0] RdWrapCnt1;
reg [RQCD:0] RdWrapCnt2;
wire [RQCD:0] RdWrapCnt;

wire [RQCD:0] RQIncAddr0;
...
always@(negedge ARESETB or posedge ACLK) 
	if   	(!ARESETB) 	RBLCnt0 <= 0;
	else if (iRBLQRead)	RBLCnt0 <= RBLQRdData[BL:0];
	else begin
		if (RdWrapAddrSt0==0)	
						RBLCnt0 <= 0;
		else begin
			if (iRQRead) RBLCnt0 <= RBLCnt0 - 1;
			else		RBLCnt0 <= RBLCnt0;
		end
	end
  	...
  	if      (!ARESETB) begin
		...
		if (RBLQRdData[RBLQW:RBLQW-1] == 2) begin
			case(RBLQRdData[BL:0]) // synopsys parallel_case
				4'd1    :  	RdWrapAddrSt0 = {3'b0, RBLQRdData[BL+1]}; 		// RdWrap Burst 2
				4'd3    :  	RdWrapAddrSt0 = {2'b0, RBLQRdData[BL+2:BL+1]}; 	// RdWrap Burst 4
				4'd7    :  	RdWrapAddrSt0 = {1'b0, RBLQRdData[BL+3:BL+1]}; 	// RdWrap Burst 8
				default :  	RdWrapAddrSt0 =        RBLQRdData[BL+4:BL+1];	// RdWrap Burst 16
			endcase
		end
		else		   		RdWrapAddrSt0 = 0;								// Increment
		if (RdWrapAddrSt0!=0)RdWrapCnt0   <= RQIncAddr0 + RdWrapAddrSt0;	// Wrap Start
		else				RdWrapCnt0 	  <= RQIncAddr0;
	end
  	else if (iRQRead) begin
  		if (RdWrapAddrSt0!=0 & (RdWrapAddrSt0 == RBLCnt0)) 					// Wrap Arround
  							RdWrapCnt0    <= RdWrapCnt0 - LatchedRBL;
  		else   	  			RdWrapCnt0    <= RdWrapCnt0 + 1;
  	end

always @(negedge ARESETB or posedge ACLK)
  	if      (!ARESETB) RdWrapCnt1 <= 0;
	else if (iRQRead)  RdWrapCnt1 <= RdWrapCnt0; 

always @(negedge ARESETB or posedge ACLK)
  	if      (!ARESETB) RdWrapCnt2 <= 0;
	else if (iRQRead)  RdWrapCnt2 <= RdWrapCnt1; 

reg DelaytRDReq;
always @(negedge ARESETB or posedge ACLK)
    if (!ARESETB)	DelaytRDReq <= 0;
	else			DelaytRDReq <= tRDReq;

wire FirstNotValid = (tRDReq & ~DelaytRDReq);

//assign RdWrapCnt = FirstNotValid | (tRDReq & RReady) ? RdWrapCnt0 : RdWrapCnt1;
assign RdWrapCnt = (tRDReq|tRDRdy) & ~RReady ? RdWrapCnt2 : FirstNotValid | ((tRDReq|tRDRdy) & RReady) ? RdWrapCnt1 : RdWrapCnt0;

wire   DecRBurstCnt = RReady & RValid;

always@(negedge ARESETB or posedge ACLK) 
	if   	(!ARESETB) 	RBurstCnt <= 0;
	else if (iRBLQRead)	RBurstCnt <= RBLQRdData[BL:0];
	else begin
		if  (DecRBurstCnt)	RBurstCnt <= RBurstCnt - 1;
		else			RBurstCnt <= RBurstCnt;
	end

wire ReadCntEnd = (RBurstCnt == 0) &  DecRBurstCnt;
wire AddrCntEnd = (RBLCnt0 == 0)   &  iRQRead;
wire ReadDatEn  = (RdLastCnt > 0)  & ~RBLQEmpty & ~RQEmpty & RReady;
...
always @(tRDIdle or tRDReq or tRDRdy or ReadDatEn or AddrCntEnd or ReadCntEnd) begin
  	NxtStRD = R_DIDLE;
  	case(1'b1)	// synopsys parallel_case
  	  	...
  	  	tRDReq  :	if (ReadCntEnd) NxtStRD = R_DIDLE;	
					else if (AddrCntEnd)	NxtStRD = R_DRDY;
  	  	          	else        	NxtStRD = R_DREQ;

  	  	tRDRdy  : 	if (ReadCntEnd)	NxtStRD = R_DIDLE;
  	  	          	else        	NxtStRD = R_DRDY;
...
assign  iRQRead = ~RQEmpty & tRDReq & RReady;
...
reg FirstNotValid;
always @(negedge ARESETB or posedge ACLK)
    if (!ARESETB)	FirstNotValid <= 0;
	else if (RReady)			FirstNotValid <= tRDReq & ~DelaytRDReq;
*/

always @(negedge ARESETB or posedge ACLK)
    if (!ARESETB)	iRValid <= 0;
	else			iRValid <= tRDReq & ~FirstNotValid;

assign RValid = iRValid & ~tRDIdle | DecRdLastCnt;// & ~FirstNotValid;
//assign  RResp  = iRQRead ? 2'b0 : 2'b11;
assign  RResp  = 2'b0;

assign  RData  = RQRdData;
assign  RId	   = RQRdData[RQW:RQW-ID];
//assign  RLast  = &RBurstCnt;
