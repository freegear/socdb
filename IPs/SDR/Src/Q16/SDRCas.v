...
reg [CasQW:0] cas_8_data;
reg [CasQW:0] cas_9_data;
reg [CasQW:0] cas_a_data;
reg [CasQW:0] cas_b_data;
reg [CasQW:0] cas_c_data;
reg [CasQW:0] cas_d_data;
reg [CasQW:0] cas_e_data;
reg [CasQW:0] cas_f_data;
...
begin
    if (!ARESETB) begin
        ...
        if (CMD == bc_scas | CMD == bc_mcas) begin
            ...
            CAS_0_BA     <= cas_1_data[BAW+RAW+CAW+TL+5:RAW+CAW+TL+5]; // 26:25
            CAS_0_RA     <= cas_1_data[RAW+CAW+TL+4:CAW+TL+4]; // 24:13
            CAS_0_CA     <= cas_1_data[CAW+TL+3: TL+3]; // 12:5
            CAS_0_TT     <= cas_1_data[TL+2: 2]; // 4:1
            ...
            cas_7_data   <= cas_8_data;
            cas_7_final  <= cas_8_final;
            cas_7_newrow <= cas_8_newrow;
            cas_8_data   <= cas_9_data;
            cas_8_final  <= cas_9_final;
            cas_8_newrow <= cas_9_newrow;
            cas_9_data   <= cas_a_data;
            cas_9_final  <= cas_a_final;
            cas_9_newrow <= cas_a_newrow;
            cas_a_data   <= cas_b_data;
            cas_a_final  <= cas_b_final;
            cas_a_newrow <= cas_b_newrow;
            cas_b_data   <= cas_c_data;
            cas_b_final  <= cas_c_final;
            cas_b_newrow <= cas_c_newrow;
            cas_c_data   <= cas_d_data;
            cas_c_final  <= cas_d_final;
            cas_c_newrow <= cas_d_newrow;
            cas_d_data   <= cas_e_data;
            cas_d_final  <= cas_e_final;
            cas_d_newrow <= cas_e_newrow;
            cas_e_data   <= cas_f_data;
            cas_e_final  <= cas_f_final;
            cas_e_newrow <= cas_f_newrow;
        ...
        if (BA_REQ) begin
            ...
            case(cas_cnt)
             		...
             		4 : begin
              		...
              		8 : begin
                        cas_8_data   <= {BA_ID, BA_BA, BA_RA, BA_CA, BA_TT, BA_RW, BA_PM};
                        cas_8_final  <= 1;
                        cas_8_newrow <= 0;
                        end
              		9 : begin
                        cas_9_data   <= {BA_ID, BA_BA, BA_RA, BA_CA, BA_TT, BA_RW, BA_PM};
                        cas_9_final  <= 1;
                        cas_9_newrow <= 0;
                        end
              		10: begin
                        cas_a_data   <= {BA_ID, BA_BA, BA_RA, BA_CA, BA_TT, BA_RW, BA_PM};
                        cas_a_final  <= 1;
                        cas_a_newrow <= 0;
                        end
              		11: begin
                        cas_b_data   <= {BA_ID, BA_BA, BA_RA, BA_CA, BA_TT, BA_RW, BA_PM};
                        cas_b_final  <= 1;
                        cas_b_newrow <= 0;
                        end
              		12: begin
                        cas_c_data   <= {BA_ID, BA_BA, BA_RA, BA_CA, BA_TT, BA_RW, BA_PM};
                        cas_c_final  <= 1;
                        cas_c_newrow <= 0;
                        end
              		13: begin
                        cas_d_data   <= {BA_ID, BA_BA, BA_RA, BA_CA, BA_TT, BA_RW, BA_PM};
                        cas_d_final  <= 1;
                        cas_d_newrow <= 0;
                        end
              		14: begin
                        cas_e_data   <= {BA_ID, BA_BA, BA_RA, BA_CA, BA_TT, BA_RW, BA_PM};
                        cas_e_final  <= 1;
                        cas_e_newrow <= 0;
                        end
              		15: begin
                        cas_f_data   <= {BA_ID, BA_BA, BA_RA, BA_CA, BA_TT, BA_RW, BA_PM};
                        cas_f_final  <= 1;
                        cas_f_newrow <= 0;
                        end
            endcase
