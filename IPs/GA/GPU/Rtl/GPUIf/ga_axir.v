...
wire BBLQHFull;
wire BBLQEmpty;
wire [8:0] BBLQWrData = {send_byte, addr_dly[2:0]};
wire [8:0] BBLQRdData;
   ...
   always@(cs or grd or BBLQFull or acs or dcs or rest_len) begin
	  case(1'b1)   // synopsys parallel_case
		 cs[IDLE] :
		   if(grd & ~BBLQFull) ns <= ST_CALC_LEN;
		   else ns <= ST_IDLE;
		 ...
		 cs[RD_DATA] :
		   ...
		   if(acs == AIDLE) begin // All end East
...
//   assign      ARID = {WID_WIDTH{1'b0}}; // no ID
   ...
   always@(posedge clk or negedge rstb) begin
	  ...
	  else if(~BBLQEmpty) dcs <= ST_DDATA;
   end // always
...
assign iBBLQRead = ~BBLQEmpty & (dcs == ST_DIDLE);
assign BBLQWrite = cs[RD_CMD];
...
*/
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) rest_addr <= 3'h0;
	  else if(iBBLQRead)	rest_addr <= BBLQRdData[2:0];
   end
...
/*   
   always@(posedge clk or negedge rstb) begin
	  ...
	  else if(dcs == ST_DDATA & RVALID) begin
		 now_byte <= next_byte;
	  end // else if
   end // always@ (posedge clk or negedge rstb)
 */
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) now_byte <= 3'h0;
	  else if(iBBLQRead)	now_byte <= BBLQRdData[8:3];
   end
