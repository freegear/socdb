...
#ifndef  __DST_WR_BUFF__
#define  __DST_WR_BUFF__
#endif
...
int    dw_used_fg = 0; // 현재 사용중인 버퍼의 번호를 저장.
uint   buff_addr[2]; // buffer에 저장된 pixel data가 저장되어 있던 memory의 주소.
uint   dst_rd_buff[2][WR_BUFF_WLEGTH]; // 8. dual buffer.  32byte * 2line
int    full_fg[2]; // 각 buffer가 full되어 있는지 나타냄.
int    sel_empty; // selected buffer is empty. 
                  // full flag와 empty flag를 이용하여 비었는지 일부만 데이터가 있는지
                  // 
int    mem_oper;    // memory write중임을 나타냄.
...
// return : write ok / buffer full
{
	...
	if(trg_baddr != buff_addr[dw_used_fg] && sel_empty == 1) { // end used buffer. store used buffer.
		// store buffer임을 나타내기만 함.
		full_fg[dw_used_fg & 1] = 1;

		// 다른 버퍼가 비어 있는지 검사.
		if(full_fg[1 & (~dw_used_fg)] == 1) { // no emptied buffer
			return WR_BUFF_FULL;
		}

		// change used buffer
		dw_used_fg = 1 & (~dw_used_fg);
		sel_empty = 0;
	} // if

	if(sel_empty == 0) buff_addr[dw_used_fg] = trg_baddr;
...
// color : pixel color
{
	...
	dst_rd_buff[dw_used_fg][wd_num] &= (~wd_mask);
	
	dst_rd_buff[dw_used_fg][wd_num] |= wd_data;
...
// return : SUCC/FAIL
{
	...
	buff_num = 1&(~dw_used_fg);

	if(full_fg[buff_num] == 1) { // dump할 data가 있을 경우 
		chk = chk_op(MEM_WRITE);
		if( mem_oper == 1) {
			if(chk == TRUE) { // End memory operation
				mem_oper = 0;
				full_fg[buff_num] = 0;
			} // if
		} // if
		else if( chk == TRUE ) { // No memory operation
			set_wr_mem(MEM_WRITE, buff_addr[buff_num], 4*WR_BUFF_WLEGTH,
					   (uchar *)dst_rd_buff[buff_num]);
...
// NOP일 경우에는 wr과 fw를 0으로 set한뒤 function 호출.
{
	...
	case ST_DM_BUFF : // dump the now buffer to memory
		full_fg[dw_used_fg] = 1;
		if(full_fg[1&(~dw_used_fg)] == 1) break;
		dw_used_fg = 1 & (~dw_used_fg);
