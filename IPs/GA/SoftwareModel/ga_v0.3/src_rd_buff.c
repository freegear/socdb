...
uint   src_rd_buff[2][4]; // dual buffer.  16byte * 2line
...
// lowb : if low vertical number, 0
{
	...
	if(split_24bpp[lowb] == 0) target_addr = src_addr;
	else target_addr = src_addr + SRC_BUFF_WLEGTH*4; // 4word = 16byte
...
//---------------------------------------------------
// source pixel이 cache에서 읽혀졌는지 확인하고,
// 그 결과를 source read buffer에 저장한다.
int get_src(void)
// return : SUCC/FAIL
{
	int    buff_sel;
	...
	// split_24bpp에 의해서 buff_addr설정을 달리함.
	if(mem_op == 0) // low(small) vertical number 
		buff_sel = small_vline;
	else buff_sel = (~small_vline) & 1;
...
// return : RUN/NOP
{
	...
	if(hit[0] < 0) { // if miss
		...
		if(chk_ld_src(ctype,hpos,0) == FAIL) { // if split pixel
			full_fg[small_vline] = 0; // clear buffer
			buff_addr[small_vline] += SRC_BUFF_WLEGTH*4; // address update
			
			// send read command for getting a next pixel
			read_src(buff_addr[small_vline], 0);
			return NOP;
		}
	} // hit
	// else
	...
	if(addr0 != addr1) {
		if(hit[1] < 0) { // if miss
			...
			if(chk_ld_src(ctype,hpos,1) == FAIL) { // if split pixel
				...
				// send read command for getting a next pixel
				read_src(buff_addr[small_vline], 1);
				return NOP;
			}
		} // hit
	} // if same  two pixel
