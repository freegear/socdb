...
uint   dst_rd_buff[4]; // dual buffer.  16byte * 2line
...
//--------------------------------------------------
// send read command
void read_src_pal(uint dst_addr)
// dst_addr : target address of source palette
{
...
//---------------------------------------------------
// source palette이 memory에서 읽혀졌는지 확인하고,
// 그 결과를 source palette read buffer에 저장한다.
int get_src_pal(void)
// return : SUCC/FAIL
{
...
// return : hit/miss
{
	...
	if(full_fg != 0 && 
	   (addr>>SRCPL_BUFF_INDEX ) == (buff_addr[i]>>SRCPL_BUFF_INDEX ))
		buff_hit = 1;
	else buff_hit = 0;
...
// return : palette
{
	...
	off_at_buff = addr & DST_BUFF_IDX_MSK; // 유효자리는 2bit --> 0x0c
	
	pixel = dst_rd_buff[off_at_buff>>2];
