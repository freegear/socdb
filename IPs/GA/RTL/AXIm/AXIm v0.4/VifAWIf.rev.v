/****************************************************
    ...
    note :
        gwdata를 받기 위해서는 항상 gwready를 1로 줘야 한다.
        gwready가 1이어야 다음 clock에서 gwdata를 받을 수 있다.
    history : 
        미완성.
          align이 맞지 않을 경우 마지막에 gwready가 뜨는 이유는?
...
//`include "VifPara.v"
  ...
  //-------------------------------------------------------
   ...
   always@(posedge clk or negedge rstb) begin
	  ...
	  else if(dcs[DPRD]) begin // change DPRE-->PDRD. 2007.1.23
		 WDATA <= gwdata;
	 	 WSTRB <= gwbe;
	  end
...
//   always@(dcs_pre_send) gwready = dcs_pre_send;
   ...
   always@(now_len or dcs_pre_send or dcs or WREADY_dly or 
	  ...
	  if (dcs[DPRE])
	  	gwready <= 1'b1;	// Insert East 20070119
