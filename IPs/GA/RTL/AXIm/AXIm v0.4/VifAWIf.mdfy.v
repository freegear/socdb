...
//`include "VifPara.v"
   ...
   always@(posedge clk or negedge rstb) begin
	  ...
	  else if(gwr == 1'b1 || (cs[IDLE] == 1'b0 && WREADY_dly == 1'b1)) begin	// gglee insert WREADY_dly 1117
		 //be_dly <= gwbe;
		 data_dly <= gwdata;
	  end // else if
   end // always
  ...
  //-------------------------------------------------------
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) send_byte <= 6'h00;  // 0 ~ 31
		  ...
		  */ 
		 if(rest_lenm1[5:2] == 4'h0) // 남은 byte가 word보다 작을때.
		   send_byte <= rest_lenm1;
		 else send_byte <= {less_wd,2'h3}+{4'h0,addr_dly[1:0]};
	  end
   end // always
   ...
   always@(posedge clk or negedge rstb) begin
	  ...
	  // Initial 
		  ...
		  */
		 now_len <= less_wd[3:0] + {3'h0,(|addr_dly[1:0])};
	  end // else if
   end // always
   ...
   always@(posedge clk or negedge rstb) begin
	  ...
	  else if((dcs[DPRD]|dcs[DSEND]) & WREADY) begin // add DPRD state. 2007.1.23
   ...
   assign WVALID = dcs[DPRD] | dcs[DSEND] | dcs[DPOST]; // add DPRD state. 2007.1.23
   assign WSTRB = (WVALID)?4'hf:4'h0;
...
//   always@(dcs_pre_send) gwready = dcs_pre_send;
   
   always@(now_len or dcs_pre_send or dcs or WREADY_dly or cs or
		   send_wd or post_t or dcs_post_d or rest_len) begin
	  if (dcs[DPRE] | cs[CALC_LEN])
	  	gwready <= 1'b1;	// Insert East 2007 0119
	  else if((|now_len[1:0]) == 1'b1 && // not single word
