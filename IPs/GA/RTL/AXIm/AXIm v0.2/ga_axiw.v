...
module   ga_axiw
  (
   ...
   wire [7:0] 			  cmp_len_4k;
   wire [2:0] 			  small_wd;
   wire [2:0] 			  less_wd; // 4k boundary와 burst length사이에 작은 값.
   ...
   always@(posedge clk or negedge rstb) begin
	  ...
	  else if(gwr == 1'b1 || cs[IDLE] == 1'b0) begin
		 be_dly <= gwbe;
		 data_dly <= gwdata;
	  end // else if
   end // always
  ...
  //-------------------------------------------------------
   ...
   assign      burst_len = (&rest_lenm1[2:0])? 
						   rest_lenm1[5:3] : rest_lenm1[5:3] + 3'h7; // word개수 추출. 0~4
   
   assign 	   cmp_len_4k = {1'h0,burst_len} + {1'b0,&addr_dly[11:6],addr_dly[5:0]};
                            // = burst_len - burst_4k
   ...
   assign      cmp_4k = ~addr_dly[11:0]; 
                   ...
                   // = ~addr_dly[11:0]
                   // 0 ~ (4k-1)
   assign      burst_4k = cmp_4k[11:3]; // 0 ~
   assign      small_wd = (cmp_len_4k[7])? burst_4k[2:0] : burst_len;
   assign      less_wd = (rest_lenm1[5:3] == 3'h0)? 3'h0 : small_wd;
   // 0 ~ 5

   assign      cmp_byte = {1'b0,rest_lenm1[4:0]} + {3'h0,addr_dly};
   
   // valid at the WR_CMD stage
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) send_byte <= 6'h00;  // 0 ~ 31
	  else if(cs[CALC_LEN]) begin
		 if(|addr_dly[2:0] == 1'b1) begin  // under word
			if(|cmp_byte[5:3] == 1'b1)
			  send_byte <= {3'h0,addr_dly[2:0]};
			else send_byte <= {3'h0,rest_lenm1[2:0]};
		 end
		 else if(rest_lenm1[5:3] == 3'h0) // 남은 byte가 word보다 작을때.
		   send_byte = rest_lenm1;
		 else send_byte <= {less_wd,3'h7};
	  end
   end // always
   
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) now_len <= 3'h0;  // 1 ~ 5
	  // Initial 
	  else if(cs[CALC_LEN]) begin
		 if(|addr_dly[2:0] == 1'b1) // under word
		   now_len <= 3'h0;
		 else 
		   now_len <= less_wd[2:0];
	  end // else if
   ...
   always@(cs or dcs or now_len or send_wd or WREADY) begin
	  case(1'b1)   // synopsys parallel_case
		...
		dcs[DPRE] :
		  //if(now_len == 3'h0) dns <= ST_DIDLE; // if single word
		  if(now_len == 3'h0) dns <= ST_DPOST;
		  else dns <= ST_DPRD;
		
		dcs[DPRD] : dns <= ST_DSEND;
		
		dcs[DSEND] :
		  if(send_wd == 3'h1 && WREADY == 1'b1) dns <= ST_DPOST;
		  else dns <= ST_DSEND;
	  ...
	  if(~rstb) begin
		 ...
		 WDATA <= gwdata;
	 	 WSTRB <= gwbe;
			  ...
			  WREADY == 1'b1) begin
		 if(WREADY_dly == 1'b0) begin // get from delaied buffer
			WDATA <= data_dly;
			WSTRB <= be_dly;
		 end
		 else begin
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) dcs_pre_d <= 1'b0;
	  else  dcs_pre_d <= dcs[DPRE];
   end // always
   

   always@(now_len or dcs or WREADY_dly or send_wd or post_t or dcs_pre_d) begin
	  if(|now_len[2:0] == 1'b1 && // not single word
		 ((dcs_pre_send & dcs[DPRD]) == 1'b1))
		gwready <= 1'b1;
	  else if(send_wd[2:1] != 2'h0 && 
				(dcs[DSEND] == 1'b1 && WREADY_dly == 1'b1))
		gwready <= 1'b1;
	  else if(~dcs_pre_d & dcs[DPOST]) gwready <= 1'b1; // if next read is existed.
	  else gwready <= 1'b0;
   end // always
   
   always@(post_t or dcs_pre_d or dcs or now_len or WREADY_dly or 
		   send_wd or rest_lenm1) begin
	  if(post_t == 1'b1 &&     //(dcs[DPRD] == 1'b1 && now_len[2:0] == 3'h0) ||
		 (dcs[DSEND] == 1'b1 && WREADY_dly == 1'b1 && send_wd == 3'h2))
		gwlast <= 1'b1;
	  else if(~dcs_pre_d & (dcs_pre_send & dcs[DPOST]) & ~(|rest_lenm1[5:3]))
		gwlast <= 1'b1;
	  else gwlast <= 1'b0;
   end // always
