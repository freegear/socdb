...
module ga_axir
  (
   ...
   always@(cs or grd or acs or dcs) begin
	  case(1'b1)   // synopsys parallel_case
		 ...
		 cs[RD_DATA] :
		   if(acs == AIDLE && dcs == ST_DIDLE) // All end
			 ns <= ST_IDLE;
		   else ns <= ST_RD_DATA;
   ...
   always@(addr_dly or cmp_len_4k or burst_4k or burst_len) begin
	  ...
	  else if(cmp_len_4k[7]) now_len <= burst_4k[2:0];
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) send_byte <= 6'h00;  // 0 ~ 31
		 ...
		 if(|addr_dly[2:0] == 1'b1) send_byte <= {3'h0,~addr_dly[2:0]};
		 //else if(|rest_len[2:0] == 1'b1) // 마지막 word는 일부분만 사용.
		 else  send_byte <= {now_len,rest_lenm1[2:0]};
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) grlast <= 1'b0;
	  //else if(RVALID == 1'b1 && (|next_byte) == 1'b0 &&
	  //	  (|rest_len) == 1'b0 && RLAST == 1'b1)
	  else if(RVALID == 1'b1 && RLAST == 1'b1)
		grlast <= 1'b1;
	  else grlast <= 1'b0;
   end // always
