...
module   ga_axiw
  (
   ...
   always@(posedge clk or negedge rstb) begin
	  ...
	  else if((gwr == 1'b1 || cs[IDLE] == 1'b0) && WREADY_dly == 1'b1)begin
		 be_dly <= gwbe;
		 data_dly <= gwdata;
	  end // else if
   end // always
  ...
  //-------------------------------------------------------
   ...
   assign      cmp_byte = {1'b0,rest_lenm1[4:0]} + {3'h0,addr_dly[2:0]};
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) send_byte <= 6'h00;  // 0 ~ 31
		 ...
		 if(|addr_dly[2:0] == 1'b1) begin  // under word
			if(|cmp_byte[5:3] == 1'b1) // if next word is existed
   ...
   always@(now_len or dcs_pre_send or dcs or WREADY_dly or 
		   send_wd or post_t or dcs_post_d or rest_len) begin
	  if(|now_len[2:0] == 1'b1 && // not single word
		...
		gwready <= 1'b1; // if next read is existed.
	  else gwready <= 1'b0;
   end // always
