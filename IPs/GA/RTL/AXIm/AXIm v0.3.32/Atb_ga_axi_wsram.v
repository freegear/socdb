...
module Atb_ga_axi;
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) wsize = 0;
		 ...
		 if(waddr[1:0] == 0) begin
			if(|wsize[5:2] == 1'b0)
			  wsize = 0;
			else wsize = wsize - 6'h04;
		 end
		 else begin
			if(wsize <= {4'h0,~waddr[1:0]})
			  wsize = 0;
			else wsize = wsize - {4'h0,~waddr[1:0]} - 1;
		 end
	  end // if (gwready)
   end  // always
   ...
   initial begin
	  ...
	  read_cmd (32'h00000300,5'd31); // 32 byte
	  $fdisplay(logfile,"");
	  
	  write_cmd(32'h00000323,5'd31); // 32 byte
	  read_cmd (32'h00000323,5'd31); // 32 byte
	  $fdisplay(logfile,"");
	  
	  write_cmd(32'h00000320,5'd30); // 31 byte
	  read_cmd (32'h00000320,5'd30); // 31 byte
	  $fdisplay(logfile,"");
	  ...
	  read_cmd (32'h00000ff0,5'd20); // 21 byte
	  $fdisplay(logfile,"");
	  
	  write_cmd(32'h00000ff8,5'd20); // 21 byte
	  read_cmd (32'h00000ff8,5'd20); // 21 byte
	  $fdisplay(logfile,"");
   end // initial
