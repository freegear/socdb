...
-- release: 2.0
-- parameters based on M25P20 datasheet rev 1.6
-----------------------------------------------------
-- Unit    : Top hierarchy
-----------------------------------------------------
...
GENERIC (	init_file: string := string'("initM25P20.txt");		-- init file name
		SIZE : positive := 1048576*2;				-- 2Mbit
		Plength : positive := 256;				-- page length
		SSIZE : positive := 262144*2;				-- Sector size 
		NB_BPi: positive := 2;					-- number of BPi bits
		...
		tHLCH: TIME := 10 ns;					-- NotHold active setup time
		tCHHH: TIME := 10 ns;					-- NotHold not active hold time
		tHHCH: TIME := 10 ns;					-- NotHold not active setup time
		tCHHL: TIME := 10 ns;					-- NotHold active hold time
		tHHQX: TIME := 15 ns;					-- NotHold high to Output Low-Z
		tHLQZ: TIME := 20 ns;					-- NotHold low to Output High-Z
	        tWHSL: TIME := 20 ns;           -- Write protect setup time (SRWD=1)
	        tSHWL: TIME := 100 ns;           -- Write protect hold time (SRWD=1)
		tDP: TIME := 3 us;					-- notS high to deep power down mode
		tRES1: TIME := 3 us;					-- notS high to Stand-By power mode w-o ID Read
		tRES2: TIME := 1.8 us;					-- notS high to Stand-By power mode with ID Read
		tW: TIME :=  15 ms;					-- write status register cycle time
		tPP: TIME :=  5 ms;					-- page program cycle time
		tSE: TIME :=  3 sec;					-- sector erase cycle time
		tBE: TIME :=  6 sec;					-- bulk erase cycle time
		tVSL: TIME := 10 us;					-- Vcc(min) to /S low
		tPUW: TIME := 10 ms;					-- Time delay to write instruction
		Vwi: REAL := 2.0 ;					-- Write inhibit voltage (unit: V)
...
---------------------------------------------------------------------
ARCHITECTURE structure OF M25P20 IS
...
GENERIC MAP(	init_file=> init_file,
 ...
 PORT MAP(	adresse,	b_en,s_en,add_pp_en,pp_en,r_en,d_req,page_prog,dtr);
