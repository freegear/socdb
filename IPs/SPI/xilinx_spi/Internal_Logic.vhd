...
-----------------------------------------------------------------------------
 ...
 SIGNAL wren, wrdi, rdsr,  read_data,fast_read, pp, se, be, dp, res: boolean:=false;
 --SIGNAL wrsr: boolean:=false; -HC 11/03/03
 ...
 SIGNAL inhib_wren,inhib_wrdi,inhib_rdsr,inhib_WRSR,inhib_READ,inhib_PP,
 		inhib_SE,inhib_BE,inhib_DP,inhib_RES:boolean:=false;
...
WAIT ON Power_up,byte_ok,wr_cycle,WEL,reset_WEL,WIP,
	...
	inhib_BE,inhib_rdsr,inhib_DP,inhib_RES;
...
IF ((byte_cpt=5 OR (byte_cpt=4 AND cpt=7)) AND PP AND (NOT only_rdsr) AND (NOT only_res)) THEN
	...
	IF (select_ok'event AND (NOT select_ok) AND PP) THEN
		REPORT "Page program cycle is started"
		severity NOTE;
		WIP<='1';
		WAIT for tPP;
...
-----------------------------------------------------------------------
 ...
 ELSIF (((byte_cpt=4 AND cpt=7 AND byte_ok) OR (byte_cpt>4)) 
	AND RES AND (NOT only_rdsr)AND (NOT select_ok)) THEN
