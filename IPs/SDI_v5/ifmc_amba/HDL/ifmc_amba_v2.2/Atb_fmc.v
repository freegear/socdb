...
module     Atb_fmc;
   ...
   initial     begin
	  ...
	  clk = 1;
	  rstb = 1; // huinse FPGA board 에서는 high active.
	  tstart = 0;
...
//	  ahb_readyin = 1;
   ...
   always@(posedge Top.Top.fmc_erase) begin
	  if(Top.Top.fmc_ifren == 1'b0)
		$fdisplay(fmc_logfile,"Flash Memory Page Erase --> page : 0x%X",
				  Top.Top.fmc_xadr[9:2]);
	  else
		$fdisplay(fmc_logfile,"Flash Memory Chip Erase ");
   end // always


   always@(posedge Top.Top.fmc_prog) begin
	  if(Top.Top.fmc_ifren == 1'b0)
		$fdisplay(fmc_logfile,"Flash Memory Progam --> addr : 0x%X, data :0x%X",
				  {Top.Top.fmc_xadr,Top.Top.fmc_yadr,2'h0},Top.Top.fmc_din);
	  else
		$fdisplay(fmc_logfile,"Flash Information Progam --> addr : 0x%X, data :0x%X",
				  {Top.Top.fmc_xadr[1:0],Top.Top.fmc_yadr,2'h0},Top.Top.fmc_din);
   end
