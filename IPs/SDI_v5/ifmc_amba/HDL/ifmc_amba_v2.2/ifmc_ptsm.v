...
//		fmw_xe <= 1'b1;
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb)  wait_nvstr <= 1'b0;
		...
		wait_nvstr <= 1'b1;
	  else if((cs[STOP]|cs[IDLE]) & sda_lpf) wait_nvstr <= 1'b0;
	  //else if(cs[ADDR_DUMMY]) wait_nvstr <= 1'b0;
	  //else if(cs[IDLE] & (|cnt1[2]) ) wait_nvstr <= 1'b0;  // cnt1이 1E이면 100us 따라서 20h까지 count
   end // always
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) cnt0 <= {9{1'b0}};
	  else if(cnt_en == 1'b0)  cnt0 <= {9{1'b0}};
	  else cnt0 <= cnt0 + {cnt0[8],8'h01};
   end

   always@(posedge clk or negedge rstb) begin
	  if(~rstb) cnt1 <= {2{1'b0}};
	  else if(cnt_en == 1'b0)  cnt1 <= {2{1'b0}};
	  else cnt1 <= cnt1 + {1'b0,cnt0[8]};
   end
	*/
   //------------------------------------------------------
