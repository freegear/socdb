/******************************************************************
 ...
 hitory :
    2006.3.2 : external control signal을 active low로 변경.
...
module        mem_ctrl
  ( 
   ...
   wire [ 3:0] 	   ext_csb;  // to port
   wire [1:0] 	   ext_beb;
   wire [1:0] 	   ext_wbeb;  //  8bit write enable
   wire 		   ext_web;   // 16bit write enable
   wire 		   ext_oeb;
   wire [15:0] 	   ext_wdata;
...
//--------------------------------------------------------------------------------
   ...
   always@(taddr or tsize) begin
	  case (tsize)  //synopsys parallel_case
		3'h0 : str_addr = {6'h00,taddr};  // 8bit
		3'h1 : str_addr = {6'h00,taddr[19:1],1'b0};  //16bit
		default : str_addr = {6'h00,taddr[19:2],2'b00};  //32bit
	  endcase // case(tsize)
   end // always
   
   assign 	  ext_csb = (sram_csx == 1'b0)? ~bsel : 4'hf;
   assign 	  ext_beb = sram_bex[1:0];
   assign 	  ext_wbeb = (sram_wex) ? 2'b11 : sram_bex[1:0];
   assign 	  ext_web = sram_wex;
   assign     ext_oeb = sram_rdx;
