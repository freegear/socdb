...
--   V1.1     D.Randjelovic 06 Feb 27   Implemented new LongTimming option
...
ENTITY S29GL128M_R2 IS
    GENERIC (
        ...
        tdevice_WBPB        : VitalDelayType    := 7500 ns;
            --Program Operation
        tdevice_POB         : VitalDelayType    := 30 us;
        ...
        mem_file_name       : STRING    := "none";--"s29gl128m_r2.mem";
        prot_file_name      : STRING    := "none";--"s29gl128m_r2_prot.mem";
        secsi_file_name     : STRING    := "none";--"s29gl128m_r2_secsi.mem";
    ...
    ATTRIBUTE VITAL_LEVEL0 of S29GL128M_R2 : ENTITY IS TRUE;
END S29GL128M_R2;

-------------------------------------------------------------------------------
-- ARCHITECTURE DECLARATION
-------------------------------------------------------------------------------
ARCHITECTURE vhdl_behavioral of S29GL128M_R2 IS
    ATTRIBUTE VITAL_LEVEL0 of vhdl_behavioral : ARCHITECTURE IS TRUE;

    CONSTANT PartID        : STRING  := "S29GL128M_R2";
    CONSTANT MaxData       : NATURAL := 16#FF#;
    CONSTANT SecSize       : NATURAL := 16#FFFF#;
...
BEGIN
    ...
    POB      :VitalBuf(PO_out,  PO_in,       (tdevice_POB     ,UnitDelay));
    ...
    WireDelay : BLOCK
    BEGIN
        w_0  : VitalWireDelay (A22_ipd, A22, tipd_A22);
        ...
        w_23 : VitalWireDelay (DQ15_ipd, DQ15, tipd_DQ15);
        w_24 : VitalWireDelay (DQ14_ipd, DQ14, tipd_DQ14);
        w_25 : VitalWireDelay (DQ13_ipd, DQ13, tipd_DQ13);
        w_26 : VitalWireDelay (DQ12_ipd, DQ12, tipd_DQ12);
        w_27 : VitalWireDelay (DQ11_ipd, DQ11, tipd_DQ11);
        w_28 : VitalWireDelay (DQ10_ipd, DQ10, tipd_DQ10);
        w_29 : VitalWireDelay (DQ9_ipd, DQ9, tipd_DQ9);
        w_30 : VitalWireDelay (DQ8_ipd, DQ8, tipd_DQ8);
        w_31 : VitalWireDelay (DQ7_ipd, DQ7, tipd_DQ7);
        w_32 : VitalWireDelay (DQ6_ipd, DQ6, tipd_DQ6);
        w_33 : VitalWireDelay (DQ5_ipd, DQ5, tipd_DQ5);
        w_34 : VitalWireDelay (DQ4_ipd, DQ4, tipd_DQ4);
        w_35 : VitalWireDelay (DQ3_ipd, DQ3, tipd_DQ3);
        w_36 : VitalWireDelay (DQ2_ipd, DQ2, tipd_DQ2);
        w_37 : VitalWireDelay (DQ1_ipd, DQ1, tipd_DQ1);
        w_38 : VitalWireDelay (DQ0_ipd, DQ0, tipd_DQ0);
        w_39 : VitalWireDelay (OENeg_ipd, OENeg, tipd_OENeg);
        w_40 : VitalWireDelay (WENeg_ipd, WENeg, tipd_WENeg);
        w_41 : VitalWireDelay (RESETNeg_ipd, RESETNeg, tipd_RESETNeg);
        w_42 : VitalWireDelay (WPNeg_ipd, WPNeg, tipd_WPNeg);
    ...
    IF (TimingChecksOn) THEN
        ...
        VitalPeriodPulseCheck (
            TestSignal        => CENeg,
            TestSignalName    => "CE#",
            PulseWidthHigh    => tpw_CENeg_posedge,
        ...
        IF reseted = '1' THEN
            ...
            IF rising_edge(write) OR
               ...
               BYTENeg'EVENT) AND WENeg = '1' AND CENeg = '0' AND
                ...
                Address <= A_tmp1;
                WPage   <= A_tmp1 / 32;
                IF (RPage  /= (A_tmp1 / 8)) OR (CENeg /= CE) THEN
                    RPchange <= true;
                ELSE
                    RPchange <= false;
                END IF;
...
END PROCESS BusCycleDecode;
    ...
    ---------------------------------------------------------------------------
    ProgTime : PROCESS(PSTART, PSUSP, PRES, BYTENeg, ESP_ACT, OTP_ACT,
                      reseted)
    ...
    BEGIN
        ...
        ELSIF reseted = '1' THEN
            IF rising_edge(PSTART) AND PDONE = '1' THEN
                IF NOT(Sec_Prot(SA) = '1' OR
                       (Ers_queue(SA) = '1' AND ESP_ACT = '1') OR
                       (FactoryProt = '1' AND OTP_ACT = '1')) THEN
...
END PROCESS;
    ...
    StateGen :PROCESS(write, Addr, D_tmp0, ULBYPASS, PDONE, EDONE, HANG,
                      sCTMOUT, sSTART_T1, reseted, sREADY, PERR, EERR)
...
WP_CTRL: PROCESS(WPNeg)
     VARIABLE Sec_Prot_reg  : std_logic := '0';

BEGIN
    --Hardware Write Protection
    IF falling_edge(WPNeg) THEN
        Sec_Prot_reg := Sec_Prot(ProtSecNum);
        Sec_Prot(ProtSecNum) := '1';
    ELSIF rising_edge(WPNeg) THEN
...
END PROCESS WP_CTRL;
    ...
    Functional : PROCESS(write, read, Addr, D_tmp0, D_tmp1, Address, SecAddr,
        ...
        --WriteBuffer
        TYPE WBDataType IS ARRAY ( 0 TO 31) OF
                          INTEGER RANGE -1 TO MaxData;
        TYPE WBAddrType IS ARRAY ( 0 TO 31) OF
    ...
    BEGIN
        ...
        IF falling_edge(write) THEN
            ...
            PATTERN_1 := (Addr=16#555#) AND (DataLo=16#AA#) ;
            PATTERN_2 := (Addr=16#2AA#) AND (DataLo=16#55#) ;
            A_PAT_1   := ((Addr=16#555#) AND (ULBYPASS='0')) OR (ULBYPASS='1');
        END IF;
        ...
        CASE current_state IS
            ...
            WHEN AS             =>
                ...
                ELSIF oe THEN
                    ...
                    ELSIF Addr = 3 THEN
                        DOut_zd(7 downto 0) <= to_slv(8,8);

                        IF FactoryProt = '1' THEN
                            DOut_zd(7) <= '1';
                        END IF;
                    ELSIF Addr = 16#E# THEN
                        DOut_zd(7 downto 0) <= to_slv(16#12#,8);
            ...
            WHEN PGMS           =>
                ...
                IF PERR /= '1' THEN
                    ...
                    FOR i IN wr_cnt downto 0 LOOP
                        ...
                        IF WBAddr(i) < 0 THEN
                            old_int := -1;
                        ELSIF OTP_ACT /= '1' THEN --mem write
                    ...
                    IF HANG /= '1' AND PDONE = '1' AND (NOT PERR'EVENT) THEN
                        FOR i IN wr_cnt downto 0 LOOP
                            ...
                            ELSE
                                ASSERT false
                                    REPORT "WriteBuffer Address error"
                                    SEVERITY warning;
                            END IF;
                            WBData(i) := -1;
                        END LOOP;
...
-------------------------------------------------------------------------------
        IF NOW = 0 ns  THEN
-------------------------------------------------------------------------------
-----s29gl128m_r2 sector protection preload file format------- ----------------
-------------------------------------------------------------------------------
--   /       - comment
...
-------------------------------------------------------------------------------
-----s29gl128m_r2 SecSi preload file format------ -----------------------------
-------------------------------------------------------------------------------
--   /       - comment
...
-------------------------------------------------------------------------------
-----s29gl128m_r2 memory preload file format ----------------------------------
-------------------------------------------------------------------------------
--   /       - comment
--   @aaaaaa   - <aaaaaa> stands for address within sector
...
-------------------------------------------------------------------------------
        ...
        -----------------------------------------------------------------------
            ...
            CFI_array(16#4F#) := 16#04#; --uniform sectors bottom protect
