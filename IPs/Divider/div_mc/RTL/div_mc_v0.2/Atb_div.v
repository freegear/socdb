...
module Atb_div;
   ...
   assign #2 	dividend    = {8'h00,romdata[1*4+0:4*4+3]};
   assign #2 	divisor     = {8'h00,romdata[5*4+0:8*4+3]};
   assign #2 	q_i         = {8'h00,romdata[9*4+0:12*4+3]};
   assign #2 	r_i         = {8'h00,romdata[13*4+0:16*4+3]};
   ...
   always@(posedge clk) begin
      if(rstb) begin
		 ...
		 if(abs_diff_q >= 24'h00004)
		   $fdisplay(result_log,"Err : Q %h",abs_diff_q);

		 if(abs_diff_r >= 24'h00004)
