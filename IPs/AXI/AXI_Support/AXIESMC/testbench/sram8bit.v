 ...
 *+   Martin Mueller    0.3      6-Sep-94   Changed to 0.1nS time scale
      ...
      disble data setup time. 2006.2.26
  *+ ***************************************************************************/

/*******************************************************************************
 * Module Name: sram8bit
 * Description: 32Kx8 15nS Asynchronous Static RAM
 *
 *******************************************************************************/
// `ifdef functional
    // use unit-delay and distributed delays;
    // do NOT use path delays
//   `timescale 1ns / 1ns
//   `delay_mode_distributed
//   `delay_mode_unit
// `else
    // use ONLY path delays
//    `timescale 1ns / 10ps
//    `delay_mode_path
// `endif
...
//`timescale 10ps/10ps
`timescale 1ns/10ps
module sram8bit(data, addr, we_n, oe_n, cs_n);
inout [7:0] data;
//input [14:0] addr;
input [17:0] addr;
input we_n, oe_n, cs_n;

//Read Cycle Parameters
parameter Taa  = 15; // address access time
parameter Tacs = 15; // cs_n     access time
parameter Tclz =  4; // cs_n to output low Z time
parameter Tchz =  7; // cs_n to output high Z time
parameter Toe  =  7; // oe_n to output  time
parameter Tohz =  6; // oe_n to output Z time
parameter Toh  =  4; // data hold from adr change time

//Write Cycle Parameters
parameter Taw  = 10; // adr valid to end of write time
parameter Tcw  = 10; // cs_n to end of write time
...
parameter Tclz =  2; // cs_n to output low Z time
parameter Tchz =  5; // cs_n to output high Z time
parameter Toe  =  5; // oe_n to output  time
parameter Tohz =  4; // oe_n to output Z time
parameter Toh  =  2; // data hold from adr change time

//Write Cycle Parameters
parameter Taw  =  7; // adr valid to end of write time
parameter Tcw  =  7; // cs_n to end of write time
parameter Tas  =  0; // address set up time
parameter Twp  =  7; // write pulse width min
parameter Tdw  =  5; // data valid to end of writ time
parameter Tow  =  2; // data act from end of writ time
parameter Twhz =  4; // we_n to output in high Z time
*/
   ...
   if (we_n==0)
    begin
     ...
     if (cs_rise - cs_fall < Tcw)
	   ...
	   /*  
		// disable the check of data setup time . 2006.2.26
