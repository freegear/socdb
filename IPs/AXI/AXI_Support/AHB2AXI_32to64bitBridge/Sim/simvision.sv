# SimVision Command Script (Tue Jul 10 15:26:26 KST 2007)
...
if {[group find -match exact -name "Write"] == {}} {
    group new -name "Write" -overlay 0
} else {
    group using "Write"
    group set -overlay 0
    group clear 0 end
...
group insert \
    {shm::tb.Bridge.AWADDR[31:0]} \
    {shm::tb.Bridge.AWSIZE[2:0]} \
    {shm::tb.Bridge.AWLEN[3:0]} \
    {shm::tb.Bridge.AWVALID} \
    {shm::tb.Bridge.WDATA[63:0]} \
    {shm::tb.Bridge.WSTRB[7:0]} \
    {shm::tb.Bridge.WREADY} \
    {shm::tb.Bridge.WVALID} \
    {shm::tb.Bridge.WLAST} \
    {shm::tb.Bridge.BRESP[1:0]} \
    {shm::tb.Bridge.BVALID} \
    {shm::tb.Bridge.BREADY}

if {[group find -match exact -name "Read"] == {}} {
    group new -name "Read" -overlay 0
} else {
    group using "Read"
    group set -overlay 0
    group clear 0 end
...
group insert \
    {shm::tb.Bridge.ARADDR[31:0]} \
    {shm::tb.Bridge.ARLEN[3:0]} \
    {shm::tb.Bridge.ARSIZE[2:0]} \
    {shm::tb.Bridge.ARVALID} \
    {shm::tb.Bridge.ARREADY} \
    {shm::tb.Bridge.RDATA[63:0]} \
    {shm::tb.Bridge.RVALID} \
    {shm::tb.Bridge.RRESP[1:0]} \
    {shm::tb.Bridge.RLAST}
...
if {[window find -match exact -name "Design Browser 1"] == {}} {
    window new DesignBrowser -name "Design Browser 1" -geometry 700x500+304+176
} else {
    window geometry "Design Browser 1" 700x500+304+176
}
...
if {[window find -match exact -name "Waveform 1"] == {}} {
    window new WaveWindow -name "Waveform 1" -geometry 1181x857+46+70
} else {
    window geometry "Waveform 1" 1181x857+46+70
}
...
waveform set \
    ...
    -units ns \
    -valuewidth 75
cursor set -using "TimeA" -time 1,600,341,000ps
waveform baseline set -time 0

set id [waveform add -signals {shm::tb.Bridge.ACLK}]
...
set groupId [waveform add -groups {"Read"}]
foreach id [waveform hierarchy contents $groupId] attrs {
    {-trace digital -color #00ff00 -symbol {}}
    {-trace digital -color #00ff00 -symbol {}}
    {-trace digital -color #00ff00 -symbol {}}
...
waveform xview limits 1186436.544ns 2014416.32ns
