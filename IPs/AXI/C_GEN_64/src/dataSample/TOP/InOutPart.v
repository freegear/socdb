...
//INOUT_STATE06_START
    ...
    // For slave ?? :: ?NAME?
    //__________________________________________________
    //Write address channel for merge slave arbiter
    //2007-6-18
    wire   [WR_SLAVEID??_WID-1:0]AWID_2_?NAME?_merge;    
    wire   [ADDR_WID-1:0]     AWADDR_2_?NAME?_merge;
    wire   [AWLEN_WID-1:0]    AWLEN_2_?NAME?_merge;
    wire   [AWSIZE_WID-1:0]   AWSIZE_2_?NAME?_merge;  
    wire   [AWBURST_WID-1:0]  AWBURST_2_?NAME?_merge; 
//ENABLE_LOCK
    wire   [AWLOCK_WID-1:0]   AWLOCK_2_?NAME?_merge;  
//ENABLE_CACHE
    wire   [AWCACHE_WID-1:0]  AWCACHE_2_?NAME?_merge; 
//ENABLE_PROT
    wire   [AWPROT_WID-1:0]   AWPROT_2_?NAME?_merge;  

    //wire   AWVALID_2_?NAME?; 
    //wire   AWREADY_?NAME?; 
    wire   AWVALID_2_?NAME?_merge; 
    wire   AWREADY_?NAME?_merge; 
    
    //Write data channel
    wire   [WR_SLAVEID??_WID-1:0]WID_2_?NAME?_merge;     
    wire   [BUS_WID-1:0]      WDATA_2_?NAME?_merge;   
//ENABLE_WSTRB
    wire   [WSTRB_WID-1:0]    WSTRB_2_?NAME?_merge;   
    //wire   WLAST_2_?NAME?;   
    //wire   WVALID_2_?NAME?;  
    //wire   WREADY_?NAME?; 
    wire   WLAST_2_?NAME?_merge;   
    wire   WVALID_2_?NAME?_merge;  
    wire   WREADY_?NAME?_merge; 

    //Write response channel
    wire   [WR_SLAVEID??_WID-1:0] BID_?NAME?_merge;     
    wire   [BRESP_WID-1:0]     BRESP_?NAME?_merge;   
    wire   BVALID_?NAME?_merge;  
    wire   BREADY_2_?NAME?_merge;  
//STATE_END
...
//INOUT_STATE08_START
    ...
    // Read address channel
    wire   [RD_SLAVEID??_WID-1:0]ARID_2_?NAME?_merge;    
    wire   [ADDR_WID-1:0]     ARADDR_2_?NAME?_merge;
    wire   [ARLEN_WID-1:0]    ARLEN_2_?NAME?_merge;
    wire   [ARSIZE_WID-1:0]   ARSIZE_2_?NAME?_merge;  
    wire   [ARBURST_WID-1:0]  ARBURST_2_?NAME?_merge; 
//ENABLE_LOCK
    wire   [ARLOCK_WID-1:0]   ARLOCK_2_?NAME?_merge;  
//ENABLE_CACHE
    wire   [ARCACHE_WID-1:0]  ARCACHE_2_?NAME?_merge; 
//ENABLE_PROT
    wire   [ARPROT_WID-1:0]   ARPROT_2_?NAME?_merge;  

    wire    ARVALID_2_?NAME?_merge; 
    wire    ARREADY_?NAME?_merge; 
    

    //Read data channel
    wire   [RD_SLAVEID??_WID-1:0] RID_?NAME?_merge;     
    wire   [RRESP_WID-1:0]     RRESP_?NAME?_merge;   
    wire   [BUS_WID-1:0]       RDATA_?NAME?_merge;  
    wire   RLAST_?NAME?_merge;  
    wire   RVALID_?NAME?_merge;  
    wire   RREADY_2_?NAME?_merge;  
...
//INOUT_STATE09_START
//______________________________________________________DEFALUT_SLAVE_START_wire
    // default slave
    // For slave ?? :: ?NAME?
    //__________________________________________________
    //Write address channel
