...
//STATE00_START
`define SLAVE??_S0    ?MAXADDR?
`define SLAVE??_E0    ?MINADDR?
//STATE_END

//STATE00_START
`define SLAVE??_S1    ?MAXADDR?
`define SLAVE??_E1    ?MINADDR?
//STATE_END

//STATE00_START
module ?NAME?_TestMaster 
//STATE_END
(
...
function [63:0] SlaveData;
	...
	reg [31:0] data32;
	begin
		data32 = {addr[TADDR_WIDTH-1:3], 3'b000};
		SlaveData = {~data32, data32};
	end
endfunction
...
begin
	...
	begin
		awrite($random, SlaveAddr(i*8), 16-1, `SIZE_DWORD, `BURST_INCR);
		for(j = 0; j < 16; j = j+1)
		begin
			if(j != 15)
...
function [31:0] WrapAddr;
	...
	begin
		...
		ttaddr = (addr[6:0]&(~ext_len[6:0])) | ((addr[6:0] + (i<<size[1:0])) & ext_len[6:0]);
		if(wrap)
			WrapAddr = {addr[TADDR_WIDTH-1:7], ttaddr};
		else
...
function [7:0] WstrbGen;
	...
	begin
		case(size)
		3'b000: WstrbGen = 8'b00000001 << addr[2:0];
		3'b001: WstrbGen = 8'b00000011 << (addr[2:1]*2);
		3'b010 : WstrbGen = 8'b00001111 << (addr[2]*4);
		default : WstrbGen = 8'b11111111;
		endcase
	end
endfunction
...
function [63:0] DataMaskGen;
	...
	begin
		case(size)
		3'b000: Mask = 8'b00000001 << addr[2:0];
		3'b001: Mask = 8'b00000011 << (addr[2:1]*2);
		3'b010: Mask = 8'b00001111 << (addr[2]*4);
