...
// modelsim ver6.5 error report fix : function model -2010/10/15
...
parameter REMAIN_BITS =BUS_WID - 32;
...
begin
    ...
    */
    ...
    $display("Test Starting.... MsterNumber [%h]", MasterNumber(0));
...
function [31:0] StartAddress0;
	input [SLAVE_WID-1:0] slave_number;
    begin
        case(slave_number)
//STATE02_START
            ?WID?'d??: StartAddress0 = `SLAVE??_S0;
//STATE_END
        endcase
    end
endfunction
...
function [31:0] EndAddress0;
	input [SLAVE_WID-1:0] slave_number;
    begin
        case(slave_number)
//STATE02_START
            ?WID?'d??: EndAddress0 = `SLAVE??_E0;
//STATE_END
        endcase
    end
endfunction
...
function [31:0] StartAddress1;
	input [SLAVE_WID-1:0] slave_number;
    begin
        case(slave_number)
//STATE02_START
            ?WID?'d??: StartAddress1 = `SLAVE??_S1;
//STATE_END
        endcase
    end
endfunction
...
function [31:0] EndAddress1;
	input [SLAVE_WID-1:0] slave_number;
    begin
        case(slave_number)
//STATE02_START
            ?WID?'d??: EndAddress1 = `SLAVE??_E1;
//STATE_END
        endcase
    end
endfunction
...
function [SLAVE_WID-1:0] AreaCheck0;
	input [31:0] address;
    begin

//STATE03_START
        ?else? if((address >= `SLAVE??_S0) && ( address < `SLAVE??_E0))
            AreaCheck0 = ??;
//STATE_END
        else
            AreaCheck0 = {SLAVE_WID{1'b1}};
    end
endfunction


function [SLAVE_WID-1:0] AreaCheck1;
	input [31:0] address;
    begin

//STATE03_START
        ?else? if((address >= `SLAVE??_S1) && ( address < `SLAVE??_E1))
            AreaCheck1 = ??;
//STATE_END
        else
            AreaCheck1 = {SLAVE_WID{1'b1}};
    end
endfunction
...
function [31:0] SlaveAreaAddr;
    input [19:0] haddr;
	input [31:0] addr;
	SlaveAreaAddr = {{REMAIN_BITS{1'b0}}, haddr[19:0], MasterNumber(0), addr[TADDR_AWIDTH-1:0]};
endfunction

function [DATA_WIDTH-1:0] SlaveAreaData;
	input [19:0] haddr;
	input [31:0] addr;
	SlaveAreaData = {{REMAIN_BITS{1'b0}}, haddr[19:0], MasterNumber(0), addr[TADDR_AWIDTH-1:0]};
endfunction
...
begin
	...
	begin
        ...
        begin
            ...
            if(SelMAP1 == 1'b0) begin
                starti = StartAddress0(k) >> 12;
                endi   = EndAddress0(k) >> 12;
            end
            else begin
                starti = StartAddress1(k) >> 12;
                endi   = EndAddress1(k) >> 12;
            end

            for(i = starti; endi > i; i=i+1)
            begin
                for(l = 0 ; 16'hFF >l ; l=l+incVal)
                begin
                    delay(700);

                    if(`RANDOM_WID == 1) begin
                        if(BUS_WID == 32)
			                awrite($random, SlaveAreaAddr(i,l), 16-1, `SIZE_WORD, `BURST_INCR);
                        else
			                awrite($random, SlaveAreaAddr(i,l), 16-1, `SIZE_DWORD, `BURST_INCR);
                    end
                    else begin
                        if(BUS_WID == 32)
			                awrite(0, SlaveAreaAddr(i,l), 16-1, `SIZE_WORD, `BURST_INCR);
                        else
			                awrite(0, SlaveAreaAddr(i,l), 16-1, `SIZE_DWORD, `BURST_INCR);
                    end

			        for(j = 0; j < 16; j = j+1)
			        begin
                        if(BUS_WID == 32)   incJ = j * 4;
                        else                incJ = j * 8;

				        if(j != 15)
					        wdata(SlaveAreaData(i, ((incJ)+l)) , {NUM_BYTE{1'b1}}, 0);
				        else//Last input
					        wdata(SlaveAreaData(i, ((incJ)+l)), {NUM_BYTE{1'b1}}, 1);
			        end
                end
            end
        end
...
task random_area_read_write;
	...
	begin
        ...
        if(BUS_WID == 32)   incAddMUL = 4;
        else                incAddMUL = 8;

        hslave_addr = $random;
        slave_byen = $random;
        slave_addr[3:0] = slave_byen * incAddMUL; // 0 4 8 C..

        if(SelMAP1 == 1'b0) begin
            while(AreaCheck0(SlaveAreaAddr(hslave_addr,slave_addr)) 
                        == {SLAVE_WID{1'b1}}) 
            begin
                hslave_addr = $random;
		        slave_addr = $random;
            ...
            while(AreaCheck1(SlaveAreaAddr(hslave_addr,slave_addr)) 
                        == {SLAVE_WID{1'b1}}) 
            begin
                hslave_addr = $random;
		        slave_addr = $random;
        ...
        ov_check = slave_addr[7:0]+(burst_len*incAddMUL);
        ...
        if(SelMAP1 == 1'b0) begin
            slave_number = AreaCheck0(SlaveAreaAddr(hslave_addr,slave_addr));
        end
        else begin
            slave_number = AreaCheck1(SlaveAreaAddr(hslave_addr,slave_addr));
        end
        ...
        delay(800);
	    $display($time, "  Random Address [%h] /BurstNum [%h]/ Master[%h] \n",
                  SlaveAreaAddr(hslave_addr, slave_addr), burst_len, MasterNumber(0));
		...
		begin
            if(BUS_WID == 32)
			    ...
			    aread(rid, SlaveAreaAddr(hslave_addr, slave_addr), burst_len, `SIZE_DWORD, `BURST_INCR);
			...
			while(burst_len != 0)
			begin
				rdata(rid, SlaveAreaData(hslave_addr, (slave_addr+(i * incAddMUL))), {DATA_WIDTH{1'b1}});
		...
		begin
            if(BUS_WID == 32)
			    ...
			    awrite(wid, SlaveAreaAddr(hslave_addr, slave_addr), burst_len, `SIZE_DWORD, `BURST_INCR);

			wdata(SlaveAreaData(hslave_addr, slave_addr), {NUM_BYTE{1'b1}}, (burst_len==0));
			i = 1;
			while(burst_len != 0)
			begin
				wdata(SlaveAreaData(hslave_addr, (slave_addr+(i*incAddMUL))), {NUM_BYTE{1'b1}}, (burst_len == 1));
				burst_len = burst_len - 1;
				i = i + 1;
			end
		end
