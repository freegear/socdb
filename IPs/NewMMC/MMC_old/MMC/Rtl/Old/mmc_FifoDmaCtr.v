...
assign	TFEmpty	= FIFOEmpty & TxActiveSync;
...
always @(EnDMA or DatMode or DMASize or DatCnt or DREQ)
begin
	NextDREQ = DREQ;
	if ((EnDMA == 1'b1)&&(DatMode == 2'b11)) // Transmit DMA
		begin
		if ( ((FIFOSize) - DatCnt) >= DMASize ) // 쓸 수 있는 용량이 남아 있으면
			NextDREQ = 1'b1;
		else
			NextDREQ = 1'b0;
		end
		...
		begin
		if (DatCnt >= DMASize )// 1회 전송크기 이상 있으면
			NextDREQ = 1'b1;
		else
			NextDREQ = 1'b0;
		end
