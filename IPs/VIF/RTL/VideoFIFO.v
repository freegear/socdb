...
reg		[AW:0]		DiffRDel;
reg					Full, Empty, EmptyN0, EmptyN1, EmptyN2;
wire				EmptyN;
reg		[1:0]		Level;
reg					ReadEnDelay;
...
always @(negedge nRST or posedge RdClk)
	if (!nRST) 	DiffRDel <= 0;
	else		DiffRDel <= Diff_s;

always @(negedge nRST or posedge RdClk)
	if (!nRST) 	EmptyN0 <= 0;
	else		EmptyN0 <= (DiffRDel < N) | ((DiffRDel==N) & (ReadEn | ReadEnDelay));

always @(negedge nRST or posedge RdClk)
	if (!nRST) 	EmptyN1 <= 0;
	else		EmptyN1 <= EmptyN0;

always @(negedge nRST or posedge RdClk)
	if (!nRST) 	EmptyN2 <= 0;
	else		EmptyN2 <= EmptyN1;

assign EmptyN = EmptyN0 | EmptyN1 | EmptyN2;
