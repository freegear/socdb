...
reg			FIFOUnderRunPCLK = FIFOUnderRun;
`else // If PCLK = 2 x ACLK
reg			FrameStartPCLK;
reg			FrameEndPCLK;
reg			FIFOUnderRunPCLK;
...
wire VifErrInt   = VifErrIntEn  & (FIFOOverRunPCLK | FIFOUnderRunPCLK);
...
always @(negedge PRESETB or posedge PCLK)
	if 		(!PRESETB) 			VifOErrSts <= 1'b0;
	else if (ClearErrSts)		VifOErrSts <= 1'b0;
	else if (FIFOOverRunPCLK & VifErrIntEn) 	
								VifOErrSts <= 1'b1;
	else						VifOErrSts <= VifOErrSts;

always @(negedge PRESETB or posedge PCLK)
	if 		(!PRESETB) 			VifUErrSts <= 1'b0;
	else if (ClearErrSts)		VifUErrSts <= 1'b0;
	else if (FIFOUnderRunPCLK & VifErrIntEn) 	
								VifUErrSts <= 1'b1;
	else						VifUErrSts <= VifUErrSts;
//------------------------------------------------------------------------------
// Field/VBlank/HBlank
reg		FieldPCLK0;
reg		FieldPCLK1;
...
always @(negedge PRESETB or posedge PCLK)
	if 	(!PRESETB)		  PRDATA <= 32'b0;
		...
		case(1'b1) // synopsys parallel_case
    	  	...
    	  	rVIFSTSRd   : PRDATA <= {28'b0, VifUErrSts, VifOErrSts, FrameStartSts, FrameEndSts};
