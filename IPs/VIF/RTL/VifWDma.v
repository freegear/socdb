...
assign WriteAck = (WriteAck0 | WriteAck1)  & DmaEnable & ~DmaEnd;
assign DataAck  =  ~DmaBusy;
assign WDatEnd  = ((BurstCnt == 0) | XDmaReqEnd) & DmaRdy; // invert DmaRdy. gtlee
assign IncCnt   =  tWWDat & DmaRdy;

assign FIFORead = ~FIFOEmpty & DmaRdy;

assign FIFOUnderRun0 = FIFOEmpty & DmaRdy;
always @(negedge nRST or posedge Clk)	// Latch 1 more clock
begin
	if (!nRST) begin
			FIFOUnderRun1  <= 0;
	end
	else begin
			FIFOUnderRun1  <= FIFOUnderRun0;
	end
end

assign FIFOUnderRun = FIFOUnderRun0 | FIFOUnderRun1;
...
begin
	if (!nRST) begin
			...
			WriteAck0  <= ~FIFOHalfFull;
			WriteAck1  <= ReMained & HWrEndSysClk & ~FIFOEmpty;
	end
end
//-------------------------------------------------------
// Meta FF.
