...
--      Pipeline:       No
--
--      Creation Date:  2006-12-19 03:48:06Z
--      Version:        2003Q4V1
--
...
Package RF1SH16x24_pkgs is
  component rdwr_RF1SH16x24
    generic(
	...
	tipd_A: VitalDelayArrayType01(3 downto 0):=(others=>(0.000 ns, 0.000 ns));
	tipd_D: VitalDelayArrayType01(23 downto 0):=(others=>(0.000 ns, 0.000 ns));
	...
	tsetup_A_CLK_posedge_posedge: VitalDelayArrayType(3 downto 0):=(others=>(1.000 ns));
	tsetup_A_CLK_negedge_posedge: VitalDelayArrayType(3 downto 0):=(others=>(1.000 ns));
	tsetup_D_CLK_posedge_posedge: VitalDelayArrayType(23 downto 0):=(others=>(1.000 ns));
	tsetup_D_CLK_negedge_posedge: VitalDelayArrayType(23 downto 0):=(others=>(1.000 ns));
	...
	thold_A_CLK_posedge_posedge: VitalDelayArrayType(3 downto 0):=(others=>(0.500 ns));
	thold_A_CLK_negedge_posedge: VitalDelayArrayType(3 downto 0):=(others=>(0.500 ns));
	thold_D_CLK_posedge_posedge: VitalDelayArrayType(23 downto 0):=(others=>(0.500 ns));
	thold_D_CLK_negedge_posedge: VitalDelayArrayType(23 downto 0):=(others=>(0.500 ns));
	tpd_CLK_Q	: VitalDelayArrayType01(23 downto 0) := (others => (1.0 ns, 1.0 ns));
	PortName	: string
    );
    port ( 
	CLK: in std_logic;
...
use WORK.RF1SH16x24_pkgs.all;

  entity rdwr_RF1SH16x24 is
    generic(
	TimingChecksOn: BOOLEAN := TRUE;
	tperiod_CLK  : VitalDelayType;
	tpw_CLK_negedge: VitalDelayType;
    ...
    attribute VITAL_LEVEL1 of rdwr_RF1SH16x24 : entity is TRUE;
end rdwr_RF1SH16x24;

-------------- -------------- -------------- -------------- --------------
architecture BEHAVIORAL of rdwr_RF1SH16x24 is
  constant ClkPort	: string:=cat("CLK",PortName);
  constant InstPort	: string:="RF1SH16x24";
...
--------------
TA_A_UTI: for i in 0 to 3 generate
...
--------------
D_UTI: for i in 0 to 23 generate
...
PROCA : process(GTP_RdWr, ANVio, DVio, WENVio)
  ...
  variable AddI  : std_logic_vector (3 downto 0) ;
	----------------------------------------------------------------------
  begin
...
use WORK.RF1SH16x24_pkgs.all;
  ...
  entity RF1SH16x24 is
    ...
    attribute VITAL_LEVEL1 of RF1SH16x24 : entity is TRUE;
end RF1SH16x24;

-----------------------------------------------------------------------------
architecture Structural of RF1SH16x24 is
  signal MEM: MEM_TYPE(15 downto 0, 23 downto 0);
  signal gnd: std_logic:='0';
  signal gndA: std_logic_vector(3 downto 0):=(others=>'0');
  signal gndD: std_logic_vector(23 downto 0):=(others=>'0');
  signal Read, Write: std_logic;
  signal GtpRst, BusyWr, BusyRd: std_logic:='0';
  signal Am: std_logic_vector(3 downto 0);
  signal Dm, Qi: std_logic_vector(23 downto 0);
