...
module       ismc_ahb
  (
   ...
   always@(posedge clk or negedge rstb) begin
	  if(~rstb) rd_cmd <= 1'b0;
	  else rd_cmd <= rd_cmd_new;
   end // always

   always@(posedge clk or negedge rstb) begin
	  if(~rstb) rd_dphase <= 1'b0;
	  else if(cs == 1'b1) begin // if wait cyclie
		 rd_dphase <= rd_cmd;
	  end
   ...
   assign 	#2 sram_addr = (wr_cmd)? ahb_addr_dp[ADDR_WIDTH-1:2] : ahb_addr[ADDR_WIDTH-1:2];
