...
static uint32_t ata_calc_lba(ata_device *dev)
{
  uint32_t lba;
  ...
  if(dev->regs.device_head & ATA_DHR_LBA) {
    ...
    lba = (dev->regs.device_head & ATA_DHR_H) << 24 |
	  ...
	  (dev->regs.cylinder_low           ) <<  8 |
	   dev->regs.sector_number
    ;
  } else {
...
static void ata_read_sect(ata_device *dev)
{
...
static void ata_write_sect(ata_device *dev)
{
  if(!dev->internals.nr_sect) {
...
*/
static void ata_read_native_max_addr(ata_device *dev)
{
...
*/
static void ata_write_sectors(ata_device *dev)
{
  size_t sector_count;
  uint32_t lba;
...
*/
static void ata_set_features(ata_device *dev)
{
