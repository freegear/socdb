...
#ifdef CONFIG_NETtel
	/* This is NETtel LED flashing code. Gotta love flashing LEDs :-) */
{
...
#ifdef CONFIG_eLIA
{
	...
	if ((ticks = (ticks + 1) & 0x3f) > 0xf)
		unsetledpp(eLIA_LEDDIAG);
	else
		setledpp(eLIA_LEDDIAG);
}
#endif
}
...
#endif /* CONFIG_NETtel || CONFIG_eLIA */
...
#endif /* CONFIG_NETtel || CONFIG_eLIA || CONFIG_MATtel */
