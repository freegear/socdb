// $Header: /cvsroot/anonymous/or1k/orp/orp_soc/lib/xilinx/unisims/RAMB16_S36_S36.v,v 1.1 2002/03/28 20:15:30 lampret Exp $
...
module RAMB16_S36_S36 (DOA, DOPA, DOB, DOPB, ADDRA, CLKA, DIA, DIPA, ENA, SSRA, WEA, ADDRB, CLKB, DIB, DIPB, ENB, SSRB, WEB);
	...
	for (ci = 0; ci < 32; ci = ci + 1) begin
		...
		if ((addra_int * 32 + ci) == (addrb_int * 32 + cj)) begin
	...
	if (((wr_mode_a == 2'b01) && (wr_mode_b == 2'b01)) ||
		...
		for (dmi = 0; dmi < 32; dmi = dmi + 1) begin
			...
			if ((addra_int * 32 + dmi) == (addrb_int * 32 + dmj)) begin
			    mem[addra_int * 32 + dmi] <= 1'bX;
			end
		    end
		end
	    end
	...
	if ((wr_mode_a == 2'b01) && (wr_mode_b != 2'b01)) begin
	    if (wea_int == 1 && web_int == 1) begin
		for (dni = 0; dni < 32; dni = dni + 1) begin
		    for (dnj = 0; dnj < 32; dnj = dnj + 1) begin
			if ((addra_int * 32 + dni) == (addrb_int * 32 + dnj)) begin
			    mem[addra_int * 32 + dni] <= dia_int[dni];
			end
		    end
		end
	    end
	...
	if ((wr_mode_a != 2'b01) && (wr_mode_b == 2'b01)) begin
	    if (wea_int == 1 && web_int == 1) begin
		for (doi = 0; doi < 32; doi = doi + 1) begin
		    for (doj = 0; doj < 32; doj = doj + 1) begin
			if ((addra_int * 32 + doi) == (addrb_int * 32 + doj)) begin
	...
	if ((wr_mode_b == 2'b00) || (wr_mode_b == 2'b10)) begin
		...
		for (dai = 0; dai < 32; dai = dai + 1) begin
		    for (daj = 0; daj < 32; daj = daj + 1) begin
			if ((addra_int * 32 + dai) == (addrb_int * 32 + daj)) begin
			    doa_out[dai] <= 1'bX;
			end
		    end
		end
	...
	if ((wr_mode_a == 2'b00) || (wr_mode_a == 2'b10)) begin
		...
		for (dbi = 0; dbi < 32; dbi = dbi + 1) begin
		    for (dbj = 0; dbj < 32; dbj = dbj + 1) begin
			if ((addra_int * 32 + dbi) == (addrb_int * 32 + dbj)) begin
			    dob_out[dbj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (dci = 0; dci < 32; dci = dci + 1) begin
		    for (dcj = 0; dcj < 32; dcj = dcj + 1) begin
			if ((addra_int * 32 + dci) == (addrb_int * 32 + dcj)) begin
			    doa_out[dci] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (ddi = 0; ddi < 32; ddi = ddi + 1) begin
		    for (ddj = 0; ddj < 32; ddj = ddj + 1) begin
			if ((addra_int * 32 + ddi) == (addrb_int * 32 + ddj)) begin
			    dob_out[ddj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b01) && (wr_mode_b == 2'b01)) ||
		...
		for (pmi = 0; pmi < 4; pmi = pmi + 1) begin
			...
			if ((addra_int * 4 + pmi) == (addrb_int * 4 + pmj)) begin
			    mem[16384 + addra_int * 4 + pmi] <= 1'bX;
			end
		    end
		end
	    end
	...
	if ((wr_mode_a == 2'b01) && (wr_mode_b != 2'b01)) begin
	    if (wea_int == 1 && web_int == 1) begin
		for (pni = 0; pni < 4; pni = pni + 1) begin
		    for (pnj = 0; pnj < 4; pnj = pnj + 1) begin
			if ((addra_int * 4 + pni) == (addrb_int * 4 + pnj)) begin
			    mem[16384 + addra_int * 4 + pni] <= dipa_int[pni];
			end
		    end
		end
	    end
	...
	if ((wr_mode_a != 2'b01) && (wr_mode_b == 2'b01)) begin
	    if (wea_int == 1 && web_int == 1) begin
		for (poi = 0; poi < 4; poi = poi + 1) begin
		    for (poj = 0; poj < 4; poj = poj + 1) begin
			if ((addra_int * 4 + poi) == (addrb_int * 4 + poj)) begin
	...
	if ((wr_mode_b == 2'b00) || (wr_mode_b == 2'b10)) begin
	    if ((wea_int == 0) && (web_int == 1) && (ssra_int == 0)) begin
		for (pai = 0; pai < 4; pai = pai + 1) begin
		    for (paj = 0; paj < 4; paj = paj + 1) begin
			if ((addra_int * 4 + pai) == (addrb_int * 4 + paj)) begin
			    dopa_out[pai] <= 1'bX;
			end
		    end
		end
	...
	if ((wr_mode_a == 2'b00) || (wr_mode_a == 2'b10)) begin
	    if ((wea_int == 1) && (web_int == 0) && (ssrb_int == 0)) begin
		for (pbi = 0; pbi < 4; pbi = pbi + 1) begin
		    for (pbj = 0; pbj < 4; pbj = pbj + 1) begin
			if ((addra_int * 4 + pbi) == (addrb_int * 4 + pbj)) begin
			    dopb_out[pbj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
	    ...
	    ((wr_mode_a == 2'b01) && (wr_mode_b == 2'b00))) begin
	    if ((wea_int == 1) && (web_int == 1) && (ssra_int == 0)) begin
		for (pci = 0; pci < 4; pci = pci + 1) begin
		    for (pcj = 0; pcj < 4; pcj = pcj + 1) begin
			if ((addra_int * 4 + pci) == (addrb_int * 4 + pcj)) begin
			    dopa_out[pci] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
	    ...
	    ((wr_mode_a == 2'b00) && (wr_mode_b == 2'b01))) begin
	    if ((wea_int == 1) && (web_int == 1) && (ssrb_int == 0)) begin
		for (pdi = 0; pdi < 4; pdi = pdi + 1) begin
		    for (pdj = 0; pdj < 4; pdj = pdj + 1) begin
			if ((addra_int * 4 + pdi) == (addrb_int * 4 + pdj)) begin
			    dopb_out[pdj] <= 1'bX;
			end
		    end
		end
	...
	if (ena_int == 1'b1) begin
		...
		if (wea_int == 1'b1) begin
			...
			doa_out[16] <= dia_int[16];
			doa_out[17] <= dia_int[17];
			doa_out[18] <= dia_int[18];
			doa_out[19] <= dia_int[19];
			doa_out[20] <= dia_int[20];
			doa_out[21] <= dia_int[21];
			doa_out[22] <= dia_int[22];
			doa_out[23] <= dia_int[23];
			doa_out[24] <= dia_int[24];
			doa_out[25] <= dia_int[25];
			doa_out[26] <= dia_int[26];
			doa_out[27] <= dia_int[27];
			doa_out[28] <= dia_int[28];
			doa_out[29] <= dia_int[29];
			doa_out[30] <= dia_int[30];
			doa_out[31] <= dia_int[31];
			dopa_out[0] <= dipa_int[0];
			dopa_out[1] <= dipa_int[1];
			dopa_out[2] <= dipa_int[2];
			dopa_out[3] <= dipa_int[3];
		    end
		    else if (wr_mode_a == 2'b01) begin
			doa_out[0] <= mem[addra_int * 32 + 0];
			doa_out[1] <= mem[addra_int * 32 + 1];
			doa_out[2] <= mem[addra_int * 32 + 2];
			doa_out[3] <= mem[addra_int * 32 + 3];
			doa_out[4] <= mem[addra_int * 32 + 4];
			doa_out[5] <= mem[addra_int * 32 + 5];
			doa_out[6] <= mem[addra_int * 32 + 6];
			doa_out[7] <= mem[addra_int * 32 + 7];
			doa_out[8] <= mem[addra_int * 32 + 8];
			doa_out[9] <= mem[addra_int * 32 + 9];
			doa_out[10] <= mem[addra_int * 32 + 10];
			doa_out[11] <= mem[addra_int * 32 + 11];
			doa_out[12] <= mem[addra_int * 32 + 12];
			doa_out[13] <= mem[addra_int * 32 + 13];
			doa_out[14] <= mem[addra_int * 32 + 14];
			doa_out[15] <= mem[addra_int * 32 + 15];
			doa_out[16] <= mem[addra_int * 32 + 16];
			doa_out[17] <= mem[addra_int * 32 + 17];
			doa_out[18] <= mem[addra_int * 32 + 18];
			doa_out[19] <= mem[addra_int * 32 + 19];
			doa_out[20] <= mem[addra_int * 32 + 20];
			doa_out[21] <= mem[addra_int * 32 + 21];
			doa_out[22] <= mem[addra_int * 32 + 22];
			doa_out[23] <= mem[addra_int * 32 + 23];
			doa_out[24] <= mem[addra_int * 32 + 24];
			doa_out[25] <= mem[addra_int * 32 + 25];
			doa_out[26] <= mem[addra_int * 32 + 26];
			doa_out[27] <= mem[addra_int * 32 + 27];
			doa_out[28] <= mem[addra_int * 32 + 28];
			doa_out[29] <= mem[addra_int * 32 + 29];
			doa_out[30] <= mem[addra_int * 32 + 30];
			doa_out[31] <= mem[addra_int * 32 + 31];
			dopa_out[0] <= mem[16384 + addra_int * 4 + 0];
			dopa_out[1] <= mem[16384 + addra_int * 4 + 1];
			dopa_out[2] <= mem[16384 + addra_int * 4 + 2];
			dopa_out[3] <= mem[16384 + addra_int * 4 + 3];
		    end
		    else begin
			doa_out[0] <= doa_out[0];
			doa_out[1] <= doa_out[1];
			...
			doa_out[16] <= doa_out[16];
			doa_out[17] <= doa_out[17];
			doa_out[18] <= doa_out[18];
			doa_out[19] <= doa_out[19];
			doa_out[20] <= doa_out[20];
			doa_out[21] <= doa_out[21];
			doa_out[22] <= doa_out[22];
			doa_out[23] <= doa_out[23];
			doa_out[24] <= doa_out[24];
			doa_out[25] <= doa_out[25];
			doa_out[26] <= doa_out[26];
			doa_out[27] <= doa_out[27];
			doa_out[28] <= doa_out[28];
			doa_out[29] <= doa_out[29];
			doa_out[30] <= doa_out[30];
			doa_out[31] <= doa_out[31];
			dopa_out[0] <= dopa_out[0];
			dopa_out[1] <= dopa_out[1];
			dopa_out[2] <= dopa_out[2];
			dopa_out[3] <= dopa_out[3];
		    end
		end
		else begin
		    doa_out[0] <= mem[addra_int * 32 + 0];
	...
	if (ena_int == 1'b1 && wea_int == 1'b1) begin
	    mem[addra_int * 32 + 0] <= dia_int[0];
	    mem[addra_int * 32 + 1] <= dia_int[1];
	    mem[addra_int * 32 + 2] <= dia_int[2];
	    mem[addra_int * 32 + 3] <= dia_int[3];
	    mem[addra_int * 32 + 4] <= dia_int[4];
	    mem[addra_int * 32 + 5] <= dia_int[5];
	    mem[addra_int * 32 + 6] <= dia_int[6];
	    mem[addra_int * 32 + 7] <= dia_int[7];
	    mem[addra_int * 32 + 8] <= dia_int[8];
	    mem[addra_int * 32 + 9] <= dia_int[9];
	    mem[addra_int * 32 + 10] <= dia_int[10];
	    mem[addra_int * 32 + 11] <= dia_int[11];
	    mem[addra_int * 32 + 12] <= dia_int[12];
	    mem[addra_int * 32 + 13] <= dia_int[13];
	    mem[addra_int * 32 + 14] <= dia_int[14];
	    mem[addra_int * 32 + 15] <= dia_int[15];
	    mem[addra_int * 32 + 16] <= dia_int[16];
	    mem[addra_int * 32 + 17] <= dia_int[17];
	    mem[addra_int * 32 + 18] <= dia_int[18];
	    mem[addra_int * 32 + 19] <= dia_int[19];
	    mem[addra_int * 32 + 20] <= dia_int[20];
	    mem[addra_int * 32 + 21] <= dia_int[21];
	    mem[addra_int * 32 + 22] <= dia_int[22];
	    mem[addra_int * 32 + 23] <= dia_int[23];
	    mem[addra_int * 32 + 24] <= dia_int[24];
	    mem[addra_int * 32 + 25] <= dia_int[25];
	    mem[addra_int * 32 + 26] <= dia_int[26];
	    mem[addra_int * 32 + 27] <= dia_int[27];
	    mem[addra_int * 32 + 28] <= dia_int[28];
	    mem[addra_int * 32 + 29] <= dia_int[29];
	    mem[addra_int * 32 + 30] <= dia_int[30];
	    mem[addra_int * 32 + 31] <= dia_int[31];
	    mem[16384 + addra_int * 4 + 0] <= dipa_int[0];
	    mem[16384 + addra_int * 4 + 1] <= dipa_int[1];
	    mem[16384 + addra_int * 4 + 2] <= dipa_int[2];
	    mem[16384 + addra_int * 4 + 3] <= dipa_int[3];
	end
    end
