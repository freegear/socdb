// $Header: /cvsroot/anonymous/or1k/orp/orp_soc/lib/xilinx/unisims/RAMB16_S2_S36.v,v 1.1 2002/03/28 20:15:30 lampret Exp $
...
module RAMB16_S2_S36 (DOA, DOB, DOPB, ADDRA, CLKA, DIA, ENA, SSRA, WEA, ADDRB, CLKB, DIB, DIPB, ENB, SSRB, WEB);
	...
	for (ci = 0; ci < 2; ci = ci + 1) begin
		...
		if ((addra_int * 2 + ci) == (addrb_int * 32 + cj)) begin
	...
	if (((wr_mode_a == 2'b01) && (wr_mode_b == 2'b01)) ||
		...
		for (dmi = 0; dmi < 2; dmi = dmi + 1) begin
			...
			if ((addra_int * 2 + dmi) == (addrb_int * 32 + dmj)) begin
	...
	if ((wr_mode_a == 2'b01) && (wr_mode_b != 2'b01)) begin
		...
		for (dni = 0; dni < 2; dni = dni + 1) begin
			...
			if ((addra_int * 2 + dni) == (addrb_int * 32 + dnj)) begin
	...
	if ((wr_mode_a != 2'b01) && (wr_mode_b == 2'b01)) begin
		...
		for (doi = 0; doi < 2; doi = doi + 1) begin
			...
			if ((addra_int * 2 + doi) == (addrb_int * 32 + doj)) begin
	...
	if ((wr_mode_b == 2'b00) || (wr_mode_b == 2'b10)) begin
		...
		for (dai = 0; dai < 2; dai = dai + 1) begin
			...
			if ((addra_int * 2 + dai) == (addrb_int * 32 + daj)) begin
			    doa_out[dai] <= 1'bX;
			end
		    end
		end
	...
	if ((wr_mode_a == 2'b00) || (wr_mode_a == 2'b10)) begin
		...
		for (dbi = 0; dbi < 2; dbi = dbi + 1) begin
			...
			if ((addra_int * 2 + dbi) == (addrb_int * 32 + dbj)) begin
			    dob_out[dbj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (dci = 0; dci < 2; dci = dci + 1) begin
			...
			if ((addra_int * 2 + dci) == (addrb_int * 32 + dcj)) begin
			    doa_out[dci] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (ddi = 0; ddi < 2; ddi = ddi + 1) begin
			...
			if ((addra_int * 2 + ddi) == (addrb_int * 32 + ddj)) begin
			    dob_out[ddj] <= 1'bX;
			end
		    end
		end
