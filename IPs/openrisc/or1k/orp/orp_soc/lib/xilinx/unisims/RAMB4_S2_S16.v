// $Header: /cvsroot/anonymous/or1k/orp/orp_soc/lib/xilinx/unisims/RAMB4_S2_S16.v,v 1.1 2002/03/28 20:15:31 lampret Exp $
...
module RAMB4_S2_S16 (DOA, DOB, ADDRA, CLKA, DIA, ENA, RSTA, WEA, ADDRB, CLKB, DIB, ENB, RSTB, WEB);
	...
	if (wea_int == 1 && web_int == 1) begin
		...
		for (mj = 0; mj < 16; mj = mj + 1) begin
		    if ((addra_int * 2 + mi) == (addrb_int * 16 + mj)) begin
	...
	if (web_int == 1 && rsta_int == 0) begin
		...
		for (aj = 0; aj < 16; aj = aj + 1) begin
		    if ((addra_int * 2 + ai) == (addrb_int * 16 + aj)) begin
			doa_out[ai] <= 1'bX;
		    end
		end
	    end
	...
	if (wea_int == 1 && rstb_int == 0) begin
		...
		for (bj = 0; bj < 16; bj = bj + 1) begin
		    if ((addra_int * 2 + bi) == (addrb_int * 16 + bj)) begin
			dob_out[bj] <= 1'bX;
		    end
		end
	    end
