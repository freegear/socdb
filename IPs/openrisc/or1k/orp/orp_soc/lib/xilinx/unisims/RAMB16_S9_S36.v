// $Header: /cvsroot/anonymous/or1k/orp/orp_soc/lib/xilinx/unisims/RAMB16_S9_S36.v,v 1.1 2002/03/28 20:15:31 lampret Exp $
...
module RAMB16_S9_S36 (DOA, DOPA, DOB, DOPB, ADDRA, CLKA, DIA, DIPA, ENA, SSRA, WEA, ADDRB, CLKB, DIB, DIPB, ENB, SSRB, WEB);
	...
	for (ci = 0; ci < 8; ci = ci + 1) begin
		...
		if ((addra_int * 8 + ci) == (addrb_int * 32 + cj)) begin
	...
	if (((wr_mode_a == 2'b01) && (wr_mode_b == 2'b01)) ||
		...
		for (dmi = 0; dmi < 8; dmi = dmi + 1) begin
			...
			if ((addra_int * 8 + dmi) == (addrb_int * 32 + dmj)) begin
	...
	if ((wr_mode_a == 2'b01) && (wr_mode_b != 2'b01)) begin
		...
		for (dni = 0; dni < 8; dni = dni + 1) begin
			...
			if ((addra_int * 8 + dni) == (addrb_int * 32 + dnj)) begin
	...
	if ((wr_mode_a != 2'b01) && (wr_mode_b == 2'b01)) begin
		...
		for (doi = 0; doi < 8; doi = doi + 1) begin
			...
			if ((addra_int * 8 + doi) == (addrb_int * 32 + doj)) begin
	...
	if ((wr_mode_b == 2'b00) || (wr_mode_b == 2'b10)) begin
		...
		for (dai = 0; dai < 8; dai = dai + 1) begin
			...
			if ((addra_int * 8 + dai) == (addrb_int * 32 + daj)) begin
			    doa_out[dai] <= 1'bX;
			end
		    end
		end
	...
	if ((wr_mode_a == 2'b00) || (wr_mode_a == 2'b10)) begin
		...
		for (dbi = 0; dbi < 8; dbi = dbi + 1) begin
			...
			if ((addra_int * 8 + dbi) == (addrb_int * 32 + dbj)) begin
			    dob_out[dbj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (dci = 0; dci < 8; dci = dci + 1) begin
			...
			if ((addra_int * 8 + dci) == (addrb_int * 32 + dcj)) begin
			    doa_out[dci] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (ddi = 0; ddi < 8; ddi = ddi + 1) begin
			...
			if ((addra_int * 8 + ddi) == (addrb_int * 32 + ddj)) begin
			    dob_out[ddj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b01) && (wr_mode_b == 2'b01)) ||
		...
		for (pmi = 0; pmi < 1; pmi = pmi + 1) begin
			...
			if ((addra_int * 1 + pmi) == (addrb_int * 4 + pmj)) begin
	...
	if ((wr_mode_a == 2'b01) && (wr_mode_b != 2'b01)) begin
		...
		for (pni = 0; pni < 1; pni = pni + 1) begin
			...
			if ((addra_int * 1 + pni) == (addrb_int * 4 + pnj)) begin
	...
	if ((wr_mode_a != 2'b01) && (wr_mode_b == 2'b01)) begin
		...
		for (poi = 0; poi < 1; poi = poi + 1) begin
			...
			if ((addra_int * 1 + poi) == (addrb_int * 4 + poj)) begin
	...
	if ((wr_mode_b == 2'b00) || (wr_mode_b == 2'b10)) begin
		...
		for (pai = 0; pai < 1; pai = pai + 1) begin
			...
			if ((addra_int * 1 + pai) == (addrb_int * 4 + paj)) begin
			    dopa_out[pai] <= 1'bX;
			end
		    end
		end
	...
	if ((wr_mode_a == 2'b00) || (wr_mode_a == 2'b10)) begin
		...
		for (pbi = 0; pbi < 1; pbi = pbi + 1) begin
			...
			if ((addra_int * 1 + pbi) == (addrb_int * 4 + pbj)) begin
			    dopb_out[pbj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (pci = 0; pci < 1; pci = pci + 1) begin
			...
			if ((addra_int * 1 + pci) == (addrb_int * 4 + pcj)) begin
			    dopa_out[pci] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (pdi = 0; pdi < 1; pdi = pdi + 1) begin
			...
			if ((addra_int * 1 + pdi) == (addrb_int * 4 + pdj)) begin
			    dopb_out[pdj] <= 1'bX;
			end
		    end
		end
