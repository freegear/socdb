// $Header: /cvsroot/anonymous/or1k/orp/orp_soc/lib/xilinx/unisims/RAMB4_S1_S4.v,v 1.1 2002/03/28 20:15:31 lampret Exp $
...
module RAMB4_S1_S4 (DOA, DOB, ADDRA, CLKA, DIA, ENA, RSTA, WEA, ADDRB, CLKB, DIB, ENB, RSTB, WEB);
	...
	if (wea_int == 1 && web_int == 1) begin
		...
		for (mj = 0; mj < 4; mj = mj + 1) begin
		    if ((addra_int * 1 + mi) == (addrb_int * 4 + mj)) begin
	...
	if (web_int == 1 && rsta_int == 0) begin
		...
		for (aj = 0; aj < 4; aj = aj + 1) begin
		    if ((addra_int * 1 + ai) == (addrb_int * 4 + aj)) begin
			doa_out[ai] <= 1'bX;
		    end
		end
	    end
	...
	if (wea_int == 1 && rstb_int == 0) begin
		...
		for (bj = 0; bj < 4; bj = bj + 1) begin
		    if ((addra_int * 1 + bi) == (addrb_int * 4 + bj)) begin
			dob_out[bj] <= 1'bX;
		    end
		end
	    end
