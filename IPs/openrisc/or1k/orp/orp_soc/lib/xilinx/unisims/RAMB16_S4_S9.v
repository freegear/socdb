// $Header: /cvsroot/anonymous/or1k/orp/orp_soc/lib/xilinx/unisims/RAMB16_S4_S9.v,v 1.1 2002/03/28 20:15:31 lampret Exp $
...
module RAMB16_S4_S9 (DOA, DOB, DOPB, ADDRA, CLKA, DIA, ENA, SSRA, WEA, ADDRB, CLKB, DIB, DIPB, ENB, SSRB, WEB);
	...
	for (ci = 0; ci < 4; ci = ci + 1) begin
		...
		if ((addra_int * 4 + ci) == (addrb_int * 8 + cj)) begin
	...
	if (((wr_mode_a == 2'b01) && (wr_mode_b == 2'b01)) ||
		...
		for (dmi = 0; dmi < 4; dmi = dmi + 1) begin
			...
			if ((addra_int * 4 + dmi) == (addrb_int * 8 + dmj)) begin
	...
	if ((wr_mode_a == 2'b01) && (wr_mode_b != 2'b01)) begin
		...
		for (dni = 0; dni < 4; dni = dni + 1) begin
			...
			if ((addra_int * 4 + dni) == (addrb_int * 8 + dnj)) begin
	...
	if ((wr_mode_a != 2'b01) && (wr_mode_b == 2'b01)) begin
		...
		for (doi = 0; doi < 4; doi = doi + 1) begin
			...
			if ((addra_int * 4 + doi) == (addrb_int * 8 + doj)) begin
	...
	if ((wr_mode_b == 2'b00) || (wr_mode_b == 2'b10)) begin
		...
		for (dai = 0; dai < 4; dai = dai + 1) begin
			...
			if ((addra_int * 4 + dai) == (addrb_int * 8 + daj)) begin
			    doa_out[dai] <= 1'bX;
			end
		    end
		end
	...
	if ((wr_mode_a == 2'b00) || (wr_mode_a == 2'b10)) begin
		...
		for (dbi = 0; dbi < 4; dbi = dbi + 1) begin
			...
			if ((addra_int * 4 + dbi) == (addrb_int * 8 + dbj)) begin
			    dob_out[dbj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (dci = 0; dci < 4; dci = dci + 1) begin
			...
			if ((addra_int * 4 + dci) == (addrb_int * 8 + dcj)) begin
			    doa_out[dci] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (ddi = 0; ddi < 4; ddi = ddi + 1) begin
			...
			if ((addra_int * 4 + ddi) == (addrb_int * 8 + ddj)) begin
			    dob_out[ddj] <= 1'bX;
			end
		    end
		end
