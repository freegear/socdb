// $Header: /cvsroot/anonymous/or1k/orp/orp_soc/lib/xilinx/unisims/RAMB16_S1_S1.v,v 1.1 2002/03/28 20:15:30 lampret Exp $
...
module RAMB16_S1_S1 (DOA, DOB, ADDRA, CLKA, DIA, ENA, SSRA, WEA, ADDRB, CLKB, DIB, ENB, SSRB, WEB);
	...
	if (((wr_mode_a == 2'b01) && (wr_mode_b == 2'b01)) ||
		...
		for (dmi = 0; dmi < 1; dmi = dmi + 1) begin
		    for (dmj = 0; dmj < 1; dmj = dmj + 1) begin
			if ((addra_int * 1 + dmi) == (addrb_int * 1 + dmj)) begin
	...
	if ((wr_mode_a == 2'b01) && (wr_mode_b != 2'b01)) begin
		...
		for (dni = 0; dni < 1; dni = dni + 1) begin
		    for (dnj = 0; dnj < 1; dnj = dnj + 1) begin
			if ((addra_int * 1 + dni) == (addrb_int * 1 + dnj)) begin
	...
	if ((wr_mode_a != 2'b01) && (wr_mode_b == 2'b01)) begin
		...
		for (doi = 0; doi < 1; doi = doi + 1) begin
		    for (doj = 0; doj < 1; doj = doj + 1) begin
			if ((addra_int * 1 + doi) == (addrb_int * 1 + doj)) begin
			    mem[addrb_int * 1 + doj] <= dib_int[doj];
			end
		    end
		end
	    end
	...
	if ((wr_mode_b == 2'b00) || (wr_mode_b == 2'b10)) begin
		...
		for (dai = 0; dai < 1; dai = dai + 1) begin
		    for (daj = 0; daj < 1; daj = daj + 1) begin
			if ((addra_int * 1 + dai) == (addrb_int * 1 + daj)) begin
			    doa_out[dai] <= 1'bX;
			end
		    end
		end
	...
	if ((wr_mode_a == 2'b00) || (wr_mode_a == 2'b10)) begin
		...
		for (dbi = 0; dbi < 1; dbi = dbi + 1) begin
		    for (dbj = 0; dbj < 1; dbj = dbj + 1) begin
			if ((addra_int * 1 + dbi) == (addrb_int * 1 + dbj)) begin
			    dob_out[dbj] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (dci = 0; dci < 1; dci = dci + 1) begin
		    for (dcj = 0; dcj < 1; dcj = dcj + 1) begin
			if ((addra_int * 1 + dci) == (addrb_int * 1 + dcj)) begin
			    doa_out[dci] <= 1'bX;
			end
		    end
		end
	...
	if (((wr_mode_a == 2'b00) && (wr_mode_b == 2'b00)) ||
		...
		for (ddi = 0; ddi < 1; ddi = ddi + 1) begin
		    for (ddj = 0; ddj < 1; ddj = ddj + 1) begin
			if ((addra_int * 1 + ddi) == (addrb_int * 1 + ddj)) begin
			    dob_out[ddj] <= 1'bX;
			end
		    end
		end
