// $Header: /cvsroot/anonymous/or1k/orp/orp_soc/lib/xilinx/unisims/RAMB4_S4_S8.v,v 1.1 2002/03/28 20:15:31 lampret Exp $
...
module RAMB4_S4_S8 (DOA, DOB, ADDRA, CLKA, DIA, ENA, RSTA, WEA, ADDRB, CLKB, DIB, ENB, RSTB, WEB);
	...
	if (wea_int == 1 && web_int == 1) begin
		...
		for (mj = 0; mj < 8; mj = mj + 1) begin
		    if ((addra_int * 4 + mi) == (addrb_int * 8 + mj)) begin
	...
	if (web_int == 1 && rsta_int == 0) begin
		...
		for (aj = 0; aj < 8; aj = aj + 1) begin
		    if ((addra_int * 4 + ai) == (addrb_int * 8 + aj)) begin
			doa_out[ai] <= 1'bX;
		    end
		end
	    end
	...
	if (wea_int == 1 && rstb_int == 0) begin
		...
		for (bj = 0; bj < 8; bj = bj + 1) begin
		    if ((addra_int * 4 + bi) == (addrb_int * 8 + bj)) begin
			dob_out[bj] <= 1'bX;
		    end
		end
	    end
