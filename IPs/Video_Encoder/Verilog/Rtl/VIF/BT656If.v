...
always @(negedge nRST or posedge VClk)
  ...
  else if (SAVclear) PixCount <= 2'b0;
...
always @(negedge nRST or posedge VClk)
  if (!nRST) begin
  	...
  	if (HCount=={ImageSize{1'b1}}) HCount <= {ImageSize{1'b1}};
  	else HCount <= HCount + 1;
  	if (HEnd) HDisable <= 1'b0;
  end
...
always @(negedge nRST or posedge VClk)
  if (!nRST) begin
  	...
  	if (VCount=={ImageSize{1'b1}}) VCount <= {ImageSize{1'b1}};
  	else VCount <= VCount + 1;
  	if (VEnd) VDisable <= 1'b0;
  end
...
assign RequestX = (HCount >= HStart[ImageSize-1:1]) & 
				  (HCount < (HStart[ImageSize-1:1] + HSize[ImageSize-1:1])) & RequestY & DmaEnable;
