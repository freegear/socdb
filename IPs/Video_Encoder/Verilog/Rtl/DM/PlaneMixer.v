...
assign GPlaneRequestX = (MixXCount >= GPlaneXPos) & (MixXCount < (GPlaneXPos + GPlaneXSize)) & GPlaneRequestY & PlaneMixerEn;
assign GPlaneRequestY = (MixYCount >= GPlaneYPos) & (MixYCount < (GPlaneYPos + GPlaneYSize));

assign VPlaneRequestX = (MixXCount >= VPlaneXPos) & (MixXCount < (VPlaneXPos + VPlaneXSize)) & VPlaneRequestY & PlaneMixerEn;
assign VPlaneRequestY = (MixYCount >= VPlaneYPos) & (MixYCount < (VPlaneYPos + VPlaneYSize));

assign CPlaneRequestX = (MixXCount >= CPlaneXPos) & (MixXCount < (CPlaneXPos + CPlaneXSize)) & CPlaneRequestY & PlaneMixerEn;
...
// MixFIFOHalfFull는 Async 신호인데, 흔들릴 경우 오동작 할 수 있으므로...
// 다음과 같이 조건만 쓸 수 있도록 한다.

parameter M_IDLE = 2'b01;
parameter M_DREQ = 2'b10;

reg [1:0]  CurStM, NxtStM;

wire  tMIdle  = CurStM[0];
wire  tMDReq  = CurStM[1];

reg [2:0]	FIFOReqCnt;
wire 		FIFOReqEnd;

wire		RequestEn = ~MixFIFOHalfFull & PlaneMixerEn;

always @(negedge nRST or posedge Clk)
  	if (!nRST) 	CurStM <= M_IDLE;
  	else        CurStM <= NxtStM;

always @(tMIdle or tMDReq or RequestEn or FIFOReqEnd) begin
  	NxtStM = M_IDLE;
  	case(1'b1)	// synopsys parallel_case
  	  	tMIdle  : 	if (RequestEn)	NxtStM = M_DREQ;
  	  	          	else        	NxtStM = M_IDLE;
                                        	
  	  	tMDReq  : 	if (FIFOReqEnd)	NxtStM = M_IDLE;
  	  				else			NxtStM = M_DREQ;
  	  	default :               	NxtStM = M_IDLE;
  	endcase
end

always @(negedge nRST or posedge Clk)
	if      (!nRST)   		FIFOReqCnt <= 0;
	else if (FIFOReqEnd) 	FIFOReqCnt <= 0;
	else if (tMDReq)		FIFOReqCnt <= FIFOReqCnt + 1;

assign FIFOReqEnd = (&FIFOReqCnt) | MixXCountEnd;

assign BPlaneDataRequest =  BPlaneRequestX & tMDReq;
assign GPlaneDataRequest =  GPlaneRequestX & tMDReq;
assign VPlaneDataRequest =  VPlaneRequestX & tMDReq;
assign CPlaneDataRequest =  CPlaneRequestX & tMDReq;
//-------------------------------------------------------------------------------
// Display Pixel Count
...
assign MixYCountClr = ~PlaneMixerEn | FIFOClear;
...
always @(negedge nRST or posedge Clk)
	if      (!nRST)   		MixYCount <= 0;
	else if (MixYCountClr) 	MixYCount <= 0;
...
MixerFIFO #(6, 24) MixerFIFO(	// 64x24
			...
			.Full			(MixFIFOFull), 
			.Empty			(MixFIFOEmpty),
			.FullN			(MixFIFOHalfFull), 
			.Level 			(MixFIFOLevel)
);

/*
//assign MixFIFOHalfFull = (MixFIFOLevel==3);
always @(negedge nRST or posedge Clk)	// Should Latch because Different Clock
    if (!nRST)  MixFIFOHalfFull <= 0;
    else		MixFIFOHalfFull <= (MixFIFOLevel==3);
*/
