...
#define DRAM_VIDEO_START	  0x40000000
#define DRAM_VIDEO_NTSC_END	  0x401B7E48
#define DRAM_VIDEO_PAL_END	  0x4041EB00
...
#define VIDEOENC_MR0	    (*(volatile unsigned *)(VIDEOENC_BASEADDR+0x00))
#define VIDEOENC_STADDR0	(*(volatile unsigned *)(VIDEOENC_BASEADDR+0x04))
#define VIDEOENC_ENADDR0	(*(volatile unsigned *)(VIDEOENC_BASEADDR+0x08))
...
int main(void)
{
    ...
    sd2Memcpy("000out_com.out", DRAM_VIDEO_START, (5)*0x100000);

    InData = 0;
    for(i=DRAM_VIDEO_NTSC_END-0x10; (DRAM_VIDEO_NTSC_END + 0x10)>=i;i=i+4) //1801800byte
    {
        long TempData;
        TempData = *((volatile smtUint32 *)(i));
...
0xfffefdfc};
	...
	UART_Initial();
	DebugPrint("UART Test\n");
	InitializeDDRCtrl();
	//DDR_Test();

	volatile smtUint32 * video_testregion = (volatile smtUint32 *)(0x40000000);// + 0x1B7E58);
    count = 0;
    i = 0;

#if 0
	DebugPrint("Write Start.....\n");
    for(i=0x00000000; (0x00000000+0x1B7E48)>=i; i=i+4) //1801800byte
    {
        //video_testregion[i] = Data[InData];
        *((volatile smtUint32 *)(i+0x40000000)) = Data[InData];
	...
	DebugPrint("Read Test .....\n");
    InData = 0;
    //for(i=0x40000000; (0x40000000 + 0x1B7E48)>=i;i=i+4) //1801800byte
    for(i=0x00000000; (0x00000000 + 0x1B7E48)>=i;i=i+4) //1801800byte
    {
        long TempData;
        TempData = *((volatile smtUint32 *)(i+0x40000000));
        if(TempData != Data[InData])
        {
...
#endif
	DebugPrint("Video data write complete.....\n");
	SMT_WRITE(VIDEOENC_STADDR0	, 0x40000000);
	//SMT_WRITE(VIDEOENC_STADDR0	, 0x40400000);
	//SMT_WRITE(VIDEOENC_STADDR0	, 0x4000141C);

#if 0
	SMT_WRITE(VIDEOENC_ENADDR0	, 0x401B7E48); //NTSC end address
	//SMT_WRITE(VIDEOENC_ENADDR0	, 0x4036FC90); //NTSC end address
	//SMT_WRITE(VIDEOENC_ENADDR0	, 0x401B9264); //NTSC end address
	//SMT_WRITE(VIDEOENC_ENADDR0	, 0x4036FC90); //NTSC end address *2 fields
	SMT_WRITE(VIDEOENC_MR0	    , 0x80000000); //NTSC start up
#else
	//SMT_WRITE(VIDEOENC_ENADDR0	, 0x4041EB00); //PAL end address
	SMT_WRITE(VIDEOENC_ENADDR0	, 0x40375F00); //MPAL end address
	//SMT_WRITE(VIDEOENC_ENADDR0	, 0x40C5C100); //PAL end address * 3 = 24fields
	//SMT_WRITE(VIDEOENC_ENADDR0	, 0x4041C3F0); //PAL end address Ref
	//SMT_WRITE(VIDEOENC_ENADDR0	, 0x4081EB00); //PAL end address
	SMT_WRITE(VIDEOENC_MR0	    , 0xC0000000); //PAL start up
#endif

	DebugPrint("Video tester initial.....\n");

    sprintf(OutChar, "addr->%x %x \n",VIDEOENC_STADDR0 , SMT_READ(VIDEOENC_STADDR0));
	DebugPrint(OutChar);
