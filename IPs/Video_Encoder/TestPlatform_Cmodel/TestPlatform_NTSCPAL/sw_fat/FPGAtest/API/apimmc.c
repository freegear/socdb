...
APP_CMD41:	//CMD55
	...
	UART_putchhex16(SMT_READ(SDCmdSta));
	UART_printf("\n\r");
	if ((SMT_READ(SDCmdSta)&RspFin)==RspFin) // CMD55¿¡ ´숑 @´? V´ °?
	{
		...
		if  ((SMT_READ(SDCmdSta)&RspFin)==RspFin)
		{
			...
			UART_printf("\n\r");
			SMT_WRITE(SDCmdSta,0xFFFF); // Clear CmdSent bit
			if((SMT_READ(SDRSP0)&0x80000000)==0x00000000)	// ?μ? initialize°¡ μ??¾?¸¸?¿¸???¼?
			{
				SMT_WRITE(SDCmdSta,0xFFFF); // Clear CmdSent bit
				APP_CMD41arg= SDRSP0; // OCR°ª; ½?¾? busy flag°¡ clearμ?
				goto APP_CMD41;
			}
			else
			{
		...
		else if ((SMT_READ(SDCmdSta)&CmdTout)==CmdTout) // CMD41¿¡ ´숑 @´? ¾??
		{
			// response time out
			//CMD1
	...
	else if ((SMT_READ(SDCmdSta)&CmdTout)==CmdTout) // ACMD41¿¡ ´숑 @´? ¾??
	{
		//UART_getch();
...
//---------------------------------------------------------------------------------------------------
	...
	*/
	...
	if (CardType)
	{
		...
		SMT_WRITE(SDFSTA, 0xFFFFFFFF); // FIFO reset
		SMT_WRITE(SDDatCon,DTST|ByteOrder|DataRx|BlkMode);// SCRG °? P´ ¹?L ´?§
...
*/
	...
	if (CardType)
	{	// Pull up register ¸| ²?'? command ACMD42¸| ³≫º¸³≫¾???
	...
	// SD CardG °? SCR register¸| P¾졁?? ¹?| ¾??´?
	// MMC´ spec version(?)8·?¾?¼?´?
	// SD´ ACMD6; º¸³≫°닊	// MMC´ CMD6; º¸³½´?
	if (CardType)
	{
		// SD CARD ´ ACMD6; º¸³≫ Bus Width¸| º¸³≫¾?μ??
		UART_printf(">> Send CMD55 (APP_CMD)\n\r");
		SDMMCCmdSend(RCA_ADDRESS, CMST|WaitRsp|(CmdIndexVal|55));
		SDMMCWaitRsp();
		UART_printf("---Response CardStatus:  ");
...
*/
	...
	SDMMCClockFreqSet(0x1);  // Low Frequecy  (spec. 400KHz)
...
void SDMMCReadMultiple(smtBoolean CardType, smtUint32 BlockCount, smtUint32 BlockAddress)
{
	...
	for (j=0; j<BlockCount; j++)
	{
		...
		while((SMT_READ(SDFSTA)&0x100)!=0x100); // fifo full ½y¡ read
...
/*
	...
	{
		//CMD23(SET_BLOCK_COUNT) |¼?Рº?G đ °¹¼?¼³d
...
//------------------------------------------------------------------------------
//  Function name   : SDMMCReadDMAtest()
//  Prototype       : void SDMMCReadDMAtest(smtUint32 BlockCount, 
//					  smtUint32 BlockSize, smtUint32 BlockAddress)
//  Return          : 
//  Argument        :
//  Comments        :  SD/MMC multiple read DMA transfer
//------------------------------------------------------------------------------
/*
void 
SDMMCReadDMAtest(
	smtUint32 BlockCount, 		// Block counter of 512byte
	smtUint32 BlockSize, 		// Block size, 512byte
	smtUint32 BlockAddress		// Block address in byte unit
	)
	*/
#if 0
void SD_TestRead(smtUint32 BlockAddress, smtUint32 TargetAddress)	
{
	int i;
	int BlockSize 	= 512;
	int BlockCount 	= 1;
	...
	// noUse:
	// DMACNoDescrp(CH0, &SDDAT, NoIncAdrType, WidthWORD, SDDATA, 
	// IncAdrType, WidthWORD, TSize32B, 512);
	...
	{	
		SMT_WRITE(
			SDDatCon,
			(
				((0x08)<<25)				// DMA transfer size 8 word??(32byte) 
				|DTST						// Data start
				|MMCPlus					// HI-MMC mode
				|WideBus					// 4bit bus mode
				|EnDMA						// Enable DMA
				|DataRx						// Data transfer mode
				|RACMD						// Data receice after command send 
				|BlkMode					// Block transfer mode  
				|TARSP						// Data transfer after recieve resonse
				|((BlockCount-1)&0xFFF)		// Block counter
			)
		);
	}
	else 
	...
	{	
		SMT_WRITE(
			...
			(
				((0x08)<<25)				// DMA transfer size 8 word??(32byte) 
				|DTST						// Data start
				|WideBus					// 4-bit bus mode
				|EnDMA						// Enable DMA
				|DataRx						// Data transfer mode
				|RACMD						// Block transfer mode  
				|BlkMode					// Block transfer mode  
				|TARSP						// Data transfer after recieve resonse
				|((BlockCount-1)&0xFFF)		// Block counter 
			)
	...
	//
	// CMD18: 	continuously transfers data blocks from
	//			card to host until interrupted by a
	//			STOP_TRANSMISSION command.
	// 
	SMT_WRITE(SDCmdSta,0xFFFF);		// Clear command status
	SDMMCCmdSend(					
		BlockAddress, 				// [Argument]: Block address in byte
		CMST						// [CMD CTRL]: Command start setting
		|WaitRsp					// [CMD CTRL]: Wait response
		|CmdIndexVal				// [CMD CTRL]: Start bit
		|18							// [CMD CTRL]:  Multiple read block command
	);
	
	
	//SMT_WRITE (SDAutoReadCon,0x00000007); 
	...
	//
	DMACNoDescrp(
		CH0, 							// [CH ]: Using DMA channel
		(MMC_BASEADDR+0x44), 			// [SRC]: Source address
		NoIncAdrType, 					// [SRC]: Non increament
		WidthWORD, 						// [SRC]: Word data width 
		(unsigned int) TargetAddress, 	// [TRG]: Target address
		IncAdrType, 					// [TRG]: auto increamnet
		WidthWORD, 						// [TRG]: Word data width
		TSize32B, 						// [BS ]: Burst size
		BlockSize*BlockCount			// [LEN]: Total transfered size
	);
	...
	SMT_WRITE(SDCmdSta,0xFFFF);		// Clear command status
	SDMMCCmdSend(					
		0x0			, 				// [Argument]: Block address in byte
		CMST						// [CMD CTRL]: Command start setting
		|WaitRsp					// [CMD CTRL]: Wait response
		|CmdIndexVal				// [CMD CTRL]: Start bit
		|12							// [CMD CTRL]:  Multiple read block command
	);
...
void SD_TestRead(smtUint32 BlockAddress, smtUint32 TargetAddress)
{
	...
	unsigned int			*pTrg = (unsigned int*)TargetAddress;
	int						BlockSize = 512;
	...
	//Block Size Setting
	//
	RegisterData	= BlockSize-1;
	SMT_WRITE(SDBSize, RegisterData);

	//
	// Data Nooperation setting for FIFO Flush (because of DMA signal)
	//
	RegisterData	= 0x001A0000;
	SMT_WRITE(SDDatCon, RegisterData);
	
	//
	// FIFO status clear
	//
	RegisterData	= 0xFFFFFFFF;
	SMT_WRITE(SDFSTA, RegisterData);
	
	//
	// Set SD Data control register settting
	//
	if (BitMode8)
	{	
		
		RegisterData	= 
						DTST		// Data transfer start
		   				|MMCPlus	// 8bit bus for HI-MMC
		   				|WideBus	// Wide bus setting
		   				|DataRx		// Data Rx setting
		   				|RACMD		// Data receice after command send
		   				|BlkMode	// Block transfer mode
		   				|TARSP;		// Data transfer after recieve resonse
	...
	//
	SMT_WRITE(SDCmdSta,0xFFFF); 	// Clear SD command status
	SDMMCCmdSend(BlockAddress,		// [Argument]: Block address in byte
	 			 CMST				// [CMD CTRL]: Command start setting
				|WaitRsp			// [CMD CTRL]: Wait response
				|CmdIndexVal		// [CMD CTRL]: Start bit
				|17);				// [CMD CTRL]: single Block read data command
	
	//
	// Wait RX fifo avail detection
	//
	while(1) 
	{	
		if(SMT_READ(SDFSTA)&0x1000)
		{
			break;
		}
	}
	...
	//
	// Read data by polling, in 4byte unit (counter 129 = 512/4)
	//
	for (i=1; i<129 ; i++)	
	{
...
#if 0
void SD_TestMRead(smtUint32 BlockAddress, smtUint32 TargetAddress, int BlockCount)	
{
	int i;
	int BlockSize 	= 512;
...
void SD_TestMRead(smtUint32 BlockAddress, smtUint32 TargetAddress, int BlockCount)	
{
    ...
    for(SectorCnt = 0; SectorCnt < BlockCount; SectorCnt++) {
        SD_TestRead(BlockAddress, TargetAddress);
        BlockAddress    += 512;
        TargetAddress   += 512;
    }
