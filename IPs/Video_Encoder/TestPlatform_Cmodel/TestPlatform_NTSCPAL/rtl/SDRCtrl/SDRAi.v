...
`else
assign  iAWReady = ~WBLQHFull & ~WQFull & ~QueErr & ~QueFul & (tWIdle | tWWSpi);
`endif
...
always @(negedge ARESETB or posedge ACLK)
  	if 		(!ARESETB) BId <= 0;
  	//else if (iAWValid) BId <= AWId;
  	else if (WDatAck)  BId <= WId;
//-------------------------------------------------------------
// Write Data Path
reg [BL:0]   WBurstCnt;
reg [BL:0]   LatchedWBL;
...
`else
assign  iARReady = tRIdle & ~QueErr & ~QueFul & ~RQHFull & ~RBLQHFull;
`endif
