...
output        FIFOAlmostEmpty, FIFOAlmostFull ;
output        FIFOEmpty ;
output        FIFOEmptyWr ;
...
-----------------------------------------------------------------------------------------------------------*/
ScFIFOCtl2048x16 #(FIFO_AW, FIFO_ML) FIFOCtl
(
    .Clki			(Clk),
    .ReadEni		(FIFORead),
    .WriteEni		(FIFOWrite),
...
parameter AW = 11 ;// address length parameter - depends on fifo depth
parameter ML = 2048-8; // MaxLatency
...
assign AlmostEmpty  = DatCnt>=(4);
assign AlmostFull   = DatCnt>=({AW{1'b1}}-1024);
