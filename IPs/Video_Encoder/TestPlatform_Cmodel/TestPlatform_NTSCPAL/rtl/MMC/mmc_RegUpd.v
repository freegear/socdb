...
input	[1:0]	iAutoReadCon;
input			SDIAutoReadConUpdSync;
...
output	[1:0]	SDIAutoReadCon;
output	        SDIAutoReadConUpdDone;
...
reg		[1:0] NextSDIAutoReadCon;
reg			NextSDIAutoReadConUpdDone;
...
reg		[1:0]	SDIAutoReadCon;
reg		SDIAutoReadConUpdDone;
...
begin 
  if (nRst == 1'b0)
	...
	begin
		...
		DelSDIAutoReadConUpdSync <= SDIAutoReadConUpdSync;
    	end
	end
end
...
assign SDIAutoReadConStg2WrEn     = SDIAutoReadConUpdSync  ^ DelSDIAutoReadConUpdSync;
...
always @(SDIAutoReadConStg2WrEn or  iAutoReadCon or SDIAutoReadCon or SDIAutoReadConUpdDone)
begin
  if (SDIAutoReadConStg2WrEn == 1'b1)
    begin
      NextSDIAutoReadCon    	= iAutoReadCon ;
      NextSDIAutoReadConUpdDone = ~(SDIAutoReadConUpdDone);
    end
  else
    begin
      NextSDIAutoReadCon  	= SDIAutoReadCon;
      NextSDIAutoReadConUpdDone = SDIAutoReadConUpdDone;
    end
end
...
begin
  if (nRst == 1'b0)
    begin
	  ...
	  SDIAutoReadCon	<= 2'b00;
	  SDIAutoReadConUpdDone<= 1'b0;
	end
		...
		begin
			...
			SDIAutoReadCon		<= NextSDIAutoReadCon;
			SDIAutoReadConUpdDone<= NextSDIAutoReadConUpdDone;
			end
	    end
end 
endmodule
