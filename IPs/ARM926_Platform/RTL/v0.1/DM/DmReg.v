...
always @(negedge PRESETB or posedge PCLK)
	if (!PRESETB) begin
		...
		GPalMemWrAddr		<= PWDATA[31:24];
	    GPalMemWrData  		<= PWDATA[23:0];
	end

`ifdef PCLKisACLK
assign PalMemWrAddr = GPalMemWrite ? GPalMemWrAddr : GPalMemRdAddr;
...
always @(negedge ARESETn or posedge ACLK)
	if (!ARESETn) begin
		...
		PalMemWrAddr <= GPalMemWrite ? GPalMemWrAddr : GPalMemRdAddr;
...
always @(negedge PRESETB or posedge PCLK)
	if 	(!PRESETB)		  PRDATA <= 32'b0;
		...
		case(1'b1) // synopsys parallel_case
			...
			rGPALMRd	: PRDATA <= {GPalMemWrAddr, PalMemRdData};
