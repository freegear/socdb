...
input  [XW:0] 	GPlaneXSize;	// max 64x64
input  [YW:0] 	GPlaneYSize;
output [XW:0]	GPlaneXSizeRef;
...
//-------------------------------------------------------------------------------
assign GraphicEnDis = GraYCountEnd | FIFOClear;
...
always @(GPlanePixFormat16 or GPlanePixFormat32 or GPlaneXSize)
	case(1'b1) // synopsys parallel_case
		GPlanePixFormat16 : GPlaneXSizeRef = {1'b0, GPlaneXSize[XW:1]}; // 16bit, /2
		GPlanePixFormat32 : GPlaneXSizeRef =  	    GPlaneXSize[XW:0] ; // 32bit
		default 		  : GPlaneXSizeRef = {2'b0, GPlaneXSize[XW:2]}; // 8bit, /4
	endcase
...
assign PalMemAddr = PalMemWrite ? PalMemWrAddr : PalMemRdAddr;

PaletteMem #(8, 24) PaletteMem(
			.clk 		(Clk), 
			.rst 		(nRST), 
//			.ce  		(GraphicEn),
			.ce  		(1'b1),
...
//ScFIFO32x32 #(5, 6, 32) GraphicFIFO(
