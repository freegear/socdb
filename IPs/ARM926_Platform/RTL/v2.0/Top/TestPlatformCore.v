...
//
// history :
//     2006.12.11 : modify JTAG signal for Multi-ICE
//
//
...
`endif
   ...
   //     (PSEL1_6) 0x3000_6000 - 0x2000_6FFC : Display Controller register
   //     
   //
   ...
   input 	 RESETn;
   input 	 Clock;
   input 	 VINITHI; // select the vector table position
   ...
   output        led0;           // test signal for altera FPGA board. 2006.12.5
   output 		 led1;           // test signal for altera FPGA board. 2006.12.5
...
`endif
   ...
   // AXI Master 0 : ARM Instruction (Read Port Only)
   ...
   // AXI Master 2 : Display Controller(Read Port Only)
   ...
   // AXI Master 4 : Video Input(BT656 Interface) : Write Port Only
   ...
   //-------------------------------------------
   // AXI Master2 : Display Controller
   wire 		 DmErrInt;
...
`endif
   ...
   //-----------------------------------
   // cpu clock monitering
   reg [23:0] 		  cpu_clk_div;
   
   
   always@(posedge ACLK_CPU or negedge ARESETn_CPU) begin
	  if(!ARESETn_CPU) begin
		 cpu_clk_div <= {24{1'b0}};
	  end // if
	  else begin
		 cpu_clk_div <= cpu_clk_div + 24'h000001;
	  end // else
   end // always

   assign led0 = cpu_clk_div[23];
   

   // bus clock monitering
   reg [23:0] 		  bus_clk_div;
   ...
   always@(posedge ACLK_BUS or negedge ARESETn) begin
	  if(!ARESETn) begin
		 ...
		 bus_clk_div <= bus_clk_div + 24'h000001;
	  end // else
   end // always

   assign led1 = bus_clk_div[23];
   ...
   //==================================================
   // AXI Master 0 and 1 : ARM Instruction/Data
   wire 		 ARMnFIQ;
   wire 		 ARMnIRQ;
   ...
   arm_axi    cpu
	 (
	  ...
	  .ACLK_CPU            ( ACLK_CPU ),
	  .ACLK_BUS            ( ACLK_BUS ),
	  .BusClockEn          ( BusClockEn ), 	// BUS clock rising indicator
	  .ARESETn             ( ARESETn_CPU ),
	  .VINITHI             ( VINITHI ),
	  ...
	  `ifdef FPGA
	  .DBG0                ( DBG0 ),       // FPGA debug 
	  .DBG1                ( DBG1 ),       // FPGA debug 
	  .DSWITCH             ( DSWITCH ),    // FPGA debug  (from DIP switches)
	  `endif
   ...
   assign 		 DMAReq = {7'd0, MMC_DMAREQ};	// Now NO DMA
   ...
   //--------------------------------------------
   // AXI Master4 : Video Input(BT656 Interface)
   ...
   // APB1 Slave0 : VIC
   assign 		 PREADY1_0 = 1;
   assign 		 IntSrc = { 13'd0, TimerIntOverFlow, TimerIntMatch, I2CInt[1], I2CAAS[1], I2CInt[0], I2CAAS[0], MMCInt, DmErrInt, UARTRXINT, UARTTXINT, UARTINT, DMAInterrupt };
   ...
   jtag_sync      arm_jtag_sync
	 (
	  ...
	  // ARM JTAG signal
	  .DBGnTRST        ( DBGnTRST ), // connectted to etrstb
	  .DBGTCKEN        ( DBGTCKEN ),
	  .DBGTDI          ( DBGTDI ),
	  .DBGTMS          ( DBGTMS ),
	  .DBGTDO          ( DBGTDO ) 
   ...
   assign 		 EDBGRQ = eedbgrq;
   assign 		 edbgack = DBGACK;
   assign 		 edbgir = DBGIR;
   
   
endmodule // TestPlatformCore
