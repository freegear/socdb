...
//
    ...
    //new_pix_counter
    unsigned int  h_new_pix_num_base=0;
...
//******************************************************
        if(w_buf_en)
        {
            ...
            if(uu && new_pix_num==0)
                    fprintf(out_file4,"in_buf_r    : %02x in_buf_g : %02x in_buf_b : %02x \n"
                                       ,in_buf_r[0],in_buf_g[0],in_buf_b[0]);
            ...
            if(uu && new_pix_num==0)
                ...
                new_mem[new_pix_num]=0xffffff & ((cal_buf_r[0]<<16) | (cal_buf_g[0]<<8) | (cal_buf_b[0]));
...
//******************************************************
        ...
        fprintf(out_file4,"h_addr0 is  : %d :: v_addr0 %d\n",h_addr0,v_addr0);    
        fprintf(out_file4,"h_addr1 is  : %d :: v_addr1 %d\n",h_addr1,v_addr1);    
        fprintf(out_file4,"++++++++++++++++++\n");
#endif
...
//                rd_cmd=0x01;
        ...
        else if(cal_cs==UU_HUP) ////// UP_UP_HUP //////
        {
            ...
            if(up_ready) 
            {
                ...
                {
                    if(h_seq_cnt==3 || h_new_pix_num==new_width-1) 
                    {
                        h_seq_cnt=0;
                        ...
                        if(height_cnt==new_height-1)
                        {
                            ...
                            h_new_pix_num_base=h_new_pix_num+1;
                            rd_num=4;
                            rd_cmd=0x01;//2-line read
                            v_new_pix_num=0;
                            rd_v_pix_num=0;
                        ...
                        else
                        {
                            ...
                            h_new_pix_num=h_new_pix_num_base;
                            h_new_pix_num_dly=h_new_pix_num_base;
                        }
                    }
                    else
                    {
                        ...
                        else//next memory로 pixel계산  
                        {
                            h_seq_cnt=h_seq_cnt+1;
                            h_comp_val=h_rate_cnt>>16;
                        }//if((h_rate_cnt>>16)==h_comp_val)
                    }//if(h_seq_cnt==3 || width_cnt==new_width) else //horizontal끝 vertical시작
                }//if(src_write) else
            }//if(ready)
        }// end UU_HUP
...
/*                    if(height_cnt==new_height-1)
                    ...
                    else v_new_pix_num=v_new_pix_num+new_width;
*/                  v_new_pix_num=v_new_pix_num+new_width;
                ...
                else
                {
                    if(v_rate_cnt>>16==v_comp_val)
                    {
                        if(v_seq_cnt==0)
                        {
                            if(h_rate_cnt==h_rate_base && rd_en && height_cnt!=new_height-1) // 추가 & ) //pipeline  memory refill
                            {
                                rd_en=0;
                                rd_num=4;
                                rd_cmd=0x20;//1-line read continue
                        ...
                        fprintf(out_file4,"HHHHHHHHHUUUUUUUU\n");
                            v_addr0=v_addr1;
                        }
                        up_enable=1;
                        mul=v_rate_cnt;
                    ...
                    {
                        if(r_mem_cs!=IDLE1 || r_mem_ns!=IDLE1)
                        {
                            go_wait=1;
                            before_state=UU_VUP;
                        }
...
*/
        ...
        {
            go_wait=0;
            r_uu_hup = end_fill & (before_state==UU_HUP);
            r_uu_vup = end_fill & (before_state==UU_VUP);
        ...
        if(up_enable) //****** up scale *******
        {
            ...
            if(cal_cs==UU_VUP)
            {
				fprintf(out_file4,"cal_buf_r : %02x  cal_buf_g : %02x  cal_buf_g : %02x :: v_seq_cnt-1=%d\n",
				        cal_buf_r[v_seq_cnt-1],cal_buf_g[v_seq_cnt-1],cal_buf_b[v_seq_cnt-1],v_seq_cnt-1);
            }
        ...
        switch(cal_cs)
        {
            ...
            case WAIT://10
                ...
                else if(r_uu_vup) cal_ns=UU_VUP;
...
//****************** end mem_read **********************
    ...
    for(i=0;i<new_width*new_height;i++)
	{
		...
		fputc((new_mem[i])&0x0ff,out_file2);
		fputc((new_mem[i]>>8)&0x0ff,out_file2);
		fputc((new_mem[i]>>16)&0x0ff,out_file2);
	}
	fclose(out_file2);
	fclose(out_file3);
	fclose(out_file4);
