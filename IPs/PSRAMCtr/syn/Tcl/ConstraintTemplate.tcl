...
pCreateClock M_PCLK    $PERIOD1 $POS_PERIOD1 $NEG_PERIOD1 $CLK_UNCERTAINTY1

pCreateClock R_PCLK    $PERIOD2 $POS_PERIOD2 $NEG_PERIOD2 $CLK_UNCERTAINTY2

#-- Not Defined Inter-Clock Uncertainty
#set_clock_uncertainty -setup $CLK_UNCERTAINTY1 -from M_PCLK -to PCLK
#set_clock_uncertainty -setup $CLK_UNCERTAINTY1 -from PCLK -to M_PCLK
...
pCreateReset PRESETn  M_PCLK $tidmaxresetb $tidminresetb
...
#----========================================================================----
set all_inputs_no_clocks [remove_from_collection [all_inputs] [list M_PCLK R_PCLK]]
...
#-- APB Interface
#-- Maximum allowable Input Delay 
set tapbinmax  [expr $PERIOD2 * 0.6]

#-- Maximum allowable Output Delay 
set tapbdomax  [expr $PERIOD2 * 0.8]
#set tapbdomax  [expr $PERIOD2 * 0.2]

set tidprimaryipmax [expr 0.7 * $PERIOD1]
...
if {[llength [find port R_*]] != 0} {
  if {[llength [filter [find port R_*] {@port_direction == in} ]] != 0} {
    set input_port_list [filter [find port R_*] {@port_direction == in}]
    foreach_in_collection input_port_name $input_port_list {
      set_input_delay -clock R_PCLK -min $tholdmax $input_port_name
      set_input_delay -clock R_PCLK -max $tapbinmax $input_port_name
    }
    unset input_port_list
  }
}

if {[llength [find port M_*]] != 0} {
  if {[llength [filter [find port M_*] {@port_direction == in} ]] != 0} {
    set input_port_list [filter [find port M_*] {@port_direction == in}]
    foreach_in_collection input_port_name $input_port_list {
      set_input_delay -clock M_PCLK -min $tidprimaryipmin $input_port_name
      set_input_delay -clock M_PCLK -max $tidprimaryipmax $input_port_name
    }
    unset input_port_list
  }
}
...
if {[llength [find port M_*]] != 0} {
  if {[llength [filter [find port M_*] {@port_direction == out} ]] != 0} {
    set output_port_list [filter [find port M_*] {@port_direction == out}]
    foreach_in_collection output_port_name $output_port_list {
      set_output_delay -clock M_PCLK -min $todprimaryopmin  $output_port_name
      set_output_delay -clock M_PCLK -max $todprimaryopmax  $output_port_name
    }
...
if {[llength [find port R_*]] != 0} {
  if {[llength [filter [find port R_*] {@port_direction == out} ]] != 0} {
    set output_port_list [filter [find port R_*] {@port_direction == out}]
    foreach_in_collection output_port_name $output_port_list {
      set_output_delay -clock R_PCLK -min $tholdmin $output_port_name
      set_output_delay -clock R_PCLK -max $tapbdomax $output_port_name
    }
    unset output_port_list
  }
}
